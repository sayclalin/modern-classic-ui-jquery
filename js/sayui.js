...
var sayutils = (function ($) {
    ...
    function isFloat (val) {
        if (typeof val == 'string') val = val.replace(/\s+/g, '').replace(sayutils.settings.groupSymbol, '').replace(sayutils.settings.decimalSymbol, '.');
        return (typeof val === 'number' || (typeof val === 'string' && val !== '')) && !isNaN(Number(val));
    }

    function isMoney (val) {
        var se = sayutils.settings;
    ...
    function isDate (val, format, retDate) {
        ...
        if (format == null) format = sayutils.settings.dateFormat;
        ...
        if (typeof val.getUTCFullYear === 'function') { // date object
            ...
            if (new RegExp('mon', 'ig').test(format)) {
                ...
                for (var m = 0, len = sayutils.settings.fullmonths.length; m < len; m++) {
                    var t = sayutils.settings.fullmonths[m];
    ...
    function isDateTime (val, format, retDate) {
        if (format == null) format = sayutils.settings.datetimeFormat;
        ...
        if (typeof val.getUTCFullYear === 'function') { // date object
            ...
            // check
            var tmp1 = sayutils.isDate(values[0], formats[0], true);
            var tmp2 = sayutils.isTime(values[1], true);
    ...
    function date (dateStr) {
        ...
        if (sayutils.isInt(dateStr)) d1 = new Date(Number(dateStr)); // for unix timestamps
        if (String(d1) == 'Invalid Date') return '';

        var months = sayutils.settings.shortmonths;
        ...
        var dsp = dd1;
        if (dd1 === dd2) dsp = time;
        if (dd1 === dd3) dsp = sayutils.lang('Yesterday');

        return '<span title="'+ dd1 +' ' + time2 +'">'+ dsp +'</span>';
    }

    function formatSize (sizeStr) {
        if (!sayutils.isFloat(sizeStr) || sizeStr === '') return '';
    ...
    function formatNumber (val, fraction, useGrouping) {
        ...
        return parseFloat(val).toLocaleString(sayutils.settings.locale, options);
    }
    ...
    function formatDate (dateStr, format) { // IMPORTANT dateStr HAS TO BE valid JavaScript Date String
        ...
        if (sayutils.isInt(dateStr)) dt = new Date(Number(dateStr)); // for unix timestamps
        ...
        return format.toLowerCase()
            .replace('month', sayutils.settings.fullmonths[month])
            .replace('mon', sayutils.settings.shortmonths[month])
            .replace(/yyyy/g, year)
            .replace(/yyy/g, year)
    ...
    function formatTime (dateStr, format) { // IMPORTANT dateStr HAS TO BE valid JavaScript Date String
        var months = sayutils.settings.shortmonths;
        var fullMonths = sayutils.settings.fullmonths;
        if (!format) format = this.settings.timeFormat;
        if (dateStr === '' || dateStr == null || (typeof dateStr == 'object' && !dateStr.getMonth)) return '';

        var dt = new Date(dateStr);
        if (sayutils.isInt(dateStr)) dt  = new Date(Number(dateStr)); // for unix timestamps
        if (sayutils.isTime(dateStr)) {
            var tmp = sayutils.isTime(dateStr, true);
            dt = new Date();
    ...
    function transition (div_old, div_new, type, callBack) {
        ...
        setTimeout(function () {
            ...
            if (div_new) {
                $(div_new).css({ 'opacity': '1' }).css(sayutils.cssPrefix({
                    'transition': '',
                    'transform' : ''
                }));
            }
            if (div_old) {
                $(div_old).css({ 'opacity': '1' }).css(sayutils.cssPrefix({
                    'transition': '',
                    'transform' : ''
                }));
            }
    ...
    function lock (box, msg, spinner) {
        ...
        sayutils.unlock(box);
        $(box).prepend(
            '<div class="sayui-lock"></div>'+
            '<div class="sayui-lock-msg"></div>'
        );
        var $lock = $(box).find('.sayui-lock');
        var mess = $(box).find('.sayui-lock-msg');
        if (!options.msg) mess.css({ 'background-color': 'transparent', 'border': '0px' });
        if (options.spinner === true) options.msg = '<div class="sayui-spinner" '+ (!options.msg ? 'style="width: 35px; height: 35px"' : '') +'></div>' + options.msg;
    ...
    function unlock (box, speed) {
        if (isInt(speed)) {
            $(box).find('.sayui-lock').fadeOut(speed);
            setTimeout(function () {
                $(box).find('.sayui-lock').remove();
                $(box).find('.sayui-lock-msg').remove();
            }, speed);
        } else {
            $(box).find('.sayui-lock').remove();
            $(box).find('.sayui-lock-msg').remove();
    ...
    /**
    *  Used in saypopup, saygrid, sayform, saylayout
    *  should be called with .call(...) method
    */

    function message(where, options) {
        var obj = this, closeTimer, edata;
        // var where.path    = 'saypopup';
        // var where.title   = '.sayui-popup-title';
        // var where.body    = '.sayui-box';
        $().saytag(); // hide all tags
        if (!options) options = { width: 200, height: 100 };
        if (options.on == null) $.extend(options, sayutils.event);
        ...
        var $tmp = $(where.box).find('.sayui-message.sayui-closing');
        if ($(where.box).find('.sayui-message.sayui-closing').length > 0) {
            clearTimeout(closeTimer);
            closeCB($tmp, $tmp.data('options') || {});
        }
        var msgCount = $(where.box).find('.sayui-message').length;
        // remove message
        if ($.trim(options.html) === '' && $.trim(options.body) === '' && $.trim(options.buttons) === '') {
            if (msgCount === 0) return; // no messages at all
            var $msg = $(where.box).find('#sayui-message'+ (msgCount-1));
            var options = $msg.data('options') || {};
            // before event
            edata = options.trigger({ phase: 'before', type: 'close', target: 'self' });
            if (edata.isCancelled === true) return;
            // default behavior
            $msg.css(sayutils.cssPrefix({
                'transition': '0.15s',
                'transform': 'translateY(-' + options.height + 'px)'
            })).addClass('sayui-closing');
            if (msgCount == 1) {
                if (this.unlock) {
                    if (where.param) this.unlock(where.param, 150); else this.unlock(150);
                }
            } else {
                $(where.box).find('#sayui-message'+ (msgCount-2)).css('z-index', 1500);
            }
            ...
            if ($.trim(options.body) !== '' || $.trim(options.buttons) !== '') {
                options.html = '<div class="sayui-message-body">'+ (options.body || '') +'</div>'+
                    '<div class="sayui-message-buttons">'+ (options.buttons || '') +'</div>';
            }
            // hide previous messages
            $(where.box).find('.sayui-message').css('z-index', 1390);
            head.data('old-z-index', head.css('z-index'));
            head.css('z-index', 1501);
            // add message
            $(where.box).find(where.body)
                .before('<div id="sayui-message' + msgCount + '" onmousedown="event.stopPropagation();" '+
                        '   class="sayui-message" style="display: none; z-index: 1500; ' +
                            (head.length === 0 ? 'top: 0px;' : 'top: ' + sayutils.getSize(head, 'height') + 'px;') +
                            ...
                            sayutils.cssPrefix('transition', '.3s', true) + '"' +
                            (options.hideOnClick === true
                                ...
                                : '') + '>' +
                        '</div>');
            $(where.box).find('#sayui-message'+ msgCount)
                .data('options', options)
                .data('prev_focus', $(':focus'));
            var display = $(where.box).find('#sayui-message'+ msgCount).css('display');
            $(where.box).find('#sayui-message'+ msgCount).css(sayutils.cssPrefix({
                'transform': (display == 'none' ? 'translateY(-' + options.height + 'px)' : 'translateY(0px)')
            }));
            if (display == 'none') {
                $(where.box).find('#sayui-message'+ msgCount).show().html(options.html);
                options.box = $(where.box).find('#sayui-message'+ msgCount);
                // before event
                edata = options.trigger({ phase: 'before', type: 'open', target: 'self' });
                if (edata.isCancelled === true) {
                    head.css('z-index', head.data('old-z-index'));
                    $(where.box).find('#sayui-message'+ msgCount).remove();
                    return;
                }
                ...
                setTimeout(function() {
                    ...
                    $(where.box).find('#sayui-message'+ msgCount).css(sayutils.cssPrefix({ 'transition': '0s' }));
                    // event after
    ...
    function locale (locale) {
        ...
        if(typeof locale !== "string" ) {
            sayutils.settings = $.extend(true, sayutils.settings, locale);
            return;
        }
        ...
        $.ajax({
            ...
            success  : function (data, status, xhr) {
                sayutils.settings = $.extend(true, sayutils.settings, data);
            },
    ...
    function checkName (params, component) { // was saycheckNameParam
        ...
        if (!sayutils.isAlphaNumeric(params.name)) {
    ...
    function checkUniqueId (id, items, itemsDecription, objName) { // was saycheckUniqueId
...
sayutils.formatters = {
    ...
    'number': function (value, params) {
        ...
        return sayutils.formatNumber(parseFloat(value), params, true);
    },

    'float': function (value, params) {
        return sayutils.formatters['number'](value, params);
    },

    'int': function (value, params) {
        return sayutils.formatters['number'](value, 0);
    },

    'money': function (value, params) {
        if (value == null || value === '') return '';
        var data = sayutils.formatNumber(Number(value), sayutils.settings.currencyPrecision || 2);
        return (sayutils.settings.currencyPrefix || '') + data + (sayutils.settings.currencySuffix || '');
    },

    'currency': function (value, params) {
        return sayutils.formatters['money'](value, params);
    },

    'percent': function (value, params) {
        if (value == null || value === '') return '';
        return sayutils.formatNumber(value, params || 1) + '%';
    },

    'size': function (value, params) {
        if (value == null || value === '') return '';
        return sayutils.formatSize(parseInt(value));
    },

    'date': function (value, params) {
        if (params === '') params = sayutils.settings.dateFormat;
        if (value == null || value === 0 || value === '') return '';
        var dt = sayutils.isDateTime(value, params, true);
        if (dt === false) dt = sayutils.isDate(value, params, true);
        return '<span title="'+ dt +'">' + sayutils.formatDate(dt, params) + '</span>';
    },

    'datetime': function (value, params) {
        if (params === '') params = sayutils.settings.datetimeFormat;
        if (value == null || value === 0 || value === '') return '';
        var dt = sayutils.isDateTime(value, params, true);
        if (dt === false) dt = sayutils.isDate(value, params, true);
        return '<span title="'+ dt +'">' + sayutils.formatDateTime(dt, params) + '</span>';
    },

    'time': function (value, params) {
        if (params === '') params = sayutils.settings.timeFormat;
        if (params === 'h12') params = 'hh:mi pm';
        if (params === 'h24') params = 'h24:mi';
        if (value == null || value === 0 || value === '') return '';
        var dt = sayutils.isDateTime(value, params, true);
        if (dt === false) dt = sayutils.isDate(value, params, true);
        return '<span title="'+ dt +'">' + sayutils.formatTime(value, params) + '</span>';
    },
    ...
    'age': function (value, params) {
        if (value == null || value === 0 || value === '') return '';
        var dt = sayutils.isDateTime(value, null, true);
        if (dt === false) dt = sayutils.isDate(value, null, true);
        return '<span title="'+ dt +'">' + sayutils.age(value) + (params ? (' ' + params) : '') + '</span>';
    },

    'interval': function (value, params) {
        if (value == null || value === 0 || value === '') return '';
        return sayutils.interval(value) + (params ? (' ' + params) : '');
    },
...
(function ($) {
    ...
    $.fn.sayrender = function (name) {
        if ($(this).length > 0) {
            if (typeof name === 'string' && sayui[name]) sayui[name].render($(this)[0]);
            if (typeof name === 'object') name.render($(this)[0]);
        }
    };

    $.fn.saydestroy = function (name) {
        if (!name && this.length > 0) name = this.attr('name');
        if (typeof name === 'string' && sayui[name]) sayui[name].destroy();
        if (typeof name === 'object') name.destroy();
    };

    $.fn.saymarker = function () {
        ...
        if (str.length === 0 || !str[0]) { // remove marker
            ...
            return $(this).each(function (index, el) {
                ...
                function replaceValue(matched) { // mark new
                    return '<span class="sayui-marker">' + matched + '</span>';
                }
            });
        }

        function clearMarkedText(index, el) {
            while (el.innerHTML.indexOf('<span class="sayui-marker">') != -1) {
                el.innerHTML = el.innerHTML.replace(/\<span class=\"sayui\-marker\"\>((.|\n|\r)*)\<\/span\>/ig, '$1'); // unmark
            }
        }
    };

    // -- saytag - there can be multiple on screen at a time

    $.fn.saytag = function (text, options) {
        // only one argument
        ...
        // remove all tags
        if ($(this).length === 0) {
            $('.sayui-tag').each(function (index, el) {
        ...
        return $(this).each(function (index, el) {
            ...
            var tagID  = sayutils.escapeId(origID);
            var $tags  = $('#sayui-tag-'+tagID);
            if (text === '' || text == null) {
                // remmove element
                $tags.css('opacity', 0);
                ...
                // insert
                $('body').append(
                    '<div onclick="event.stopPropagation()" style="display:none;" id="sayui-tag-'+ origID +'" '+
                    '       class="sayui-tag '+ ($(el).parents('.sayui-popup, .sayui-overlay-popup, .sayui-message').length > 0 ? 'sayui-tag-popup' : '') + '">'+
                    '   <div style="margin: -2px 0px 0px -2px; white-space: nowrap;">'+
                    '      <div class="sayui-tag-body '+ options.className +'" style="'+ (options.style || '') +'">'+ text +'</div>'+
                    '   </div>' +
                    '</div>');
                $tags = $('#sayui-tag-'+tagID);
                $(el).data('saytag', $tags.get(0)).data('checkIfMoved', checkIfMoved);
            }
            ...
            setTimeout(function () {
                ...
                $tags.css({
                    ...
                    .find('.sayui-tag-body').addClass(pos['posClass']);
                ...
                if (options.hideOnKeyPress) {
                    $(el).on('keypress.saytag', hideTag);
                }
                if (options.hideOnBlur) {
                    $(el).on('blur.saytag', hideTag);
                }
                if (options.hideOnClick) {
                    $(document).on('click.saytag', hideTag)
                }
            ...
            function checkIfMoved(checkOnly, instant) {
                ...
                if ($(el).length === 0 || (offset.left === 0 && offset.top === 0) || $tags.find('.sayui-tag-body').length === 0) {
                    clearInterval($tags.data('timer'));
                    hideTag();
                    return;
                }
                ...
                var tagBody  = $tags.find('.sayui-tag-body');
                ...
                if (options.position == 'top') {
                    ...
                    if (tagBody.data('posClass') !== posClass) {
                        tagBody.removeClass('sayui-tag-right sayui-tag-left sayui-tag-top sayui-tag-bottom')
                            .addClass(posClass)
                            .data('posClass', posClass);
                    }
                }
                if ($tags.data('position') !== posLeft + 'x' + posTop && checkOnly !== true) {
                    $tags.css(sayutils.cssPrefix({ 'transition': (instant ? '0s' : '.2s') })).css({
                        left: posLeft + 'px',
                        top : posTop + 'px'
    ...
    // sayoverlay - appears under the element, there can be only one at a time

    $.fn.sayoverlay = function (html, options) {
        var obj  = this;
        var name = '';
        var defaults = {
        ...
        if (this.length === 0 || options.html === '' || options.html == null) {
            if ($('#sayui-overlay'+ name).length > 0) {
                tmp_hide = $('#sayui-overlay'+ name)[0].hide;
                if (typeof tmp_hide === 'function') tmp_hide();
            } else {
                $('#sayui-overlay'+ name).remove();
            }
            return $(this);
        }
        ...
        // append
        $('body').append(
            '<div id="sayui-overlay'+ name +'" style="display: none; left: 0px; top: 0px; '+ options.overlayStyle +'"'+
            '        class="sayui-reset sayui-overlay '+ ($(this).parents('.sayui-popup, .sayui-overlay-popup, .sayui-message').length > 0 ? 'sayui-overlay-popup' : '') +'">'+
            '    <style></style>'+
            '    <div style="min-width: 100%; '+ options.style +'" class="'+ options['class'] +'"></div>'+
            '</div>'
        );
        // init
        var div1 = $('#sayui-overlay'+ name);
        ...
        div1.data('element', obj.length > 0 ? obj[0] : null)
            ...
            .on('mousedown', function (event) {
                $('#sayui-overlay'+ name).data('keepOpen', true);
        ...
        setTimeout(function () {
            resize();
            $(document).off('.sayoverlayHide').on('click.sayoverlayHide', hide);
            if (typeof options.onShow === 'function') options.onShow();
        }, 10);
        ...
        function resize () {
            var div1 = $('#sayui-overlay'+ name);
            var div2 = div1.find(' > div');
            var menu = $('#sayui-overlay'+ name +' div.menu');
            ...
            if (div1.length > 0) {
                ...
                switch (options.align) {
                    case 'both':
                        boxLeft = 17;
                        if (options.width === 0) options.width = sayutils.getSize($(obj), 'width');
                    ...
                    case 'left':
                        ...
                        break;
                    case 'right':
                        boxLeft = sayutils.getSize($(obj), 'width') - w + 10;
                        tipLeft = w - 40;
                        break;
                }
                ...
                if (options.contextMenu) { // context menu
                    ...
                    Y = (offset.top + sayutils.getSize(obj, 'height') + options.top + 7);
                ...
                if ((maxHeight > -50 && maxHeight < 210) || options.openAbove === true) {
                    ...
                    div1.find('>style').html(
                        '#sayui-overlay'+ name +':before { margin-left: '+ parseInt(tipLeft) +'px; }'+
                        '#sayui-overlay'+ name +':after { margin-left: '+ parseInt(tipLeft) +'px; }'
                    );
                } else {
                    // show under
                    if (options.maxHeight && maxHeight > options.maxHeight) maxHeight = options.maxHeight;
                ...
                if (options.contextMenu || options.noTip) { // context menu
                    div1.find('>style').html(
                        '#sayui-overlay'+ name +':before { display: none; }'+
                        '#sayui-overlay'+ name +':after { display: none; }'
                    );
                }
                // check scroll bar (needed to avoid horizontal scrollbar)
                if (overflowY && options.align != 'both') div2.width(w + sayutils.scrollBarSize() + 2);
            }
            menu.css('overflow-y', 'auto');
        }
    };

    $.fn.saymenu = function (menu, options) {
        /*
        ITEM STRUCTURE
            item : {
                id       : null,
        ...
        */
        ...
        if (menu === 'refresh') {
            ...
            if ($('#sayui-overlay'+ name).length > 0) {
                options = $.extend($.fn.saymenuOptions, options);
                var scrTop = $('#sayui-overlay'+ name +' div.menu').scrollTop();
                $('#sayui-overlay'+ name +' div.menu').html(getMenuHTML());
                $('#sayui-overlay'+ name +' div.menu').scrollTop(scrTop);
                mresize();
            } else {
                $(this).saymenu(options);
            }
        } else if (menu === 'refresh-index') {
            var $menu  = $('#sayui-overlay'+ name +' div.menu');
            var cur    = $menu.find('tr[index='+ options.index +']');
            var scrTop = $menu.scrollTop();
            $menu.find('tr.sayui-selected').removeClass('sayui-selected'); // clear all
            cur.addClass('sayui-selected'); // select current
            // scroll into view
            if (cur.length > 0) {
            ...
            $.fn.saymenuClick = function (event, index) {
                var keepOpen = false;
                ...
                // -- hide
                var div = $('#sayui-overlay'+ name);
                div.removeData('keepOpen');
                if (typeof div[0].hide === 'function' && !keepOpen) {
                    div[0].hide();
                }
            };
            $.fn.saymenuDown = function (event, index) {
                var $el  = $(event.target).parents('tr');
                var tmp  = $el.find('.sayui-icon');
                if ((options.type == 'check') || (options.type == 'radio')) {
                   var item = options.items[index];
                   item.checked = !item.checked;
                   if (item.checked) {
                       if (options.type == 'radio') {
                           tmp.parents('table').find('.sayui-icon')
                               .removeClass('sayui-icon-check')
                               .addClass('sayui-icon-empty');
                       }
                       tmp.removeClass('sayui-icon-empty').addClass('sayui-icon-check');
                   } else if (options.type == 'check') {
                       tmp.removeClass('sayui-icon-check').addClass('sayui-icon-empty');
                   }
                }
                // highlight record
                $el.parent().find('tr').removeClass('sayui-selected');
                $el.addClass('sayui-selected');
            };
            var html = '';
            if (options.search) {
                html +=
                    ...
                    '    <div class="sayui-icon icon-search" style="position: absolute; margin-top: 4px; margin-left: 6px; width: 11px; background-position: left !important;"></div>'+
            ...
            html += '<div class="menu" style="position: absolute; top: '+ (options.search ? 40 : 0) + 'px; bottom: 0px; width: 100%;">' +
                        getMenuHTML() +
                    '</div>';
            var ret = $(this).sayoverlay(html, options);
            setTimeout(function () {
                $('#sayui-overlay'+ name +' #menu-search')
                    .on('keyup', change)
                ...
                if (options.search) {
                    ...
                    $('#sayui-overlay'+ name +' #menu-search').focus();
                }
                mresize();
            }, 200);
            mresize();
        ...
        function mresize() {
            setTimeout(function () {
                // show selected
                $('#sayui-overlay'+ name +' tr.sayui-selected').removeClass('sayui-selected');
                var cur    = $('#sayui-overlay'+ name +' tr[index='+ options.index +']');
                var scrTop = $('#sayui-overlay'+ name +' div.menu').scrollTop();
                cur.addClass('sayui-selected');
                if (options.tmp) options.tmp.contentHeight = $('#sayui-overlay'+ name +' table').height() + (options.search ? 50 : 10);
                if (options.tmp) options.tmp.contentWidth  = $('#sayui-overlay'+ name +' table').width();
                if ($('#sayui-overlay'+ name).length > 0) $('#sayui-overlay'+ name)[0].resize();
                // scroll into view
                if (cur.length > 0) {
                    var top    = cur[0].offsetTop - 5; // 5 is margin top
                    var el     = $('#sayui-overlay'+ name +' div.menu');
                    var height = el.height();
                    $('#sayui-overlay'+ name +' div.menu').scrollTop(scrTop);
                    if (top < scrTop || top + cur.height() > scrTop + height) {
                        $('#sayui-overlay'+ name +' div.menu').animate({ 'scrollTop': top - (height - cur.height() * 2) / 2 }, 200, 'linear');
                    }
                }
            }, 1);
        }
        ...
        function change(event) {
            ...
            switch (key) {
                case 13: // enter
                    $('#sayui-overlay'+ name).remove();
                    $.fn.saymenuClick(event, options.index);
                    break;
                case 9:  // tab
                case 27: // escape
                    $('#sayui-overlay'+ name).remove();
                    $.fn.saymenuClick(event, -1);
                    break;
                case 38: // up
                    options.index = sayutils.isInt(options.index) ? parseInt(options.index) : 0;
                    options.index--;
            ...
            $(obj).saymenu('refresh', options);
            mresize();
        }

        function getMenuHTML() {
            if (options.spinner) {
                return  '<table class="sayui-drop-menu"><tbody><tr><td style="padding: 5px 10px 10px 10px; text-align: center">'+
                        '    <div class="sayui-spinner" style="width: 18px; height: 18px; position: relative; top: 5px;"></div> '+
                        '    <div style="display: inline-block; padding: 3px; color: #999;">'+ sayutils.lang('Loading...') +'</div>'+
                        '</td></tr></tbody></table>';
            }
            var count        = 0;
            var menu_html    = '<table cellspacing="0" cellpadding="0" class="sayui-drop-menu"><tbody>';
            ...
            for (var f = 0; f < options.items.length; f++) {
                ...
                if (['radio', 'check'].indexOf(options.type) != -1) {
                    if (mitem.checked === true) icon = 'sayui-icon-check'; else icon = 'sayui-icon-empty';
                }
                if (mitem.hidden !== true) {
                    var imgd = '';
                    var txt = mitem.text;
                    if (typeof options.render === 'function') txt = options.render(mitem, options);
                    if (img)  imgd = '<td class="menu-icon"><div class="sayui-tb-image sayui-icon '+ img +'"></div></td>';
                    if (icon) imgd = '<td class="menu-icon" align="center"><span class="sayui-icon '+ icon +'"></span></td>';
                    // render only if non-empty
                    if (txt != null && txt !== '' && !(/^-+$/.test(txt))) {
                        var bg = (count % 2 === 0 ? 'sayui-item-even' : 'sayui-item-odd');
                        ...
                        menu_html +=
                            '<tr index="'+ f + '" style="'+ (mitem.style ? mitem.style : '') +'" '+ (mitem.tooltip ? 'title="'+ sayutils.lang(mitem.tooltip) +'"' : '') +
                            '        class="'+ bg +' '+ (options.index === f ? 'sayui-selected' : '') + ' ' + (mitem.disabled === true ? 'sayui-disabled' : '') +'"'+
                            '        onmousedown="if ('+ (mitem.disabled === true ? 'true' : 'false') + ') return;'+
                            '               jQuery.fn.saymenuDown(event, \''+ f +'\');"'+
                            '        onclick="event.stopPropagation(); '+
                            '               if ('+ (mitem.disabled === true ? 'true' : 'false') + ') return;'+
                            '               jQuery.fn.saymenuClick(event, \''+ f +'\');">'+
                                imgd +
                            '   <td class="menu-text" colspan="'+ colspan +'">'+ sayutils.lang(txt) +'</td>'+
                            '   <td class="menu-count">'+
    ...
    $.fn.saycolor = function (options, callBack) {
        var obj   = this;
        var el    = $(this)[0];
        var index = [-1, -1];
        if ($.fn.saycolorPalette == null) {
            $.fn.saycolorPalette = [
        ...
        if ($('#sayui-overlay').length === 0) {
            $(el).sayoverlay(getColorHTML(options), {
                onHide: function () {
            ...
            $('#sayui-overlay .sayui-color').parent().html(getColorHTML(options));
        }
        // bind events
        $('#sayui-overlay .color')
            .off('.saycolor')
            .on('mousedown.saycolor', function (event) {
                var color = $(event.originalEvent.target).attr('name');
                index = $(event.originalEvent.target).attr('index').split(':');
                $(el).data('_color', color);
            })
            .on('mouseup.saycolor', function () {
                setTimeout(function () {
                    if ($("#sayui-overlay").length > 0) $('#sayui-overlay').removeData('keepOpen')[0].hide();
                }, 10);
            });
        $('#sayui-overlay input')
            .off('.saycolor')
            .on('mousedown.saycolor', function (event) {
                $('#sayui-overlay').data('keepOpen', true);
                setTimeout(function () { $('#sayui-overlay').data('keepOpen', true); }, 10);
                event.stopPropagation();
            })
            .on('keyup.saycolor', function (event) {
                if (this.value !== '' && this.value[0] !== '#') this.value = '#' + this.value;
            })
            .on('change.saycolor', function (event) {
                var tmp = this.value;
                if (tmp.substr(0, 1) == '#') tmp = tmp.substr(1);
                if (tmp.length != 6) {
                    $(this).saytag('Invalid color.');
                    return;
                }
                $.fn.saycolorPalette[pal.length - 1].unshift(tmp.toUpperCase());
                $(el).saycolor(options, callBack);
                setTimeout(function() { $('#sayui-overlay input')[0].focus(); }, 100);
            })
            .sayfield('hex');
        ...
        function getColorHTML(options) {
            ...
            var html  = '<div class="sayui-color" onmousedown="event.stopPropagation(); event.preventDefault()">'+ // prevent default is needed otherwiser selection gets unselected
...
(function ($) {
    var saygrid = function(options) {
        ...
        $.extend(true, this, sayobj.grid, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.saygrid = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!sayutils.checkName(method, 'saygrid')) return;
            // remember items
            ...
            // register new object
            sayui[object.name] = object;
            return object;

        } else {
            var obj = sayui[$(this).attr('name')];
            if (!obj) return null;
    ...
    saygrid.prototype = {
        ...
        buttons: {
            'reload'   : { type: 'button', id: 'sayui-reload', icon: 'sayui-icon-reload', tooltip: 'Reload data in the list' },
            'columns'  : { type: 'drop', id: 'sayui-column-on-off', icon: 'sayui-icon-columns', tooltip: 'Show/hide columns', arrow: false, html: '' },
            'search'   : { type: 'html',   id: 'sayui-search',
                            html: '<div class="sayui-icon icon-search-down sayui-search-down" '+
                                  'onclick="var obj = sayui[jQuery(this).parents(\'div.sayui-grid\').attr(\'name\')]; obj.searchShowFields();"></div>'
                          },
            'search-go': { type: 'drop',  id: 'sayui-search-advanced', icon: 'sayui-icon-search', text: 'Search', tooltip: 'Open Search Fields' },
            'add'      : { type: 'button', id: 'sayui-add', text: 'Add New', tooltip: 'Add new record', icon: 'sayui-icon-plus' },
            'edit'     : { type: 'button', id: 'sayui-edit', text: 'Edit', tooltip: 'Edit selected record', icon: 'sayui-icon-pencil', disabled: true },
            'delete'   : { type: 'button', id: 'sayui-delete', text: 'Delete', tooltip: 'Delete selected records', icon: 'sayui-icon-cross', disabled: true },
            'save'     : { type: 'button', id: 'sayui-save', text: 'Save', tooltip: 'Save changed records', icon: 'sayui-icon-check' }
        },
        ...
        add: function (record, first) {
            ...
            for (var i = 0; i < record.length; i++) {
                ...
                if (rec.sayui && rec.sayui.summary === true) {
        ...
        localSort: function (silent, noResetRefresh) {
            ...
            if (silent !== true && obj.show.statusSort) {
                setTimeout(function () {
                    obj.status(sayutils.lang('Sorting took') + ' ' + time/1000 + ' ' + sayutils.lang('sec'));
                }, 10);
            }
            return time;
            ...
            function preparePaths() {
                for (var i = 0; i < obj.records.length; i++) {
                    ...
                    if (rec.sayui && rec.sayui.parent_recid != null)
                        rec.sayui._path = getRecordPath(rec);
                }
            }
            ...
            function cleanupPaths() {
                for (var i = 0; i < obj.records.length; i++) {
                    var rec = obj.records[i];
                    if (rec.sayui && rec.sayui.parent_recid != null)
                        rec.sayui._path = null;
                }
            }

            // compare two paths, from root of tree to given records
            function compareRecordPaths(a, b) {
                if ((!a.sayui || a.sayui.parent_recid == null) && (!b.sayui || b.sayui.parent_recid == null)) {
            ...
            function getRecordPath(rec) {
                if (!rec.sayui || rec.sayui.parent_recid == null) return [rec];
                if (rec.sayui._path)
                    return rec.sayui._path;
                // during actual sort, we should never reach this point
                var subrec = obj.get(rec.sayui.parent_recid);
                if (!subrec) {
                    console.log('ERROR: no parent record: '+rec.sayui.parent_recid);
                    return [rec];
                }
        ...
        localSearch: function (silent) {
            ...
            if (this.searchData.length > 0 && !url) {
                ...
                for (var i = 0; i < this.records.length; i++) {
                    ...
                    if (match) {
                        if (rec && rec.sayui)
                            addParent(rec.sayui.parent_recid);
            ...
            if (silent !== true && obj.show.statusSearch) {
                setTimeout(function () {
                    obj.status(sayutils.lang('Search took') + ' ' + time/1000 + ' ' + sayutils.lang('sec'));
                }, 10);
            }
            return time;
            ...
            function searchRecord(rec) {
                ...
                for (var j = 0; j < obj.searchData.length; j++) {
                    ...
                    case 'is':
                        if (obj.parseField(rec, search.field) == sdata.value) fl++; // do not hide record
                            ...
                            var val1 = sayutils.formatDate(tmp, 'yyyy-mm-dd');
                            var val2 = sayutils.formatDate(sayutils.isDate(val2, sayutils.settings.dateFormat, true), 'yyyy-mm-dd');
                            if (val1 == val2) fl++;
                        }
                        else if (search.type == 'time') {
                            var tmp  = (obj.parseField(rec, search.field + '_') instanceof Date ? obj.parseField(rec, search.field + '_') : obj.parseField(rec, search.field));
                            var val1 = sayutils.formatTime(tmp, 'hh24:mi');
                            var val2 = sayutils.formatTime(val2, 'hh24:mi');
                            if (val1 == val2) fl++;
                        }
                        else if (search.type == 'datetime') {
                            var tmp  = (obj.parseField(rec, search.field + '_') instanceof Date ? obj.parseField(rec, search.field + '_') : obj.parseField(rec, search.field));
                            var val1 = sayutils.formatDateTime(tmp, 'yyyy-mm-dd|hh24:mm:ss');
                            var val2 = sayutils.formatDateTime(sayutils.isDateTime(val2, sayutils.settings.datetimeFormat, true), 'yyyy-mm-dd|hh24:mm:ss');
                            if (val1 == val2) fl++;
                        }
                        break;
                    case 'between':
                        if (['int', 'float', 'money', 'currency', 'percent'].indexOf(search.type) != -1) {
                            ...
                            var val2 = sayutils.isDate(val2, sayutils.settings.dateFormat, true);
                            var val3 = sayutils.isDate(val3, sayutils.settings.dateFormat, true);
                            if (val3 != null) val3 = new Date(val3.getTime() + 86400000); // 1 day
                            if (val1 >= val2 && val1 < val3) fl++;
                        }
                        else if (search.type == 'time') {
                            var val1 = (obj.parseField(rec, search.field + '_') instanceof Date ? obj.parseField(rec, search.field + '_') : obj.parseField(rec, search.field));
                            var val2 = sayutils.isTime(val2, true);
                            var val3 = sayutils.isTime(val3, true);
                            ...
                            var val1 = (obj.parseField(rec, search.field + '_') instanceof Date ? obj.parseField(rec, search.field + '_') : obj.parseField(rec, search.field));
                            var val2 = sayutils.isDateTime(val2, sayutils.settings.datetimeFormat, true);
                            var val3 = sayutils.isDateTime(val3, sayutils.settings.datetimeFormat, true);
                    ...
                    case 'in':
                        ...
                        if (tmp.indexOf(sayutils.isFloat(val1) ? parseFloat(val1) : val1) !== -1) fl++;
                        if (tmp.indexOf(sayutils.isFloat(val1b) ? parseFloat(val1b) : val1b) !== -1) fl++;
                        break;
                    case 'not in':
                        var tmp = sdata.value;
                        if (sdata.svalue) tmp = sdata.svalue;
                ...
                if ((obj.last.logic == 'OR' && fl !== 0) ||
                    ...
                    return true;
                if (rec.sayui && rec.sayui.children && rec.sayui.expanded !== true) {
                    // there are closed children, search them too.
                    for (var r = 0; r < rec.sayui.children.length; r++) {
                        var subRec = rec.sayui.children[r];
        ...
        refreshRanges: function () {
            ...
            for (var i = 0; i < this.ranges.length; i++) {
                ...
                var td1   = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(first.recid) + ' td[col="'+ first.column +'"]');
                var td2   = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(last.recid) + ' td[col="'+ last.column +'"]');
                var td1f  = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(first.recid) + ' td[col="'+ first.column +'"]');
                var td2f  = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(last.recid) + ' td[col="'+ last.column +'"]');
                ...
                if (first.column < this.last.colStart && last.column > this.last.colStart) {
                    td1 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(first.recid) + ' td[col="start"]');
                }
                if (first.column < this.last.colEnd && last.column > this.last.colEnd) {
                    _lastColumn = '"end"';
                    td2 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(last.recid) + ' td[col="end"]');
                }
                ...
                if (td1f.length > 0 || td2f.length > 0) {
                    if ($range.length === 0) {
                        rec1.append('<div id="grid_'+ this.name +'_f' + rg.name +'" class="sayui-selection" style="'+ rg.style +'">'+
                                        (rg.name == 'selection' ?  '<div id="grid_'+ this.name +'_resizer" class="sayui-selection-resizer"></div>' : '')+
                                    '</div>');
                        $range = $('#grid_'+ this.name +'_f'+ rg.name);
                    } else {
                        $range.attr('style', rg.style);
                        $range.find('.sayui-selection-resizer').show();
                    }
                    if (td2f.length === 0) {
                        td2f  = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(last.recid) +' td:last-child');
                        if (td2f.length === 0) td2f = $('#grid_'+ this.name +'_frec_bottom td:first-child');
                        $range.css('border-right', '0px');
                        $range.find('.sayui-selection-resizer').hide();
                    }
                ...
                if (td1.length > 0 || td2.length > 0) {
                    if ($range.length === 0) {
                        rec2.append('<div id="grid_'+ this.name +'_' + rg.name +'" class="sayui-selection" style="'+ rg.style +'">'+
                                        (rg.name == 'selection' ?  '<div id="grid_'+ this.name +'_resizer" class="sayui-selection-resizer"></div>' : '')+
                                    '</div>');
                    ...
                    if (td1.length === 0) {
                        td1 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(first.recid) +' td:first-child');
            ...
            // add resizer events
            $(this.box).find('.sayui-selection-resizer')
            ...
            if (this.selectType == 'row') {
                for (var a = 0; a < arguments.length; a++) {
                    ...
                    if (this.searchData.length !== 0 || (index + 1 >= this.last.range_start && index + 1 <= this.last.range_end)) {
                        recEl1 = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid));
                        recEl2 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid));
                    }
                    if (this.selectType == 'row') {
                        if (sel.indexes.indexOf(index) != -1) continue;
                        sel.indexes.push(index);
                        if (recEl1 && recEl2) {
                            recEl1.addClass('sayui-selected').data('selected', 'yes').find('.sayui-col-number').addClass('sayui-row-selected');
                            recEl2.addClass('sayui-selected').data('selected', 'yes').find('.sayui-col-number').addClass('sayui-row-selected');
                            recEl1.find('.sayui-grid-select-check').prop("checked", true);
                        }
                        selected++;
                    }
                }
                ...
                for (var recid in new_sel) {
                    ...
                    if (index + 1 >= this.last.range_start && index + 1 <= this.last.range_end) {
                        recEl1 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid));
                        recEl2 = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid));
                    }
                    ...
                    for (var t = 0; t < new_sel[recid].length; t++) {
                        ...
                        if (recEl1) {
                            recEl1.find('#grid_'+ this.name +'_data_'+ index +'_'+ col).addClass('sayui-selected');
                            recEl1.find('.sayui-col-number').addClass('sayui-row-selected');
                            recEl1.data('selected', 'yes');
                            recEl1.find('.sayui-grid-select-check').prop("checked", true);
                        }
                        if (recEl2) {
                            recEl2.find('#grid_'+ this.name +'_data_'+ index +'_'+ col).addClass('sayui-selected');
                            recEl2.find('.sayui-col-number').addClass('sayui-row-selected');
                            recEl2.data('selected', 'yes');
                            recEl2.find('.sayui-grid-select-check').prop("checked", true);
                        }
                        selected++;
                    }
                ...
                for (var c = 0; c < col_sel.length; c++) {
                    $(this.box).find('#grid_'+ this.name +'_column_'+ col_sel[c] +' .sayui-col-header').addClass('sayui-col-selected');
                }
            }
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                var recEl1 = $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid));
                var recEl2 = $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid));
                if (this.selectType == 'row') {
                    if (sel.indexes.indexOf(index) == -1) continue;
                    // event before
                    ...
                    recEl1.removeClass('sayui-selected sayui-inactive').removeData('selected').find('.sayui-col-number').removeClass('sayui-row-selected');
                    recEl2.removeClass('sayui-selected sayui-inactive').removeData('selected').find('.sayui-col-number').removeClass('sayui-row-selected');
                    if (recEl1.length != 0) {
                    ...
                    recEl1.find('.sayui-grid-select-check').prop("checked", false);
                    unselected++;
                } else {
                    var col  = arguments[a].column;
                    if (!sayutils.isInt(col)) { // unselect all columns
                        var cols = [];
                    ...
                    $('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid)).find(' > td[col='+ col +']').removeClass('sayui-selected sayui-inactive');
                    $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid)).find(' > td[col='+ col +']').removeClass('sayui-selected sayui-inactive');
                    ...
                    if (!isColSelected) {
                       $(this.box).find('.sayui-grid-columns td[col='+ col +'] .sayui-col-header, .sayui-grid-fcolumns td[col='+ col +'] .sayui-col-header').removeClass('sayui-col-selected');
                    }
                    if (!isRowSelected) {
                        $('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid)).find('.sayui-col-number').removeClass('sayui-row-selected');
                    }
                    unselected++;
                    if (s.length === 0) {
        ...
        selectAll: function () {
            ...
            // add selected class
            if (this.selectType == 'row') {
                $(this.box).find('.sayui-grid-records tr').not('.sayui-empty-record')
                    .addClass('sayui-selected').data('selected', 'yes').find('.sayui-col-number').addClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-frecords tr').not('.sayui-empty-record')
                    .addClass('sayui-selected').data('selected', 'yes').find('.sayui-col-number').addClass('sayui-row-selected');
                $(this.box).find('input.sayui-grid-select-check').prop('checked', true);
            } else {
                $(this.box).find('.sayui-grid-columns td .sayui-col-header, .sayui-grid-fcolumns td .sayui-col-header').addClass('sayui-col-selected');
                $(this.box).find('.sayui-grid-records tr .sayui-col-number').addClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-records tr').not('.sayui-empty-record')
                    .find('.sayui-grid-data').not('.sayui-col-select').addClass('sayui-selected').data('selected', 'yes');
                $(this.box).find('.sayui-grid-frecords tr .sayui-col-number').addClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-frecords tr').not('.sayui-empty-record')
                    .find('.sayui-grid-data').not('.sayui-col-select').addClass('sayui-selected').data('selected', 'yes');
                $(this.box).find('input.sayui-grid-select-check').prop('checked', true);
            }
            ...
            var sel = this.getSelection();
            if (sel.length == 1) this.toolbar.enable('sayui-edit'); else this.toolbar.disable('sayui-edit');
            if (sel.length >= 1) this.toolbar.enable('sayui-delete'); else this.toolbar.disable('sayui-delete');
            this.addRange('selection');
            $('#grid_'+ this.name +'_check_all').prop('checked', true);
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        selectNone: function () {
            ...
            if (this.selectType == 'row') {
                $(this.box).find('.sayui-grid-records tr.sayui-selected').removeClass('sayui-selected sayui-inactive').removeData('selected')
                    .find('.sayui-col-number').removeClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-frecords tr.sayui-selected').removeClass('sayui-selected sayui-inactive').removeData('selected')
                    .find('.sayui-col-number').removeClass('sayui-row-selected');
                $(this.box).find('input.sayui-grid-select-check').prop('checked', false);
            } else {
                $(this.box).find('.sayui-grid-columns td .sayui-col-header, .sayui-grid-fcolumns td .sayui-col-header').removeClass('sayui-col-selected');
                $(this.box).find('.sayui-grid-records tr .sayui-col-number').removeClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-frecords tr .sayui-col-number').removeClass('sayui-row-selected');
                $(this.box).find('.sayui-grid-data.sayui-selected').removeClass('sayui-selected sayui-inactive').removeData('selected');
                $(this.box).find('input.sayui-grid-select-check').prop('checked', false);
            }
            sel.indexes = [];
            sel.columns = {};
            this.toolbar.disable('sayui-edit', 'sayui-delete');
            this.removeRange('selection');
            $('#grid_'+ this.name +'_check_all').prop('checked', false);
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        search: function (field, value) {
            ...
            if (arguments.length === 0) {
                ...
                for (var i = 0; i < this.searches.length; i++) {
                    ...
                    if (['int', 'float', 'money', 'currency', 'percent'].indexOf(search.type) != -1) {
                        var fld1 = field1.data('sayfield');
                        var fld2 = field2.data('sayfield');
                    ...
                    if (['list', 'enum'].indexOf(search.type) != -1) {
                        ...
                        if ($.isArray(value1)) {
                            ...
                            for (var j = 0; j < value1.length; j++) {
                                svalue.push(sayutils.isFloat(value1[j].id) ? parseFloat(value1[j].id) : String(value1[j].id).toLowerCase());
                    ...
                    if ((value1 !== '' && value1 != null) || (value2 != null && value2 !== '')) {
                        ...
                        try {
                            if (search.type == 'date' && operator == 'between') {
                                tmp.value[0] = value1; // sayutils.isDate(value1, sayutils.settings.dateFormat, true).getTime();
                                tmp.value[1] = value2; // sayutils.isDate(value2, sayutils.settings.dateFormat, true).getTime();
                            }
                            if (search.type == 'date' && operator == 'is') {
                                tmp.value = value1; // sayutils.isDate(value1, sayutils.settings.dateFormat, true).getTime();
                            }
                        } catch (e) {
                ...
                if (value != null) {
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            for (var i = 0; i < this.searches.length; i++) {
                                var search = this.searches[i];
                                if (    search.type == 'text' || (search.type == 'alphanumeric' && sayutils.isAlphaNumeric(value))
                                        || (search.type == 'int' && sayutils.isInt(value)) || (search.type == 'float' && sayutils.isFloat(value))
                                        || (search.type == 'percent' && sayutils.isFloat(value)) || ((search.type == 'hex' || search.type == 'color') && sayutils.isHex(value))
                                        || (search.type == 'currency' && sayutils.isMoney(value)) || (search.type == 'money' && sayutils.isMoney(value))
                                        || (search.type == 'date' && sayutils.isDate(value)) || (search.type == 'time' && sayutils.isTime(value))
                                        || (search.type == 'datetime' && sayutils.isDateTime(value)) || (search.type == 'enum' && sayutils.isAlphaNumeric(value))
                                        || (search.type == 'list' && sayutils.isAlphaNumeric(value))
                                    ) {
                                    var tmp = {
                                        field    : search.field,
                                        type     : search.type,
        ...
        searchOpen: function () {
            ...
            if (edata.isCancelled === true) {
                setTimeout(function () { obj.toolbar.uncheck('sayui-search-advanced'); }, 1);
                return;
            }
            // show search
            $('#tb_'+ this.name +'_toolbar_item_sayui-search-advanced').sayoverlay({
                html    : this.getSearchesHTML(),
                name    : this.name + '-searchOverlay',
                left    : -10,
                'class' : 'sayui-grid-searches',
                onShow  : function () {
                    obj.initSearches();
                    $('#sayui-overlay-'+ obj.name +'-searchOverlay .sayui-grid-searches').data('grid-name', obj.name);
                    var sfields = $('#sayui-overlay-'+ this.name +'-searchOverlay .sayui-grid-searches *[rel=search]');
        ...
        searchClose: function () {
            ...
            if (!this.box) return;
            if (this.searches.length === 0) return;
            if (this.toolbar) this.toolbar.uncheck('sayui-search-advanced');
            // hide search
            $().sayoverlay({ name: this.name + '-searchOverlay' });
            $().sayoverlay({ name: this.name + '-searchOverlay' }); // need to call twice as first ignored after click
        },
        ...
        searchReset: function (noRefresh) {
            ...
            if (this.searches.length > 0) {
                if (!this.multiSearch || !this.show.searchAll) {
                    ...
                    this.last.field   = 'all';
                    this.last.caption = sayutils.lang('All Fields');
                }
            }
        ...
        searchShowFields: function () {
            var obj  = this;
            var el   = $('#grid_'+ this.name +'_search_all');
            var html = '<div class="sayui-select-field"><table><tbody>';
            for (var s = -1; s < this.searches.length; s++) {
                var search = this.searches[s];
                if (s == -1) {
                    if (!this.multiSearch || !this.show.searchAll) continue;
                    search = { field: 'all', caption: sayutils.lang('All Fields') };
                } else {
                    if (this.searches[s].hidden === true || this.searches[s].simple === false) continue;
                }
                html += '<tr '+ (sayutils.isIOS ? 'onTouchStart' : 'onClick') +'="sayui[\''+ this.name +'\'].initAllField(\''+ search.field +'\');'+
                        '      event.stopPropagation(); jQuery(\'#grid_'+ this.name +'_search_all\').sayoverlay({ name: \''+ this.name +'-searchFields\' });">'+
                        '   <td>'+
                        '       <span class="sayui-column-check sayui-icon-'+ (search.field == this.last.field ? 'check' : 'empty') +'"></span>'+
                        '   </td>'+
            ...
            setTimeout(function () {
                $(el).sayoverlay({ html: html, name: obj.name + '-searchFields', left: -10 });
            }, 1);
        },
        ...
        initAllField: function (field, value) {
            var el = $('#grid_'+ this.name +'_search_all');
            if (field == 'all') {
                var search = { field: 'all', caption: sayutils.lang('All Fields') };
                el.sayfield('clear');
                el.change();
            } else {
                var search = this.getSearch(field);
                ...
                el.sayfield(st, $.extend({}, search.options, { suffix: '', autoFormat: false, selected: value }));
            ...
            el.attr('placeholder', sayutils.lang(search.caption));
            $().sayoverlay({ name: this.name + '-searchFields' });
        },
        ...
        skip: function (offset, callBack) {
            ...
            if (url) {
                ...
                if (this.offset < 0 || !sayutils.isInt(this.offset)) this.offset = 0;
                this.clear(true);
        ...
        request: function (cmd, add_params, url, callBack) {
            ...
            var params = {};
            if (!sayutils.isInt(this.offset)) this.offset = 0;
            if (!sayutils.isInt(this.last.xhr_offset)) this.last.xhr_offset = 0;
            // add list params
            params['cmd']         = cmd;
            ...
            if (this.last.xhr_offset === 0) {
                obj.lock(sayutils.lang(obj.msgRefresh), true);
            } else {
                var more = $('#grid_'+ this.name +'_rec_more, #grid_'+ this.name +'_frec_more');
                if (this.autoLoad === true) {
                    more.show().find('td').html('<div><div style="width: 20px; height: 20px;" class="sayui-spinner"></div></div>');
                } else {
                    more.find('td').html('<div>'+ sayutils.lang('Load') + ' ' + obj.limit + ' ' + sayutils.lang('More') + '...</div>');
                }
            }
            ...
            if (!$.isEmptyObject(obj.routeData)) {
                var info  = sayutils.parseRoute(url);
           ...
           if (sayutils.settings.dataType == 'HTTP') {
            ...
            if (sayutils.settings.dataType == 'JSON') {
        ...
        requestComplete: function(status, cmd, callBack) {
            ...
            this.unlock();
            setTimeout(function () {
                if (obj.show.statusResponse) obj.status(sayutils.lang('Server Response') + ' ' + ((new Date()).getTime() - obj.last.xhr_start)/1000 +' ' + sayutils.lang('sec'));
            }, 10);
            ...
            if (status != 'error') {
                ...
                if (responseText != null && responseText != '') {
                    ...
                    if (data == null) {
                        data = {
                            status       : 'error',
                            message      : sayutils.lang(this.msgNotJSON),
                    ...
                    if (data['status'] == 'error') {
                        ...
                        if (cmd == 'get') {
                            ...
                            if (this.last.xhr_offset === 0) {
                                this.records = [];
                                this.summary = [];
                                if (sayutils.isInt(data.total)) this.total = parseInt(data.total);
                            } else {
                                if (data.total != -1 && parseInt(data.total) != parseInt(this.total)) {
                                    this.message(sayutils.lang(this.msgNeedReload), function () {
                ...
                data = {
                    status       : 'error',
                    message      : sayutils.lang(this.msgAJAXerror),
                    responseText : responseText
                };
                obj.error(sayutils.lang(this.msgAJAXerror));
            }
            // event after
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            if (!url) {
        ...
        getChanges: function () {
            ...
            for (var r = 0; r < this.records.length; r++) {
                ...
                if (rec.sayui && rec.sayui.changes != null) {
                    changes.push($.extend(true, { recid: rec.recid }, rec.sayui.changes));
                }
            }
            return changes;
        },
        ...
        mergeChanges: function () {
            ...
            for (var c = 0; c < changes.length; c++) {
                ...
                for (var s in changes[c]) {
                    ...
                    if (record.sayui) delete record.sayui.changes;
                }
            }
            this.refresh();
        },
        ...
        editField: function (recid, column, value, event) {
            ...
            var val = (rec.sayui && rec.sayui.changes && rec.sayui.changes[col.field] != null ? sayutils.stripTags(rec.sayui.changes[col.field]) : sayutils.stripTags(rec[col.field]));
            if (val == null) val = '';
            ...
            if (edit.items.length > 0 && !$.isPlainObject(edit.items[0])) {
                edit.items = sayobj.field.prototype.normMenu(edit.items);
            }
            switch (edit.type) {
                ...
                case 'select':
                    ...
                    el.addClass('sayui-editable')
                        .html('<select id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" column="'+ column +'" class="sayui-input"'+
                ...
                case 'div':
                    ...
                    el.addClass('sayui-editable')
                        .html('<div id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" class="sayui-input"'+
                            ...
                            '></div>' + edit.outTag);
                    if (value == null) el.find('div.sayui-input').text(typeof val != 'object' ? val : '');
                    // add blur listener
                    var input = el.find('div.sayui-input').get(0);
                    setTimeout(function () {
                        var tmp = input;
                ...
                default:
                    ...
                    el.addClass('sayui-editable')
                        .html('<input id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" autocorrect="off" autocomplete="off" spellcheck="false" type="text" '+
                            ...
                            '    field="'+ col.field +'" recid="'+ recid +'" column="'+ column +'" class="sayui-input"'+ edit.inTag +
                            '/>' + edit.outTag);
                    // issue #499
                    if (edit.type == 'number') {
                        val = sayutils.formatNumber(val);
                    }
                    if (edit.type == 'date') {
                        val = sayutils.formatDate(sayutils.isDate(val, edit.format, true), edit.format);
                    }
                    if (value == null) el.find('input').val(typeof val != 'object' ? val : '');
                    // init sayfield
                    var input = el.find('input').get(0);
                    $(input).sayfield(edit.type, $.extend(edit, { selected: val }));
                    // add blur listener
                    setTimeout(function () {
                        var tmp = input;
                        if (edit.type == 'list') {
                            tmp = $($(input).data('sayfield').helpers.focus).find('input');
            ...
            setTimeout(function () {
                el.find('input, select, div.sayui-input')
                    ...
                    .on('click', function (event) {
                        if (edit.type == 'div') {
                            expand.call(el.find('div.sayui-input')[0], null);
                        } else {
                    ...
                    .on('keydown', function (event) {
                        ...
                        switch (event.keyCode) {
                            case 8: // backspace;
                                if (edit.type == 'list' && !$(input).data('sayfield')) { // cancel backspace when deleting element
                                    event.preventDefault();
                                }
                                break;
                            case 9:
                            case 13:
                                ...
                                break;
                            case 37:
                                if (sayutils.getCursorPosition(el) === 0) {
                                    event.preventDefault();
                                }
                                break;
                            case 39:
                                if (sayutils.getCursorPosition(el) == val.length) {
                                    sayutils.setCursorPosition(el, val.length);
                                    event.preventDefault();
                                }
                                break;
                        }
                        ...
                        setTimeout(function () {
                            switch (event.keyCode) {
                                ...
                                case 27: // escape
                                    ...
                                    if (rec.sayui && rec.sayui.changes && rec.sayui.changes[col.field] != null) old = rec.sayui.changes[col.field];
                ...
                // focus and select
                setTimeout(function () {
                    var tmp = el.find('.sayui-input');
                    var len = $(tmp).val().length;
                    if (edit.type == 'div') len = $(tmp).text().length;
                    if (tmp.length > 0) {
                        tmp.focus();
                        clearTimeout(obj.last.kbd_timer); // keep focus
                        if (tmp[0].tagName != 'SELECT') sayutils.setCursorPosition(tmp[0], len);
                        tmp[0].resize = expand;
                        expand.call(tmp[0], null);
                    }
                }, 50);
                // event after
                obj.trigger($.extend(edata, { phase: 'after', input: el.find('input, select, div.sayui-input') }));
            }, 5); // needs to be 5-10
            return;
            ...
            function expand(event) {
                try {
                    ...
                    var width = sayutils.getStrWidth(val, style);
        ...
        editChange: function (el, index, column, event) {
            ...
            if (el.type == 'checkbox') {
                if (rec.sayui && rec.sayui.editable === false) el.checked = !el.checked;
                new_val = el.checked;
            }
            ...
            var edata = {
                ...
                value_new: new_val,
                value_previous: (rec.sayui && rec.sayui.changes && rec.sayui.changes.hasOwnProperty(col.field) ? rec.sayui.changes[col.field]: old_val),
            ...
            while (true) {
                ...
                if ((typeof new_val != 'object' && String(old_val) != String(new_val)) ||
                    ...
                    if (edata.isCancelled !== true) {
                        ...
                        // default action
                        rec.sayui = rec.sayui || {};
                        rec.sayui.changes = rec.sayui.changes || {};
                        rec.sayui.changes[col.field] = edata.value_new;
                        // event after
                        this.trigger($.extend(edata, { phase: 'after' }));
                    }
                } else {
                    ...
                    if (edata.isCancelled !== true) {
                        ...
                        // default action
                        if (rec.sayui && rec.sayui.changes) delete rec.sayui.changes[col.field];
                        if (rec.sayui && $.isEmptyObject(rec.sayui.changes)) delete rec.sayui.changes;
                        // event after
                        this.trigger($.extend(edata, { phase: 'after' }));
                    }
                }
            ...
            if (!summary) {
                if (rec.sayui && rec.sayui.changes && rec.sayui.changes[col.field] != null) {
                    cell.addClass('sayui-changed');
                } else {
                    cell.removeClass('sayui-changed');
                }
                // update cell data
            ...
            if (this.show.toolbarSave) {
                if (this.getChanges().length > 0) this.toolbar.enable('sayui-save'); else this.toolbar.disable('sayui-save');
            }
        },
        ...
        "delete": function (force) {
            ...
            if (this.msgDelete != '' && !force) {
                this.message({
                    width   : 350,
                    height  : 170,
                    body    : '<div class="sayui-centered">' + sayutils.lang(obj.msgDelete) + '</div>',
                    buttons : '<button class="sayui-btn sayui-btn-red" onclick="sayui[\''+ this.name +'\'].delete(true)">' + sayutils.lang('Yes') + '</button>'+
                              '<button class="sayui-btn" onclick="sayui[\''+ this.name +'\'].message()">' + sayutils.lang('No') + '</button>',
                    onOpen: function (event) {
                        ...
                        setTimeout(function () {
                            $(this.box).find('.sayui-btn:last-child').focus();
                ...
                if (typeof recs[0] != 'object') {
                    ...
                    for (var r = 0; r < recs.length; r++) {
                        ...
                        if (ind != null && fld != 'recid') {
                            ...
                            if (rec.sayui && rec.sayui.changes) delete rec.sayui.changes[fld];
        ...
        click: function (recid, event) {
            ...
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                ...
                if ($(event.target).parents('td').hasClass('sayui-col-select')) fselect = true;
        ...
        focus: function (event) {
            ...
            // default behaviour
            this.hasFocus = true;
            $(this.box).find('.sayui-inactive').removeClass('sayui-inactive');
            setTimeout(function () {
                var $input = $(obj.box).find('#grid_'+ obj.name + '_focus');
                if (!$input.is(':focus')) $input.focus();
            }, 10);
        ...
        blur: function (event) {
            ...
            // default behaviour
            this.hasFocus = false;
            $(this.box).find('.sayui-selected').addClass('sayui-inactive');
            $(this.box).find('.sayui-selection').addClass('sayui-inactive');
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        },

        keydown: function (event) {
            // this method is called from sayutils
            var obj = this;
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            ...
            if (edata.isCancelled === true) return;
            // default behavior
            if ($(this.box).find('>.sayui-message').length > 0) {
            ...
            var recEL    = $('#grid_'+ obj.name +'_rec_'+ (ind != null ? sayutils.escapeId(obj.records[ind].recid) : 'none'));
            var cancel   = false;
            var key      = event.keyCode;
            ...
            switch (key) {
                ...
                case 37: // left
                    ...
                    if (this.selectType == 'row') {
                        if (recEL.length <= 0) break;
                        var tmp = this.records[ind].sayui || {};
        ...
        dblClick: function (recid, event) {
            ...
            if (edit) {
                ...
                this.select({ recid: recid, column: column });
                if (this.show.expandColumn || (rec.sayui && Array.isArray(rec.sayui.children))) this.toggle(recid);
            }
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        },
        ...
        contextMenu: function (recid, column, event) {
            ...
            if (sayutils.isFloat(recid)) recid = parseFloat(recid);
            var sel = this.getSelection();
            if (this.selectType == 'row') {
        ...
        toggle: function (recid) {
            ...
            rec.sayui = rec.sayui || {};
            if (rec.sayui.expanded === true) return this.collapse(recid); else return this.expand(recid);
        },
        ...
        expand: function (recid) {
            ...
            var rec  = this.records[ind];
            rec.sayui = rec.sayui || {};
            var id   = sayutils.escapeId(recid);
            var children = rec.sayui.children;
            if (Array.isArray(children)) {
                if (rec.sayui.expanded === true || children.length === 0) return false; // already shown
                var edata = this.trigger({ phase: 'before', type: 'expand', target: this.name, recid: recid });
                if (edata.isCancelled === true) return false;
                rec.sayui.expanded = true;
                children.forEach(function (child) {
                    child.sayui = child.sayui || {};
                    child.sayui.parent_recid = rec.recid;
                    if (child.sayui.children == null) child.sayui.children = [];
                });
                ...
                if (rec.sayui.expanded == 'none') return false;
                // insert expand row
                $('#grid_'+ this.name +'_rec_'+ id).after(
                        '<tr id="grid_'+ this.name +'_rec_'+ recid +'_expanded_row" class="sayui-expanded-row">'+
                        '    <td colspan="100" class="sayui-expanded2">'+
                        '        <div id="grid_'+ this.name +'_rec_'+ recid +'_expanded"></div>'+
                        '    </td>'+
                        '    <td class="sayui-grid-data-last"></td>'+
                        '</tr>');

                $('#grid_'+ this.name +'_frec_'+ id).after(
                        '<tr id="grid_'+ this.name +'_frec_'+ recid +'_expanded_row" class="sayui-expanded-row">'+
                            (this.show.lineNumbers ? '<td class="sayui-col-number"></td>' : '') +
                        '    <td class="sayui-grid-data sayui-expanded1" colspan="100">'+
                ...
                var rosay = $(this.box).find('#grid_'+ this.name +'_frec_'+ recid +'_expanded');
                var innerHeight = row1.find('> div:first-child').height();
                if (row1.height() < innerHeight) {
                    row1.css({ height: innerHeight + 'px' });
                }
                if (rosay.height() < innerHeight) {
                    rosay.css({ height: innerHeight + 'px' });
                }
                // default action
                $('#grid_'+ this.name +'_rec_'+ id).attr('expanded', 'yes').addClass('sayui-expanded');
                $('#grid_'+ this.name +'_frec_'+ id).attr('expanded', 'yes').addClass('sayui-expanded');
        ...
        collapse: function (recid) {
            ...
            var children = rec.sayui.children;
            if (Array.isArray(children)) {
                if (rec.sayui.expanded !== true) return false; // already hidden
                var edata = this.trigger({ phase: 'before', type: 'collapse', target: this.name, recid: recid });
                if (edata.isCancelled === true) return false;
                clearExpanded(rec);
                var stops = [];
                for (var r = rec; r != null; r = this.get(r.sayui.parent_recid))
                    stops.push(r.sayui.parent_recid);
                // stops contains 'undefined' plus the ID of all nodes in the path from 'rec' to the tree root
                var start = ind + 1;
                var end   = start;
                while (true) {
                    if (this.records.length <= end + 1 || this.records[end+1].sayui == null ||
                        stops.indexOf(this.records[end+1].sayui.parent_recid) >= 0) {
                        break;
                    }
                    end++;
                }
                ...
                if (edata.isCancelled === true) return false;
                // default action
                $('#grid_'+ this.name +'_rec_'+ id).removeAttr('expanded').removeClass('sayui-expanded');
                $('#grid_'+ this.name +'_frec_'+ id).removeAttr('expanded').removeClass('sayui-expanded');
                $('#grid_'+ this.name +'_cell_'+ this.get(recid, true) +'_expand div').html('+');
            ...
            function clearExpanded(rec) {
                rec.sayui.expanded = false;
                for (var i = 0; i < rec.sayui.children.length; i++) {
                    var subRec = rec.sayui.children[i];
                    if (subRec.sayui.expanded) {
            ...
            if (typeof sel[0] == 'object') { // cell copy
                ...
                for (var r = 0 ; r < recs.length; r++) {
                    ...
                    for (var c = minCol; c <= maxCol; c++) {
                        ...
                        text += sayutils.stripTags(this.getCellHTML(ind, c)) + '\t';
                    }
                ...
                for (var c = 0; c < this.columns.length; c++) {
                    ...
                    text += '"' + sayutils.stripTags(colName) + '"\t';
                }
                text = text.substr(0, text.length-1); // remove last \t
                text += '\n';
                // copy selected text
        ...
        paste: function (text) {
            ...
            for (var t = 0; t < text.length; t++) {
                ...
                for (var dt = 0; dt < tmp.length; dt++) {
                    ...
                    rec.sayui = rec.sayui || {};
                    rec.sayui.changes = rec.sayui.changes || {};
                    rec.sayui.changes[field] = tmp[dt];
                    cols.push(col + cnt);
                    cnt++;
                }
        ...
        resize: function () {
            ...
            $(this.box).find('> div.sayui-grid-box')
        ...
        update: function (cells) {
            ...
            if (cells == null) {
                for (var index = this.last.range_start - 1; index <= this.last.range_end - 1; index++) {
                    if (index < 0) continue;
                    var rec = this.records[index] || {};
                    if (!rec.sayui) rec.sayui = {};
                    for (var column = 0; column < this.columns.length; column++) {
                        var row  = $(this.box).find('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(rec.recid));
                        ...
                        // assign style
                        if (rec.sayui.style != null && !$.isEmptyObject(rec.sayui.style)) {
                            if (typeof rec.sayui.style == 'string') {
                                row.attr('style', rec.sayui.style);
                            }
                            if ($.isPlainObject(rec.sayui.style) && typeof rec.sayui.style[column] == 'string') {
                                cell.attr('style', rec.sayui.style[column]);
                            }
                        } else {
                            cell.attr('style', '');
                        }
                        // assign class
                        if (rec.sayui.class != null && !$.isEmptyObject(rec.sayui.class)) {
                            if (typeof rec.sayui.class == 'string') {
                                row.addClass(rec.sayui.class);
                            }
                            if ($.isPlainObject(rec.sayui.class) && typeof rec.sayui.class[column] == 'string') {
                                cell.addClass(rec.sayui.class[column]);
                            }
                        }
                    }
                }
        ...
        refreshCell: function (recid, field) {
            ...
            // assign style
            if (rec.sayui && rec.sayui.style != null && !$.isEmptyObject(rec.sayui.style)) {
                if (typeof rec.sayui.style == 'string') {
                    $(this.box).find('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(rec.recid)).attr('style', rec.sayui.style);
                }
                if ($.isPlainObject(rec.sayui.style) && typeof rec.sayui.style[col_ind] == 'string') {
                    cell.attr('style', rec.sayui.style[col_ind]);
                }
            } else {
                cell.attr('style', '');
            }
            // assign class
            if (rec.sayui && rec.sayui.class != null && !$.isEmptyObject(rec.sayui.class)) {
                if (typeof rec.sayui.class == 'string') {
                    $(this.box).find('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(rec.recid)).addClass(rec.sayui.class);
                }
                if ($.isPlainObject(rec.sayui.class) && typeof rec.sayui.class[col_ind] == 'string') {
                    cell.addClass(rec.sayui.class[col_ind]);
                }
            }
        },

        refreshRow: function (recid, ind) {
            var tr1 = $(this.box).find('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid));
            var tr2 = $(this.box).find('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid));
            if (tr1.length > 0) {
                ...
                var st = (this.records[ind].sayui ? this.records[ind].sayui.style : '');
                if (typeof st == 'string') {
                    var tr1 = $(this.box).find('#grid_'+ this.name +'_frec_'+ sayutils.escapeId(recid));
                    var tr2 = $(this.box).find('#grid_'+ this.name +'_rec_'+ sayutils.escapeId(recid));
                    tr1.attr('custom_style', st);
                    tr2.attr('custom_style', st);
                    if (tr1.hasClass('sayui-selected')) {
        ...
        refresh: function () {
            ...
            if (this.show.toolbar) {
                ...
                if (this.toolbar && this.toolbar.get('sayui-column-on-off') && this.toolbar.get('sayui-column-on-off').checked) {
                    // no action
                } else {
                    ...
                    if (typeof this.toolbar == 'object') {
                        ...
                        for (var t = 0; t < tmp.length; t++) {
                            if (tmp[t].id == 'sayui-search' || tmp[t].type == 'break') continue;
            ...
            if (this.last.multi) {
                el.attr('placeholder', '[' + sayutils.lang('Multiple Fields') + ']');
                el.sayfield('clear');
            } else {
                el.attr('placeholder', sayutils.lang(this.last.caption));
            }
            ...
            var rows = obj.find({ 'sayui.expanded': true }, true);
            for (var r = 0; r < rows.length; r++) {
                var tmp = obj.records[rows[r]].sayui;
            ...
            if (obj.markSearch) {
                setTimeout(function () {
                    ...
                    if (str.length > 0) $(obj.box).find('.sayui-grid-data > div').saymarker(str);
                }, 50);
            }
            // enable/disable toolbar search button
            if (this.show.toolbarSave) {
        ...
        refreshBody: function () {
            // -- separate summary
            var tmp = this.find({ 'sayui.summary': true }, true);
            if (tmp.length > 0) {
            ...
            var bodyHTML =
                '<div id="grid_'+ this.name +'_frecords" class="sayui-grid-frecords" style="margin-bottom: '+ (sayutils.scrollBarSize() - 1) +'px;">'+
                    recHTML[0] +
                '</div>'+
                '<div id="grid_'+ this.name +'_records" class="sayui-grid-records" onscroll="sayui[\''+ this.name +'\'].scroll(event);">' +
                    recHTML[1] +
                '</div>'+
                '<div id="grid_'+ this.name +'_scroll1" class="sayui-grid-scroll1" style="height: '+ sayutils.scrollBarSize() +'px"></div>'+
                // Columns need to be after to be able to overlap
                '<div id="grid_'+ this.name +'_fcolumns" class="sayui-grid-fcolumns">'+
                '    <table><tbody>'+ colHTML[0] +'</tbody></table>'+
                '</div>'+
                '<div id="grid_'+ this.name +'_columns" class="sayui-grid-columns">'+
            ...
            if (this.records.length === 0 && this.msgEmpty) {
                $('#grid_'+ this.name +'_body')
                    .append('<div id="grid_'+ this.name + '_empty_msg" class="sayui-grid-empty-msg"><div>'+ this.msgEmpty +'</div></div>');
        ...
        render: function (box) {
            ...
            if (box != null) {
                if ($(this.box).find('#grid_'+ this.name +'_body').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('sayui-reset sayui-grid')
                        .html('');
                }
                this.box = box;
            }
            ...
            $(this.box)
                .attr('name', this.name)
                .addClass('sayui-reset sayui-grid')
                .html('<div class="sayui-grid-box">'+
                      '    <div id="grid_'+ this.name +'_header" class="sayui-grid-header"></div>'+
                      '    <div id="grid_'+ this.name +'_toolbar" class="sayui-grid-toolbar"></div>'+
                      '    <div id="grid_'+ this.name +'_body" class="sayui-grid-body"></div>'+
                      '    <div id="grid_'+ this.name +'_fsummary" class="sayui-grid-body sayui-grid-summary"></div>'+
                      '    <div id="grid_'+ this.name +'_summary" class="sayui-grid-body sayui-grid-summary"></div>'+
                      '    <div id="grid_'+ this.name +'_footer" class="sayui-grid-footer"></div>'+
                      '    <textarea id="grid_'+ this.name +'_focus" class="sayui-grid-focus-input"></textarea>'+
                      '</div>');
            if (this.selectType != 'row') $(this.box).addClass('sayui-ss');
            ...
            $(this.box).find('#grid_'+ this.name + '_focus')
                ...
                .on('paste', function (event) {
                    var el = this;
                    setTimeout(function () { sayui[obj.name].paste(el.value); el.value = ''; }, 1)
                })
                .on('keydown', function (event) {
                    sayui[obj.name].keydown.call(sayui[obj.name], event);
                });
            ...
            if ($('.sayui-layout').length === 0) { // if there is layout, it will send a resize event
                $(window).off('resize.sayui-'+ this.name)
                    .on('resize.sayui-'+ this.name, function (event) {
                        sayui[obj.name].resize();
                    });
            }
            return (new Date()).getTime() - time;
            ...
            function mouseStart (event) {
                ...
                if (obj.last.userSelect == 'text') {
                    ...
                    $(obj.box).find('.sayui-grid-body').css(sayutils.cssPrefix('user-select', 'none'));
                }
                // regular record select
                if (obj.selectType == 'row' && ($(event.target).parents().hasClass('sayui-head') || $(event.target).hasClass('sayui-head'))) return;
                if (obj.last.move && obj.last.move.type == 'expand') return;
                // if altKey - alow text selection
                if (event.altKey) {
                    $(obj.box).find('.sayui-grid-body').css(sayutils.cssPrefix('user-select', 'text'));
                    obj.selectNone();
                    ...
                    var tmps = false;
                    while (tmp) {
                        if (tmp.classList && tmp.classList.contains('sayui-grid')) break;
                    ...
                    if (obj.last.move) {
                        ...
                        if ($owner.hasClass('sayui-grid-records') || $owner.hasClass('sayui-grid-frecords')
                                || $owner.hasClass('sayui-grid-columns') || $owner.hasClass('sayui-grid-fcolumns')
                                || $owner.hasClass('sayui-grid-summary')) {
                        ...
                        if ($(target).hasClass('sayui-grid-footer') || $(target).parents('div.sayui-grid-footer').length > 0) {
                ...
                if (obj.reorderRows == true) {
                    ...
                    if ($(el).hasClass('sayui-col-number')) {
                        obj.selectNone();
                        obj.last.move.reorder = true;
                        // supress hover
                        var eColor = $(obj.box).find('.sayui-even.sayui-empty-record').css('background-color');
                        var oColor = $(obj.box).find('.sayui-odd.sayui-empty-record').css('background-color');
                        $(obj.box).find('.sayui-even td').not('.sayui-col-number').css('background-color', eColor);
                        $(obj.box).find('.sayui-odd td').not('.sayui-col-number').css('background-color', oColor);
                        ...
                        if (!mv.ghost) {
                            ...
                            var recs = $(obj.box).find('.sayui-grid-records');
            ...
            function mouseMove (event) {
                ...
                if (obj.reorderRows == true && obj.last.move.reorder) {
                    ...
                    if (recid != mv.from) {
                        ...
                        var rosay = $('#grid_'+ obj.name + '_rec_'+ recid);
                        $(obj.box).find('.tmp-ghost').css('border-top', '0px');
                        rosay.addClass('tmp-ghost').css('border-top', '2px solid #769EFC');
                        // MOVABLE GHOST
                        // if (event.screenY - mv.lastY < 0) row1.after(rosay); else rosay.after(row1);
                ...
                if (recid == null) {
                    ...
                    if (isNaN(col)) {
                        ...
                        $(obj.box).find('.sayui-grid-columns .sayui-col-header, .sayui-grid-fcolumns .sayui-col-header').removeClass('sayui-col-selected');
                        $(obj.box).find('.sayui-col-number').removeClass('sayui-row-selected');
                        delete mv.colRange;
                    } else {
                        ...
                        if (mv.colRange != newRange) {
                            ...
                            if (edataCol.isCancelled !== true) {
                                ...
                                for (var j = parseInt(tmp[0]); j <= parseInt(tmp[1]); j++) {
                                    $(obj.box).find('#grid_'+ obj.name +'_column_' + j + ' .sayui-col-header').addClass('sayui-col-selected');
                                }
                                $(obj.box).find('.sayui-col-number').not('.sayui-head').addClass('sayui-row-selected');
                                // show new range
            ...
            function mouseStop (event) {
                ...
                if ($(event.target).parents().hasClass('.sayui-head') || $(event.target).hasClass('.sayui-head')) return;
        ...
        initColumnOnOff: function () {
            ...
            var obj = this;
            // line number
            var col_html = '<div class="sayui-col-on-off">'+
                '<table><tbody>'+
                '<tr id="grid_'+ this.name +'_column_ln_check" onclick="sayui[\''+ obj.name +'\'].columnOnOff(event, \'line-numbers\'); event.stopPropagation();">'+
                '   <td style="width: 30px; text-align: center; padding-right: 3px; color: #888;">'+
                '      <span class="sayui-column-check sayui-icon-'+ (!obj.show.lineNumbers ? 'empty' : 'check') +'"></span>'+
                '   </td>'+
                '   <td onclick="jQuery(\'.sayui-overlay\')[0].hide();">'+
                '      <label>'+ sayutils.lang('Line #') +'</label>'+
                '   </td>'+
                '</tr>';
            // columns
            for (var c = 0; c < this.columns.length; c++) {
                ...
                col_html +=
                    ...
                    '       onclick="sayui[\''+ obj.name +'\'].columnOnOff(event, \''+ col.field +'\'); event.stopPropagation();">'+
                    '   <td style="width: 30px; text-align: center; padding-right: 3px; color: #888;">'+
                    '      <span class="sayui-column-check sayui-icon-'+ (col.hidden ? 'empty' : 'check') +'"></span>'+
                    '   </td>'+
                    '   <td onclick="jQuery(\'.sayui-overlay\')[0].hide();">'+
                    '       <label>'+ sayutils.stripTags(tmp) +'</label>'+
                    '   </td>'+
                    '</tr>';
            }
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            ...
            if (url && obj.show.skipRecords) {
                col_html +=
                        ...
                        '    <div style="cursor: pointer; padding: 2px 8px; cursor: default">'+ sayutils.lang('Skip') +
                        ...
                        '               sayui[\''+ obj.name +'\'].skip(this.value); '+
                        '               jQuery(\'.sayui-overlay\')[0].hide(); '+
                        '            }"/> '+ sayutils.lang('Records')+
                        '    </div>'+
                        '</td></tr>';
            }
            // save/restore state
            if (obj.show.saveRestoreState) {
                col_html += '<tr><td colspan="2" onclick="var obj = sayui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'sayui-column-on-off\'); obj.stateSave();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ sayutils.lang('Save Grid State') + '</div>'+
                            '</td></tr>'+
                            '<tr><td colspan="2" onclick="var obj = sayui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'sayui-column-on-off\'); obj.stateReset();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ sayutils.lang('Restore Default State') + '</div>'+
                            '</td></tr>';
            }
            col_html += "</tbody></table></div>";
            this.toolbar.get('sayui-column-on-off').html = col_html;
        },
        ...
        initColumnDrag: function ( box ) {
            ...
            function dragColStart ( event ) {
                ...
                _dragData.timeout = setTimeout(function(){
                    ...
                    var edata,
                        ...
                        origColumn,
                        origColumnNumber,
                        invalidPreColumns = [ 'sayui-col-number', 'sayui-col-expand', 'sayui-col-select' ],
                        invalidPostColumns = [ 'sayui-head-last' ],
                        invalidColumns = invalidPreColumns.concat( invalidPostColumns ),
                        preColumnsSelector = '.sayui-col-number, .sayui-col-expand, .sayui-col-select',
                        preColHeadersSelector = '.sayui-head.sayui-col-number, .sayui-head.sayui-col-expand, .sayui-head.sayui-col-select';

                    // do nothing if it is not a header
                    if ( !$( event.originalEvent.target ).parents().hasClass( 'sayui-head' ) ) return;
                    ...
                    _dragData.columnHead = origColumn = $( event.originalEvent.target ).parents( '.sayui-head' );
                    ...
                    columns = _dragData.columns = $( obj.box ).find( '.sayui-head:not(.sayui-head-last)' );

                    //add events
                    $( document ).on( 'mouseup', dragColEnd );
                    $( document ).on( 'mousemove', dragColOver );

                    _dragData.originalPos = parseInt( $( event.originalEvent.target ).parent( '.sayui-head' ).attr( 'col' ), 10 );
                    ...
                    $( _dragData.ghost ).find( '[col]:not([col="' + _dragData.originalPos + '"]), .sayui-toolbar, .sayui-grid-header' ).remove();
                    $( _dragData.ghost ).find( preColumnsSelector ).remove();
                    $( _dragData.ghost ).find( '.sayui-grid-body' ).css({ top: 0 });
                    ...
                    $( _dragData.ghost ).css({
                        ...
                        zIndex: 999999,
                        opacity: 0
                    }).addClass( '.sayui-grid-ghost' ).animate({
                            width: selectedCol.width(),
                            height: $(obj.box).find('.sayui-grid-body:first').height(),
                            left : event.pageX,
                            top : event.pageY,
                            opacity: 0.8
                        }, 0 );
            ...
            function dragColOver ( event ) {
                ...
                var cursorX = event.originalEvent.pageX,
                    ...
                    lastWidth = $( '.sayui-head:not(.sayui-head-last)' ).width();
            ...
            function markIntersection( intersection ){
                ...
                if ( !_dragData.lastInt || _dragData.lastInt !== intersection ){
                    ...
                    $('.sayui-head').removeClass('sayui-col-intersection');
                    ...
                    if ( intersection >= _dragData.columns.length ) {
                        ...
                        $( _dragData.columns[ _dragData.columns.length - 1 ] ).addClass('sayui-col-intersection');
                        ...
                        $( _dragData.columns[ _dragData.numberPreColumnsPresent ] ).prev().addClass('sayui-col-intersection');
                    } else {
                        ...
                        $( _dragData.columns[intersection - 1] ).addClass('sayui-col-intersection');
                    }
                }
            }
            ...
            return {
                remove: function(){
                    ...
                    $( obj.box ).find( '.sayui-head' ).removeAttr( 'draggable' );
        ...
        columnOnOff: function (event, field) {
            var $el = $(event.target).parents('tr').find('.sayui-column-check');
            // event before
            ...
            var rows = obj.find({ 'sayui.expanded': true }, true);
            for (var r = 0; r < rows.length; r++) {
                var tmp = this.records[r].sayui;
                if (tmp && !Array.isArray(tmp.children)) {
                    this.records[r].sayui.expanded = false;
                }
            }
            // show/hide
            if (field == 'line-numbers') {
                ...
                if (this.show.lineNumbers) {
                    $el.addClass('sayui-icon-check').removeClass('sayui-icon-empty');
                } else {
                    $el.addClass('sayui-icon-empty').removeClass('sayui-icon-check');
                }
                this.refreshBody();
                this.resizeRecords();
            } else {
            ...
            if (!event.shiftKey && !event.metaKey && !event.ctrlKey) {
                ...
                setTimeout(function () {
                    $().sayoverlay({ name: obj.name + '_toolbar' });
                }, 150);
            }
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        initToolbar: function () {
            ...
            if (this.toolbar['render'] == null) {
                ...
                this.toolbar.items = [];
                this.toolbar = $().saytoolbar($.extend(true, {}, this.toolbar, { name: this.name +'_toolbar', owner: this }));
                ...
                if (this.show.toolbarReload || this.show.toolbarColumns) {
                    this.toolbar.items.push({ type: 'break', id: 'sayui-break0' });
                }
                if (this.show.toolbarInput) {
                    var html =
                        '<div class="sayui-toolbar-search">'+
                        '<table cellpadding="0" cellspacing="0"><tbody><tr>'+
                        '    <td>'+ this.buttons['search'].html +'</td>'+
                        '    <td>'+
                        '        <input type="text" id="grid_'+ this.name +'_search_all" class="sayui-search-all" tabindex="-1" '+
                        '            placeholder="'+ sayutils.lang(this.last.caption) +'" value="'+ this.last.search +'"'+
                        '            onfocus="clearTimeout(sayui[\''+ this.name +'\'].last.kbd_timer);"'+
                        '            onkeydown="if (event.keyCode == 13 &amp;&amp; sayutils.isIE) this.onchange();"'+
                        '            onchange="'+
                        '                var grid = sayui[\''+ this.name +'\']; '+
                        '                var val = this.value; '+
                        '                var sel = jQuery(this).data(\'selected\');'+
                        '                var fld = jQuery(this).data(\'sayfield\'); '+
                        ...
                        '    </td>'+
                        '    <td>'+
                        '        <div class="sayui-search-clear" id="grid_'+ this.name +'_searchClear"  '+
                        '             onclick="var obj = sayui[\''+ this.name +'\']; obj.searchReset();" style="display: none"'+
                        '        >&#160;&#160;</div>'+
                        '    </td>'+
                        '</tr></tbody></table>'+
                        '</div>';
                    this.toolbar.items.push({ type: 'html', id: 'sayui-search', html: html });
                }
                ...
                if ((this.show.toolbarSearch || this.show.toolbarInput) && (this.show.toolbarAdd || this.show.toolbarEdit || this.show.toolbarDelete || this.show.toolbarSave)) {
                    this.toolbar.items.push({ type: 'break', id: 'sayui-break1' });
                }
                ...
                if (this.show.toolbarSave) {
                    if (this.show.toolbarAdd || this.show.toolbarDelete || this.show.toolbarEdit) {
                        this.toolbar.items.push({ type: 'break', id: 'sayui-break2' });
                    }
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        case 'sayui-reload':
                            ...
                            obj.trigger($.extend(edata2, { phase: 'after' }));
                            break;
                        case 'sayui-column-on-off':
                            obj.initColumnOnOff();
                            obj.initResize();
                            obj.resize();
                            break;
                        case 'sayui-search-advanced':
                            var tb = this;
                            var it = this.get(id);
                            if (it.checked) {
                                obj.searchClose();
                                ...
                                function tmp_close() {
                                    if ($('#sayui-overlay-'+ obj.name + '-searchOverlay').data('keepOpen') === true) return;
                                    tb.uncheck(id);
        ...
        resizeBoxes: function () {
            ...
            if (this.show.toolbar) {
                toolbar.css({
                    top:   ( 0 + (this.show.header ? sayutils.getSize(header, 'height') : 0) ) + 'px',
                    left:  '0px',
                    right: '0px'
                });
            }
            if (this.summary.length > 0) {
                fsummary.css({
                    bottom: ( 0 + (this.show.footer ? sayutils.getSize(footer, 'height') : 0) ) + 'px'
                });
                summary.css({
                    bottom: ( 0 + (this.show.footer ? sayutils.getSize(footer, 'height') : 0) ) + 'px',
                    right: '0px'
            ...
            body.css({
                top: ( 0 + (this.show.header ? sayutils.getSize(header, 'height') : 0) + (this.show.toolbar ? sayutils.getSize(toolbar, 'height') : 0) ) + 'px',
                bottom: ( 0 + (this.show.footer ? sayutils.getSize(footer, 'height') : 0) + (this.summary.length > 0 ? sayutils.getSize(summary, 'height') : 0) ) + 'px',
                left:   '0px',
                right:  '0px'
            });
        },
        ...
        resizeRecords: function () {
            ...
            $(this.box).find('.sayui-empty-record').remove();
            // -- Calculate Column size in PX
            var box      = $(this.box);
            var grid     = $(this.box).find('> div.sayui-grid-box');
            var header   = $('#grid_'+ this.name +'_header');
            var toolbar  = $('#grid_'+ this.name +'_toolbar');
            var summary  = $('#grid_'+ this.name +'_summary');
            var fsummary = $('#grid_'+ this.name +'_fsummary');
            ...
            if (body.height() - columns.height() < $(records).find('>table').height() + (bodyOverflowX ? sayutils.scrollBarSize() : 0)) bodyOverflowY = true;

            // body might be expanded by data
            if (!this.fixedBody) {
                // allow it to render records, then resize
                var calculatedHeight = sayutils.getSize(columns, 'height')
                    + sayutils.getSize($('#grid_'+ obj.name +'_records table'), 'height')
                    + (bodyOverflowX ? sayutils.scrollBarSize() : 0);
                obj.height = calculatedHeight
                    + sayutils.getSize(grid, '+height')
                    + (obj.show.header ? sayutils.getSize(header, 'height') : 0)
                    + (obj.show.toolbar ? sayutils.getSize(toolbar, 'height') : 0)
                    + (summary.css('display') != 'none' ? sayutils.getSize(summary, 'height') : 0)
                    + (obj.show.footer ? sayutils.getSize(footer, 'height') : 0);
                grid.css('height', obj.height);
                body.css('height', calculatedHeight);
                box.css('height', sayutils.getSize(grid, 'height') + sayutils.getSize(box, '+height'));
            } else {
                // fixed body height
                var calculatedHeight =  grid.height()
                    - (this.show.header ? sayutils.getSize(header, 'height') : 0)
                    - (this.show.toolbar ? sayutils.getSize(toolbar, 'height') : 0)
                    - (summary.css('display') != 'none' ? sayutils.getSize(summary, 'height') : 0)
                    - (this.show.footer ? sayutils.getSize(footer, 'height') : 0);
                body.css('height', calculatedHeight);
            }
            ...
            if (bodyOverflowX || bodyOverflowY) {
                columns.find('> table > tbody > tr:nth-child(1) td.sayui-head-last').css('width', sayutils.scrollBarSize()).show();
                records.css({
                    top: ((this.columnGroups.length > 0 && this.show.columns ? 1 : 0) + sayutils.getSize(columns, 'height')) +'px',
                    "-webkit-overflow-scrolling": "touch",
                    "overflow-x": (bodyOverflowX ? 'auto' : 'hidden'),
                    "overflow-y": (bodyOverflowY ? 'auto' : 'hidden')
                });
            } else {
                columns.find('> table > tbody > tr:nth-child(1) td.sayui-head-last').hide();
                records.css({
                    top: ((this.columnGroups.length > 0 && this.show.columns ? 1 : 0) + sayutils.getSize(columns, 'height')) +'px',
                    overflow: 'hidden'
                });
            ...
            if (bodyOverflowX) {
                frecords.css('margin-bottom', sayutils.scrollBarSize());
                scroll1.show();
            } else {
            ...
            function addEmptyRow(row, height, grid) {
                ...
                var html2 = '';
                var htmlp = '';
                html1 += '<tr class="'+ (row % 2 ? 'sayui-even' : 'sayui-odd') + ' sayui-empty-record" style="height: '+ height +'px">';
                html2 += '<tr class="'+ (row % 2 ? 'sayui-even' : 'sayui-odd') + ' sayui-empty-record" style="height: '+ height +'px">';
                if (grid.show.lineNumbers)  html1 += '<td class="sayui-col-number"></td>';
                if (grid.show.selectColumn) html1 += '<td class="sayui-grid-data sayui-col-select"></td>';
                if (grid.show.expandColumn) html1 += '<td class="sayui-grid-data sayui-col-expand"></td>';
                html2 += '<td class="sayui-grid-data-spacer" col="start" style="border-right: 0"></td>';
                for (var j = 0; j < grid.columns.length; j++) {
                    var col = grid.columns[j];
                    if ((col.hidden || j < grid.last.colStart || j > grid.last.colEnd) && !col.frozen) continue;
                    htmlp = '<td class="sayui-grid-data" '+ (col.attr != null ? col.attr : '') +' col="'+ j +'"></td>';
                    if (col.frozen) html1 += htmlp; else html2 += htmlp;
                }
                html1 += '<td class="sayui-grid-data-last"></td> </tr>';
                html2 += '<td class="sayui-grid-data-last" col="end"></td> </tr>';
                $('#grid_'+ grid.name +'_frecords > table').append(html1);
                $('#grid_'+ grid.name +'_records > table').append(html2);
            }
            if (body.length > 0) {
                var width_max = parseInt(body.width())
                    - (bodyOverflowY ? sayutils.scrollBarSize() : 0)
            ...
            columns.find('> table > tbody > tr:nth-child(1) td')
                ...
                .each(function (index, el) {
                    ...
                    // last column
                    if ($(el).hasClass('sayui-head-last')) {
                        if (obj.last.colEnd + 1 < obj.columns.length) {
                            ...
                            $(el).css('width', sayutils.scrollBarSize() + (width_diff > 0 && percent === 0 ? width_diff : 0) + 'px');
                        }
                    }
                });
            ...
            records.find('> table > tbody > tr:nth-child(1) td')
                ...
                .each(function (index, el) {
                    ...
                    // last column
                    if ($(el).hasClass('sayui-grid-data-last') && $(el).parents('.sayui-grid-frecords').length === 0) { // not in frecords
                        if (obj.last.colEnd + 1 < obj.columns.length) {
                            var width = 0;
                            for (var i = obj.last.colEnd + 1; i < obj.columns.length; i++) {
                                if (!obj.columns[i] || obj.columns[i].frozen || obj.columns[i].hidden) continue;
        ...
        getSearchesHTML: function () {
            ...
            for (var i = 0; i < this.searches.length; i++) {
                ...
                var btn = '';
                if (showBtn == false) {
                    btn = '<button class="sayui-btn close-btn" onclick="obj = sayui[\''+ this.name +'\']; if (obj) obj.searchClose()">X</button>';
                    showBtn = true;
                }
                ...
                var operator =
                    '<select id="grid_'+ this.name +'_operator_'+ i +'" class="sayui-input" onclick="event.stopPropagation();"' +
                    '   onchange="sayui[\''+ this.name + '\'].initOperator(this, '+ i +')">' +
                ...
                switch (s.type) {
                    ...
                    case 'enum':
                        ...
                        html += '<input rel="search" type="text" id="grid_'+ this.name +'_field_'+ i +'" name="'+ s.field +'" '+
                                '   class="sayui-input" style="'+ tmpStyle + s.style +'" '+ s.inTag +'/>';
                        break;
                    ...
                    case 'datetime':
                        ...
                        html += '<input rel="search" type="text" class="sayui-input" style="'+ tmpStyle + s.style +'" id="grid_'+ this.name +'_field_'+ i +'" name="'+ s.field +'" '+ s.inTag +'/>'+
                                '<span id="grid_'+ this.name +'_range_'+ i +'" style="display: none">&#160;-&#160;&#160;'+
                                '<input rel="search" type="text" class="sayui-input" style="'+ tmpStyle + s.style +'" id="grid_'+ this.name +'_field2_'+ i +'" name="'+ s.field +'" '+ s.inTag +'/>'+
                                '</span>';
                        break;

                    case 'select':
                        html += '<select rel="search" class="sayui-input" style="'+ s.style +'" id="grid_'+ this.name +'_field_'+ i +'" '+
            ...
            html += '<tr>'+
                    ...
                    '        <div>'+
                    '        <button class="sayui-btn" onclick="obj = sayui[\''+ this.name +'\']; if (obj) { obj.searchReset(); }">'+ sayutils.lang('Reset') + '</button>'+
                    '        <button class="sayui-btn sayui-btn-blue" onclick="obj = sayui[\''+ this.name +'\']; if (obj) { obj.search(); }">'+ sayutils.lang('Search') + '</button>'+
                    '        </div>'+
                    '    </td>'+
                    '</tr></tbody></table>';
            return html;
            ...
            function getOperators(type, fieldOperators) {
                ...
                for (var i = 0; i < operators.length; i++) {
                    ...
                    html += '<option value="'+ oper +'">'+ sayutils.lang(text) +'</option>\n';
                }
                return html;
            }
        },
...
//                    break;
                case 'between':
                    range.show();
                    fld2.sayfield(search.type, search.options);
                    break;
                case 'not null':
                case 'null':
                    fld1.hide();
        ...
        initSearches: function () {
            ...
            for (var s = 0; s < this.searches.length; s++) {
                ...
                switch (search.type) {
                    case 'text':
                    case 'alphanumeric':
                        $('#grid_'+ this.name +'_field_' + s).sayfield(search.type, search.options);
                        break;
                    ...
                    case 'datetime':
                        $('#grid_'+ this.name +'_field_'+s).sayfield(search.type, search.options);
                        $('#grid_'+ this.name +'_field2_'+s).sayfield(search.type, search.options);
                    ...
                    case 'enum':
                        ...
                        $('#grid_'+ this.name +'_field_'+s).sayfield(search.type, $.extend({ openOnFocus: true }, options));
                ...
                if (sdata != null) {
                    if (sdata.type == 'int' && ['in', 'not in'].indexOf(sdata.operator) != -1) {
                        $('#grid_'+ this.name +'_field_'+ s).sayfield('clear').val(sdata.value);
                    }
            ...
            // add on change event
            $('#sayui-overlay-'+ this.name +'-searchOverlay .sayui-grid-searches *[rel=search]').on('keypress', function (evnt) {
                if (evnt.keyCode == 13) {
                    obj.search();
                    $().sayoverlay({ name: obj.name + '-searchOverlay' });
                }
            });
        },
        ...
        getColumnsHTML: function () {
            ...
            function getGroups () {
                ...
                if (obj.show.lineNumbers) {
                    html1 += '<td class="sayui-head sayui-col-number">'+
                            '    <div style="height: '+ (obj.recordHeight+1) +'px">&#160;</div>'+
                            '</td>';
                }
                if (obj.show.selectColumn) {
                    html1 += '<td class="sayui-head sayui-col-select">'+
                            '    <div style="height: 25px">&#160;</div>'+
                            '</td>';
                }
                if (obj.show.expandColumn) {
                    html1 += '<td class="sayui-head sayui-col-expand">'+
                            '    <div style="height: 25px">&#160;</div>'+
                            '</td>';
                }
                var ii = 0;
                html2 += '<td id="grid_'+ obj.name + '_column_start" class="sayui-head" col="start" style="border-right: 0"></td>';
                    ...
                    if (colspan <= 0) {
                        ...
                        for (var si = 0; si < obj.sortData.length; si++) {
                            if (obj.sortData[si].field == col.field) {
                                if (new RegExp('asc', 'i').test(obj.sortData[si].direction))  sortStyle = 'sayui-sort-up';
                                if (new RegExp('desc', 'i').test(obj.sortData[si].direction)) sortStyle = 'sayui-sort-down';
                            }
                        }
                        var resizer = "";
                        if (col.resizable !== false) {
                            resizer = '<div class="sayui-resizer" name="'+ ii +'"></div>';
                        }
                        tmpf = '<td id="grid_'+ obj.name + '_column_' + ii +'" class="sayui-head '+ sortStyle +'" col="'+ ii + '" '+
                               '    rowspan="2" colspan="'+ colspan +'" '+
                               '    oncontextmenu = "sayui[\''+ obj.name +'\'].contextMenu(null, '+ ii +', event);"'+
                               '    onclick="sayui[\''+ obj.name +'\'].columnClick(\''+ col.field +'\', event);"'+
                               '    ondblclick="sayui[\''+ obj.name +'\'].columnDblClick(\''+ col.field +'\', event);">'+
                                   resizer +
                               '    <div class="sayui-col-group sayui-col-header '+ (sortStyle ? 'sayui-col-sorted' : '') +'">'+
                        ...
                        tmpf = '<td id="grid_'+ obj.name + '_column_' + ii +'" class="sayui-head" col="'+ ii + '" '+
                               '        colspan="'+ colspan +'">'+
                               '    <div class="sayui-col-group">'+
                ...
                html2 += '<td id="grid_'+ obj.name + '_column_end" class="sayui-head" col="end"></td></tr>';
                return [html1, html2];
            }
            ...
            function getColumns (master) {
                ...
                if (obj.show.lineNumbers) {
                    html1 += '<td class="sayui-head sayui-col-number" '+
                            '       onclick="sayui[\''+ obj.name +'\'].columnClick(\'line-number\', event);"'+
                            '       ondblclick="sayui[\''+ obj.name +'\'].columnDblClick(\'line-number\', event);">'+
                            '    <div>#</div>'+
                            '</td>';
                }
                if (obj.show.selectColumn) {
                    html1 += '<td class="sayui-head sayui-col-select"'+
                            ...
                            '            onclick="var grid = sayui[\''+ obj.name +'\'];'+
                ...
                html1 += '<td class="sayui-head sayui-head-last"><div>&#160;</div></td>';
                html2 += '<td class="sayui-head sayui-head-last" col="end"><div>&#160;</div></td>';
                html1 += '</tr>';
                html2 += '</tr>';
                return [html1, html2];
            }
        ...
        getColumnCellHTML: function (i) {
            ...
            if (col == null) return '';
            // reorder style
            var reorderCols = (this.reorderColumns && (!this.columnGroups || !this.columnGroups.length)) ? ' sayui-reorder-cols-head ' : '';
            // sort style
            var sortStyle = '';
            for (var si = 0; si < this.sortData.length; si++) {
                if (this.sortData[si].field == col.field) {
                    if (new RegExp('asc', 'i').test(this.sortData[si].direction))  sortStyle = 'sayui-sort-up';
                    if (new RegExp('desc', 'i').test(this.sortData[si].direction)) sortStyle = 'sayui-sort-down';
                }
            }
            // col selected
            ...
            var html = '<td id="grid_'+ this.name + '_column_' + i +'" col="'+ i +'" class="sayui-head '+ sortStyle + reorderCols + '" ' +
                             (this.columnTooltip == 'normal' && col.tooltip ? 'title="'+ col.tooltip +'" ' : '') +
                        '    onmouseover = "sayui[\''+ this.name +'\'].columnTooltipShow(\''+ i +'\', event);"'+
                        '    onmouseout  = "sayui[\''+ this.name +'\'].columnTooltipHide(\''+ i +'\', event);"'+
                        '    oncontextmenu = "sayui[\''+ this.name +'\'].contextMenu(null, '+ i +', event);"'+
                        '    onclick="sayui[\''+ this.name +'\'].columnClick(\''+ col.field +'\', event);"'+
                        '    ondblclick="sayui[\''+ this.name +'\'].columnDblClick(\''+ col.field +'\', event);">'+
                             (col.resizable !== false ? '<div class="sayui-resizer" name="'+ i +'"></div>' : '') +
                        '    <div class="sayui-col-header '+ (sortStyle ? 'sayui-col-sorted' : '') +' '+ (selected ? 'sayui-col-selected' : '') +'">'+
                        '        <div class="'+ sortStyle +'"></div>'+
                                (!col.caption ? '&#160;' : col.caption) +
                        '    </div>'+
                        '</td>';
        ...
        columnTooltipShow: function (ind) {
            ...
            setTimeout(function () {
                if ($el.prop('_mouse_over') === true && $el.prop('_mouse_tooltip') !== true) {
                    ...
                    // show tooltip
                    $el.saytag(item.tooltip, { position: pos });
                }
            }, 1);
        },
        ...
        getRecordsHTML: function () {
            ...
            html1 += '<tr id="grid_'+ this.name + '_frec_bottom" line="bottom" style="height: '+ ((buffered - limit) * this.recordHeight) +'px">'+
                    ...
                    '    <td colspan="2000" class="sayui-load-more"></td>'+
                    '</tr>'+
                    '</tbody></table>';
        ...
        scroll: function (event) {
            ...
            if (colStart != null) {
                ...
                if (colStart != obj.last.colStart || colEnd != obj.last.colEnd) {
                    ...
                    if (deltaStart < 5 && deltaEnd < 5) {
                        var $cfirst = $box.find('.sayui-grid-columns #grid_'+ obj.name +'_column_start');
                        var $clast  = $box.find('.sayui-grid-columns .sayui-head-last');
                        var $rfirst = $box.find('#grid_'+ obj.name +'_records .sayui-grid-data-spacer');
                        var $rlast  = $box.find('#grid_'+ obj.name +'_records .sayui-grid-data-last');
                        var $sfirst = $box.find('#grid_'+ obj.name +'_summary .sayui-grid-data-spacer');
                        var $slast  = $box.find('#grid_'+ obj.name +'_summary .sayui-grid-data-last');
                        // remove on left
                        ...
                        if (colStart < obj.last.colStart) {
                            for (var i = obj.last.colStart - 1; i >= colStart; i--) {
                                ...
                                $rfirst.each(function (ind, el) {
                                    ...
                                    var td    = '<td class="sayui-grid-data" col="'+ i +'" style="height: 0px"></td>'; // width column
            ...
            $('#grid_'+ this.name + '_footer .sayui-footer-right').html(
                (obj.show.statusRange ? sayutils.formatNumber(this.offset + t1) + '-' + sayutils.formatNumber(this.offset + t2) +
                        (this.total != -1 ? ' ' + sayutils.lang('of') + ' ' +    sayutils.formatNumber(this.total) : '') : '') +
                (url && obj.show.statusBuffered ? ' ('+ sayutils.lang('buffered') + ' '+ sayutils.formatNumber(buffered) +
                        (this.offset > 0 ? ', skip ' + sayutils.formatNumber(this.offset) : '') + ')' : '')
            );
            ...
            if (first < start || first == 1 || this.last.pull_refresh) { // scroll down
                ...
                for (var i = parseInt(rec_start) + 1; i <= end; i++) {
                    ...
                    var tmp2 = this.records[i-1].sayui;
                    ...
                    if (more.css('display') == 'none') {
                        more.show()
                            .on('click', function () {
                                ...
                                $(this).find('td').html('<div><div style="width: 20px; height: 20px;" class="sayui-spinner"></div></div>');
                            });
                    }
                    if (more.find('td .sayui-spinner').length > 0 || more.find('td').text().indexOf('Load') == -1) {
                        more.find('td').html('<div>'+ sayutils.lang('Load') + ' ' + obj.limit + ' ' + sayutils.lang('More') + '...</div>');
                    }
                }
            }
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if (ind == -1) {
                rec_html1 += '<tr line="0">';
                rec_html2 += '<tr line="0">';
                if (this.show.lineNumbers)  rec_html1 += '<td class="sayui-col-number" style="height: 0px;"></td>';
                if (this.show.selectColumn) rec_html1 += '<td class="sayui-col-select" style="height: 0px;"></td>';
                if (this.show.expandColumn) rec_html1 += '<td class="sayui-col-expand" style="height: 0px;"></td>';
                rec_html2 += '<td class="sayui-grid-data sayui-grid-data-spacer" col="start" style="height: 0px; width: 0px;"></td>';
                for (var i = 0; i < this.columns.length; i++) {
                    var col = this.columns[i];
                    tmph = '<td class="sayui-grid-data" col="'+ i +'" style="height: 0px;"></td>';
                    if (col.frozen && !col.hidden) {
                        rec_html1 += tmph;
                    } else {
                ...
                rec_html1 += '<td class="sayui-grid-data-last" style="height: 0px"></td>';
                rec_html2 += '<td class="sayui-grid-data-last" col="end" style="height: 0px"></td>';
                rec_html1 += '</tr>';
                rec_html2 += '</tr>';
            ...
            var id = sayutils.escapeId(record.recid);
            var isRowSelected = false;
            if (sel.indexes.indexOf(ind) != -1) isRowSelected = true;
            var rec_style = (record.sayui ? record.sayui.style : '');
            if (rec_style == null || typeof rec_style != 'string') rec_style = '';
            var rec_class = (record.sayui ? record.sayui.class : '');
            ...
            rec_html1 += '<tr id="grid_'+ this.name +'_frec_'+ record.recid +'" recid="'+ record.recid +'" line="'+ lineNum +'" index="'+ ind +'" '+
                ' class="'+ (lineNum % 2 === 0 ? 'sayui-even' : 'sayui-odd') + ' ' + rec_class +
                    (isRowSelected && this.selectType == 'row' ? ' sayui-selected' : '') +
                    (record.sayui && record.sayui.editable === false ? ' sayui-no-edit' : '') +
                    (record.sayui && record.sayui.expanded === true ? ' sayui-expanded' : '') + '" ' +
                (summary !== true ?
                    (sayutils.isIOS ?
                        '    onclick  = "sayui[\''+ this.name +'\'].dblClick(jQuery(this).attr(\'recid\'), event);"'
                        :
                        '    onclick  = "sayui[\''+ this.name +'\'].click(jQuery(this).attr(\'recid\'), event);"'+
                        '    oncontextmenu = "sayui[\''+ this.name +'\'].contextMenu(jQuery(this).attr(\'recid\'), null, event);"'
                     )
                    : ''
                ) +
                (this.selectType == 'row' ?
                    ' onmouseover="jQuery(\'#grid_'+ this.name +'_rec_\'+ sayutils.escapeId(jQuery(this).attr(\'recid\'))).addClass(\'sayui-record-hover\')"'+
                    ' onmouseout ="jQuery(\'#grid_'+ this.name +'_rec_\'+ sayutils.escapeId(jQuery(this).attr(\'recid\'))).removeClass(\'sayui-record-hover\')"'
                    :
                    '') +
            ...
            rec_html2 += '<tr id="grid_'+ this.name +'_rec_'+ record.recid +'" recid="'+ record.recid +'" line="'+ lineNum +'" index="'+ ind +'" '+
                ...
                (summary !== true ?
                    (sayutils.isIOS ?
                        '    onclick  = "var obj = sayui[\''+ this.name +'\']; obj.dblClick(jQuery(this).attr(\'recid\'), event);"'
                        :
                        '    onclick  = "var obj = sayui[\''+ this.name +'\']; obj.click(jQuery(this).attr(\'recid\'), event);"'+
                        '    oncontextmenu = "var obj = sayui[\''+ this.name +'\']; obj.contextMenu(jQuery(this).attr(\'recid\'), null, event);"'
                     )
                    : ''
                ) +
                (this.selectType == 'row' ?
                    ' onmouseover="jQuery(\'#grid_'+ this.name +'_frec_\' + sayutils.escapeId(jQuery(this).attr(\'recid\'))).addClass(\'sayui-record-hover\')"'+
                    ' onmouseout ="jQuery(\'#grid_'+ this.name +'_frec_\' + sayutils.escapeId(jQuery(this).attr(\'recid\'))).removeClass(\'sayui-record-hover\')"'
                    :
                    '') +
                ' style="height: '+ this.recordHeight +'px; '+ (!isRowSelected && rec_style != '' ? rec_style : rec_style.replace('background-color', 'none')) +'" '+
                    (rec_style != '' ? 'custom_style="'+ rec_style +'"' : '') +
            ...
            if (this.show.lineNumbers) {
                rec_html1 += '<td id="grid_'+ this.name +'_cell_'+ ind +'_number' + (summary ? '_s' : '') + '" '+
                            '   class="sayui-col-number '+ (isRowSelected  ? ' sayui-row-selected' : '') +'"'+
            ...
            if (this.show.selectColumn) {
                rec_html1 +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_select' + (summary ? '_s' : '') + '" class="sayui-grid-data sayui-col-select">'+
                            (summary !== true ?
                            '    <div>'+
                            '        <input class="sayui-grid-select-check" type="checkbox" tabindex="-1" '+
            ...
            if (this.show.expandColumn) {
                var tmp_img = '';
                if (record.sayui && record.sayui.expanded === true)  tmp_img = '-'; else tmp_img = '+';
                if (record.sayui && record.sayui.expanded == 'none') tmp_img = '';
                if (record.sayui && record.sayui.expanded == 'spinner') tmp_img = '<div class="sayui-spinner" style="width: 16px; margin: -2px 2px;"></div>';
                rec_html1 +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_expand' + (summary ? '_s' : '') + '" class="sayui-grid-data sayui-col-expand">'+
                            (summary !== true ?
                            '    <div ondblclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;" '+
                            '            onclick="sayui[\''+ this.name +'\'].toggle(jQuery(this).parents(\'tr\').attr(\'recid\')); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '        '+ tmp_img +' </div>'
                            :
                            '' ) +
            ...
            rec_html2 += '<td class="sayui-grid-data-spacer" col="start" style="border-right: 0"></td>';
            var col_ind   = 0;
            var col_skip  = 0;
            while (true) {
                var col_span  = 1;
                ...
                if (col_skip > 0) {
                    ...
                    record.sayui.colspan[this.columns[col_ind-1].field] = 0; // need it for other methods
                    col_skip--;
                    continue;
                } else if (record.sayui) {
                    var tmp1 = record.sayui.colspan;
                ...
                if (record.sayui) {
                    if (typeof record.sayui.colspan == 'object') {
                        var span = parseInt(record.sayui.colspan[col.field]) || null;
                        if (span > 1) {
            ...
            rec_html1 += '<td class="sayui-grid-data-last"></td>';
            rec_html2 += '<td class="sayui-grid-data-last" col="end"></td>';
            rec_html1 += '</tr>';
            rec_html2 += '</tr>';
            return [rec_html1, rec_html2];
        },
        ...
        getCellHTML: function (ind, col_ind, summary, col_span) {
            ...
            var isChanged     = !summary && record && record.sayui && record.sayui.changes && record.sayui.changes[col.field] != null;
            var addStyle      = '';
            var addClass      = '';
            var sel           = this.last.selection;
            var isRowSelected = false;
            ...
            if (sel.indexes.indexOf(ind) != -1) isRowSelected = true;
            if (col_span == null) {
                if (record && record.sayui && record.sayui.colspan && record.sayui.colspan[col.field]) {
                    col_span = record.sayui.colspan[col.field];
                } else {
                    col_span = 1;
                }
            }
            // expand icon
            if (col_ind === 0 && record && record.sayui && Array.isArray(record.sayui.children)) {
                var level  = 0;
                var subrec = this.get(record.sayui.parent_recid, true);
                while (true) {
                    if (subrec != null) {
                        level++
                        var tmp = this.records[subrec].sayui;
                ...
                if (record.sayui.parent_recid){
                        for (var i = 0; i < level; i++) {
                        infoBubble += '<span class="sayui-show-children sayui-icon-empty"></span>';
                        }
                }
                infoBubble += '<span class="sayui-show-children '+
                        (record.sayui.children.length > 0
                            ? (record.sayui.expanded ? 'sayui-icon-collapse' : 'sayui-icon-expand')
                            : 'sayui-icon-empty'
                        ) +'" '+
                    ' onclick="event.stopPropagation(); sayui[\''+ this.name + '\'].toggle(jQuery(this).parents(\'tr\').attr(\'recid\'))"></span>';
            }
            // info bubble
            if (col.info === true) col.info = {};
            if (col.info != null) {
                if (!col.info.icon) col.info.icon = 'sayui-icon-info';
                infoBubble += '<span class="sayui-info '+ col.info.icon +'" style="'+ (col.info.style || '') + '" '+
                    ' onclick="event.stopPropagation(); sayui[\''+ this.name + '\'].showBubble('+ ind +', '+ col_ind +')"></span>';
            }
            // various renderers
            if (col.render != null) {
                ...
                if (typeof col.render == 'string') {
                    ...
                    // formatters
                    var func = sayutils.formatters[tmp[0]];
                ...
                if (this.show.recordTitles) {
                    // title overwrite
                    var title = sayutils.stripTags(String(data).replace(/"/g, "''"));
                    if (col.title != null) {
            ...
            if (record && record.sayui) {
                if (typeof record.sayui.style == 'object') {
                    if (typeof record.sayui.style[col_ind] == 'string') addStyle += record.sayui.style[col_ind] + ';';
                    if (typeof record.sayui.style[col.field] == 'string') addStyle += record.sayui.style[col.field] + ';';
                }
                if (typeof record.sayui.class == 'object') {
                    if (typeof record.sayui.class[col_ind] == 'string') addClass += record.sayui.class[col_ind] + ' ';
                    if (typeof record.sayui.class[col.field] == 'string') addClass += record.sayui.class[col.field] + ' ';
                }
            }
            var isCellSelected = false;
            if (isRowSelected && $.inArray(col_ind, sel.columns[ind]) != -1) isCellSelected = true;
            // data
            data =  '<td class="sayui-grid-data'+ (isCellSelected ? ' sayui-selected' : '') + ' ' + addClass +
                        (isChanged ? ' sayui-changed' : '') +
                        '" '+
        ...
        showBubble: function (ind, col_ind) {
            ...
            var rec  = this.records[ind];
            var el   = $(this.box).find('#grid_'+ this.name +'_data_'+ ind +'_'+ col_ind + ' .sayui-info');
            if (this.last.bubbleEl) $(this.last.bubbleEl).saytag();
            ...
            if (typeof info.render == 'function') {
                ...
                for (var i = 0; i < fields.length; i++) {
                    ...
                    if (tmp.length > 1) {
                        if (sayutils.formatters[tmp[1]]) {
                            val = sayutils.formatters[tmp[1]](val, tmp[2] || null);
                        } else {
                            console.log('ERROR: sayutils.formatters["'+ tmp[1] + '"] does not exists.')
                        }
                    }
        ...
        getCellEditable: function (ind, col_ind) {
            ...
            var edit = (rec.sayui ? rec.sayui.editable : null);
        ...
        getCellValue: function (ind, col_ind, summary) {
            ...
            if (record && record.sayui && record.sayui.changes && record.sayui.changes[col.field] != null) {
                data = record.sayui.changes[col.field];
            }
        ...
        getFooterHTML: function () {
            return '<div>'+
                '    <div class="sayui-footer-left"></div>'+
                '    <div class="sayui-footer-right"></div>'+
                '    <div class="sayui-footer-center"></div>'+
                '</div>';
        },

        status: function (msg) {
            if (msg != null) {
                $('#grid_'+ this.name +'_footer').find('.sayui-footer-left').html(msg);
            } else {
                // show number of selected
                var msgLeft = '';
                var sel = this.getSelection();
                if (sel.length > 0) {
                    if (this.show.statusSelection && sel.length > 1) {
                        msgLeft = String(sel.length).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ' ' + sayutils.lang('selected');
                    }
                    if (this.show.statusRecordID && sel.length == 1) {
                        var tmp = sel[0];
                        if (typeof tmp == 'object') tmp = tmp.recid + ', '+ sayutils.lang('Column') +': '+ tmp.column;
                        msgLeft = sayutils.lang('Record ID') + ': '+ tmp + ' ';
                    }
                }
                $('#grid_'+ this.name +'_footer .sayui-footer-left').html(msgLeft);
                // toolbar
                if (sel.length == 1) this.toolbar.enable('sayui-edit'); else this.toolbar.disable('sayui-edit');
                if (sel.length >= 1) this.toolbar.enable('sayui-delete'); else this.toolbar.disable('sayui-delete');
            }
        ...
        lock: function (msg, showSpinner) {
            ...
            setTimeout(function () {
                ...
                sayutils.lock.apply(window, args);
            }, 10);
        },
        ...
        unlock: function (speed) {
            ...
            setTimeout(function () {
                ...
                if ($(box).find('.sayui-message').not('.sayui-closing').length > 0) return;
                sayutils.unlock(box, speed);
            }, 25); // needed timer so if server fast, it will not flash
        },

        stateSave: function (returnOnly) {
            var obj = this;
            if (!sayutils.hasLocalStorage) return null;
            var state = {
                columns     : [],
            ...
            if (returnOnly !== true) {
                ...
                if (edata.isCancelled === true) { if (typeof callBack == 'function') callBack({ status: 'error', message: 'Request aborted.' }); return; }
                try {
                    var savedState = $.parseJSON(localStorage.sayui || '{}');
                    if (!savedState) savedState = {};
                    if (!savedState.states) savedState.states = {};
                    savedState.states[(this.stateId || this.name)] = state;
                    localStorage.sayui = JSON.stringify(savedState);
                } catch (e) {
                    delete localStorage.sayui;
                    return null;
                }
                // event after
                this.trigger($.extend(edata, { phase: 'after' }));
        ...
        stateRestore: function (newState) {
            ...
            if (!newState) {
                ...
                try {
                    if (!sayutils.hasLocalStorage) return false;
                    var tmp = $.parseJSON(localStorage.sayui || '{}');
                    if (!tmp) tmp = {};
                ...
                for (var c = 0; c < obj.columns.length; c++) {
                    ...
                    if (['date', 'age'].indexOf(column.render.split(':')[0]) != -1) {
                        if (!rec[column.field + '_']) {
                            ...
                            if (sayutils.isInt(dt)) dt = parseInt(dt);
                            rec[column.field + '_'] = new Date(dt);
                        }
                    }
                    // time
                    if (['time'].indexOf(column.render) != -1) {
                        if (sayutils.isTime(rec[column.field])) { // if string
                            var tmp = sayutils.isTime(rec[column.field], true);
                            var dt = new Date();
                            ...
                            if (sayutils.isInt(tmp)) tmp = parseInt(tmp);
        ...
        nextCell: function (index, col_ind, editable) {
            ...
            var tmp  = this.records[index].sayui;
        ...
        message: function(options, callBack) {
            if (typeof options == 'string') {
                options = {
                    ...
                    body    : '<div class="sayui-centered">' + options + '</div>',
                    buttons : '<button class="sayui-btn" onclick="sayui[\''+ this.name +'\'].message()">Ok</button>',
                    onOpen  : function (event) {
                        setTimeout(function () {
                            $(this.box).find('.sayui-btn').focus();
                        }, 25);
                    },
            ...
            sayutils.message.call(this, {
                box   : this.box,
                path  : 'sayui.' + this.name,
                title : '.sayui-grid-header:visible',
                body  : '.sayui-grid-box'
            }, options);
        }
    };

    $.extend(saygrid.prototype, sayutils.event);
    sayobj.grid = saygrid;
})(jQuery);
...
(function ($) {
    var saylayout = function (options) {
        ...
        this.name    = null;     // unique name for sayui
        this.panels  = [];
        this.tmp     = {};
        ...
        $.extend(true, this, sayobj.layout, options);
    };

    var saypanels = ['top', 'left', 'main', 'preview', 'right', 'bottom'];

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.saylayout = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!sayutils.checkName(method, 'saylayout')) return;
            var panels = method.panels || [];
            var object = new saylayout(method);
            $.extend(object, { handlers: [], panels: [] });
            // add defined panels
            for (var p = 0, len = panels.length; p < len; p++) {
                object.panels[p] = $.extend(true, {}, saylayout.prototype.panel, panels[p]);
            ...
            for (var p1 = 0; p1 < saypanels.length; p1++) {
                if (object.get(saypanels[p1]) != null) continue;
                object.panels.push($.extend(true, {}, saylayout.prototype.panel, { type: saypanels[p1], hidden: (saypanels[p1] !== 'main'), size: 50 }));
            }
            if ($(this).length > 0) {
                object.render($(this)[0]);
            }
        ...
        function initTabs(object, panel, tabs) {
            ...
            $().saydestroy(object.name + '_' + panel + '_tabs'); // destroy if existed
            pan.tabs = $().saytabs($.extend({}, tabs, { owner: object, name: object.name + '_' + panel + '_tabs' }));
            pan.show.tabs = true;
            return true;
        }
        ...
        function initToolbar(object, panel, toolbar) {
            ...
            $().saydestroy(object.name + '_' + panel + '_toolbar'); // destroy if existed
            pan.toolbar = $().saytoolbar($.extend({}, toolbar, { owner: object, name: object.name + '_' + panel + '_toolbar' }));
    ...
    saylayout.prototype = {
        ...
        content: function (panel, data, transition) {
            ...
            if (data instanceof jQuery) {
                console.log('ERROR: You can not pass jQuery object to saylayout.content() method');
                return false;
            }
            var pname    = '#layout_'+ this.name + '_panel_'+ p.type;
            var current  = $(pname + '> .sayui-panel-content');
            var panelTop = 0;
            if (current.length > 0) {
                $(pname).scrollTop(0);
            ...
            if (p.content === '') {
                ...
                if (!p.hidden) {
                    if (transition != null && transition !== '') {
                        // apply transition
                        var div1 = $(pname + '> .sayui-panel-content');
                        div1.after('<div class="sayui-panel-content new-panel" style="'+ div1[0].style.cssText +'"></div>');
                        var div2 = $(pname + '> .sayui-panel-content.new-panel');
                        ...
                        sayutils.transition(div1[0], div2[0], transition, function () {
                            div1.remove();
        ...
        message: function(panel, options) {
            ...
            if (typeof options == 'string') {
                options = {
                    ...
                    height  : (options.length < 300 ? 170: 250),
                    body    : '<div class="sayui-centered">' + options + '</div>',
                    buttons : '<button class="sayui-btn" onclick="sayui[\''+ this.name +'\'].message(\''+ panel +'\')">Ok</button>',
                    onOpen  : function (event) {
                        setTimeout(function () {
                            $(this.box).find('.sayui-btn').focus();
                        }, 25);
            ...
            sayutils.message.call(this, {
                ...
                param : panel,
                path  : 'sayui.' + this.name,
                title : '.sayui-panel-title:visible',
                body  : '.sayui-panel-content'
            }, options);
        },
        ...
        sizeTo: function (panel, size, instant) {
            ...
            // resize
            $(obj.box).find(' > div > .sayui-panel')
                .css(sayutils.cssPrefix('transition', (instant !== true ? '.2s' : '0s')));
            setTimeout(function () {
                obj.set(panel, { size: size });
            }, 1);
            // clean
            setTimeout(function () {
                $(obj.box).find(' > div > .sayui-panel').css(sayutils.cssPrefix('transition', '0s'));
                obj.resize();
            }, 500);
            return true;
        },
        ...
        show: function (panel, immediate) {
            ...
            if (immediate === true) {
                ...
                $(obj.box).find(' > div > .sayui-panel').css(sayutils.cssPrefix('transition', '.2s'));
        ...
        el: function (panel) {
            var el = $('#layout_'+ this.name +'_panel_'+ panel +'> .sayui-panel-content');
        ...
        hideToolbar: function (panel) {
            ...
            $('#layout_'+ this.name +'_panel_'+ panel +'> .sayui-panel-toolbar').hide();
            this.resize();
        },
        ...
        showToolbar: function (panel) {
            ...
            if (!pan) return;
            pan.show.toolbar = true;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .sayui-panel-toolbar').show();
            this.resize();
        },
        ...
        assignToolbar: function (panel, toolbar) {
            if (typeof toolbar == 'string' && sayui[toolbar] != null) toolbar = sayui[toolbar];
            var pan = this.get(panel);
            pan.toolbar = toolbar;
            var tmp = $(this.box).find(panel +'> .sayui-panel-toolbar');
            if (pan.toolbar != null) {
                if (tmp.find('[name='+ pan.toolbar.name +']').length === 0) {
                    tmp.sayrender(pan.toolbar);
        ...
        hideTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = false;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .sayui-panel-tabs').hide();
            this.resize();
        },
        ...
        showTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = true;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .sayui-panel-tabs').show();
            this.resize();
        },
        ...
        render: function (box) {
            ...
            $(obj.box)
                .attr('name', obj.name)
                .addClass('sayui-layout')
                .html('<div></div>');
            if ($(obj.box).length > 0) $(obj.box)[0].style.cssText += obj.style;
            // create all panels
            for (var p1 = 0; p1 < saypanels.length; p1++) {
                var pan  = obj.get(saypanels[p1]);
                var html =  '<div id="layout_'+ obj.name + '_panel_'+ saypanels[p1] +'" class="sayui-panel">'+
                            '    <div class="sayui-panel-title"></div>'+
                            '    <div class="sayui-panel-tabs"></div>'+
                            '    <div class="sayui-panel-toolbar"></div>'+
                            '    <div class="sayui-panel-content"></div>'+
                            '</div>'+
                            '<div id="layout_'+ obj.name + '_resizer_'+ saypanels[p1] +'" class="sayui-resizer"></div>';
            ...
            function resizeStart(type, evnt) {
                ...
                // lock all panels
                for (var p1 = 0; p1 < saypanels.length; p1++) {
                    var $tmp = $(obj.el(saypanels[p1])).parent().find('.sayui-lock');
                    if ($tmp.length > 0) {
                        $tmp.attr('locked', 'previous');
                    } else {
                        obj.lock(saypanels[p1], { opacity: 0 });
                    }
                }
        ...
        refresh: function (panel) {
            ...
            if (typeof panel == 'string') {
                ...
                if (typeof p.content == 'object' && typeof p.content.render === 'function') {
                    p.content.box = $(pname +'> .sayui-panel-content')[0];
                    setTimeout(function () {
                        // need to remove unnecessary classes
                        if ($(pname +'> .sayui-panel-content').length > 0) {
                            $(pname +'> .sayui-panel-content')
                                .removeClass()
                                .removeAttr('name')
                                .addClass('sayui-panel-content')
                ...
                var tmp = $(obj.box).find(pname +'> .sayui-panel-tabs');
                if (p.show.tabs) {
                    if (tmp.find('[name='+ p.tabs.name +']').length === 0 && p.tabs != null) tmp.sayrender(p.tabs); else p.tabs.refresh();
                } else {
                    tmp.html('').removeClass('sayui-tabs').hide();
                }
                tmp = $(obj.box).find(pname +'> .sayui-panel-toolbar');
                if (p.show.toolbar) {
                    if (tmp.find('[name='+ p.toolbar.name +']').length === 0 && p.toolbar != null) tmp.sayrender(p.toolbar); else p.toolbar.refresh();
                } else {
                    tmp.html('').removeClass('sayui-toolbar').hide();
                }
                // show title
                tmp = $(obj.box).find(pname +'> .sayui-panel-title');
                if (p.title) {
        ...
        resize: function () {
            ...
            var l, t, w, h, e;
            // calculate %
            for (var p = 0; p < saypanels.length; p++) {
                if (saypanels[p] === 'main') continue;
                tmp = this.get(saypanels[p]);
                if (!tmp) continue;
            ...
            if (ptop != null && ptop.hidden !== true) {
                ...
                if (ptop.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_top').show().css({
                        ...
                        if (edata.isCancelled === true) return;
                        // default action
                        sayui[obj.name].tmp.events.resizeStart('top', event);
                        // event after
                        obj.trigger($.extend(edata, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pleft != null && pleft.hidden !== true) {
                ...
                if (pleft.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_left').show().css({
                        ...
                        if (edata.isCancelled === true) return;
                        // default action
                        sayui[obj.name].tmp.events.resizeStart('left', event);
                        // event after
                        obj.trigger($.extend(edata, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pright != null && pright.hidden !== true) {
                ...
                if (pright.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_right').show().css({
                        ...
                        if (edata.isCancelled === true) return;
                        // default action
                        sayui[obj.name].tmp.events.resizeStart('right', event);
                        // event after
                        obj.trigger($.extend(edata, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pbottom != null && pbottom.hidden !== true) {
                ...
                if (pbottom.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_bottom').show().css({
                        ...
                        if (edata.isCancelled === true) return;
                        // default action
                        sayui[obj.name].tmp.events.resizeStart('bottom', event);
                        // event after
                        obj.trigger($.extend(edata, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pprev != null && pprev.hidden !== true) {
                ...
                if (pprev.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_preview').show().css({
                        ...
                        if (edata.isCancelled === true) return;
                        // default action
                        sayui[obj.name].tmp.events.resizeStart('preview', event);
                        // event after
                        obj.trigger($.extend(edata, { phase: 'after' }));
                        return false;
                    });
            ...
            for (var p1 = 0; p1 < saypanels.length; p1++) {
                var pan = this.get(saypanels[p1]);
                var tmp2 = '#layout_'+ this.name +'_panel_'+ saypanels[p1] +' > .sayui-panel-';
                var tabHeight = 0;
                if (pan) {
                    if (pan.title) {
                        tabHeight += sayutils.getSize($(tmp2 + 'title').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                    if (pan.show.tabs) {
                        if (pan.tabs != null && sayui[this.name +'_'+ saypanels[p1] +'_tabs']) sayui[this.name +'_'+ saypanels[p1] +'_tabs'].resize();
                        tabHeight += sayutils.getSize($(tmp2 + 'tabs').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                    if (pan.show.toolbar) {
                        if (pan.toolbar != null && sayui[this.name +'_'+ saypanels[p1] +'_toolbar']) sayui[this.name +'_'+ saypanels[p1] +'_toolbar'].resize();
                        tabHeight += sayutils.getSize($(tmp2 + 'toolbar').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                }
            ...
            this._resize_timer = setTimeout(function () {
                for (var e in sayui) {
                    if (typeof sayui[e].resize == 'function') {
                        // sent to all none-layouts
                        if (sayui[e].panels == null) sayui[e].resize();
                        // only send to nested layouts
                        var parent = $(sayui[e].box).parents('.sayui-layout');
                        if (parent.length > 0 && parent.attr('name') == obj.name) sayui[e].resize();
                    }
                }
            }, 100);
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        destroy: function () {
            ...
            if (edata.isCancelled === true) return;
            if (sayui[this.name] == null) return false;
            // clean up
...
(function ($) {
    ...
    $.fn.saypopup = function(method, options) {
        if (method == null) {
            options = {};
            method  = 'open';
        }
        ...
        if ($(this).length > 0 && method == 'open') {
            if ($(this).find('div[rel=title], div[rel=body], div[rel=buttons]').length > 0) {
                ...
                if ($('#sayui-popup').length > 0) {
                    var tmp  = $('#sayui-popup').data('options');
                    saypopup._prev = {
                        template : saypopup._template,
                        title    : tmp.title,
                        body     : tmp.body,
                        buttons  : tmp.buttons
                    };
                }
                saypopup._template = this;
        ...
        // show popup
        return saypopup[method]($.extend({}, dlgOptions, options));
    };
    ...
    saypopup = {
        ...
        open: function (options) {
            var obj = this;
            if (saypopup.status == 'closing') {
                setTimeout(function () { obj.open.call(obj, options); }, 100);
                return;
            }
            // get old options and merge them
            var old_options = $('#sayui-popup').data('options');
            ...
            setTimeout(function () { $('#sayui-popup').data('options', options); }, 100);
            // if new - reset event handlers
            if ($('#sayui-popup').length === 0) {
                // saypopup.handlers  = []; // if commented, allows to add saypopup.on() for all
                saypopup.onMax     = null;
                saypopup.onMin     = null;
                saypopup.onToggle  = null;
                saypopup.onOpen    = null;
                saypopup.onClose   = null;
                saypopup.onKeydown = null;
            }
            if (options.onOpen)    saypopup.onOpen    = options.onOpen;
            if (options.onClose)   saypopup.onClose   = options.onClose;
            if (options.onMax)     saypopup.onMax     = options.onMax;
            if (options.onMin)     saypopup.onMin     = options.onMin;
            if (options.onToggle)  saypopup.onToggle  = options.onToggle;
            if (options.onKeydown) saypopup.onKeydown = options.onKeydown;
            ...
            if (window.innerHeight == undefined) {
                ...
                if (sayutils.engine === 'IE7') { maxW += 21; maxH += 4; }
            } else {
            ...
            if ($('#sayui-popup').length === 0) {
                ...
                if (edata.isCancelled === true) return;
                saypopup.status = 'opening';
                // output message
                saypopup.lockScreen(options);
                var btn = '';
                if (options.showClose) {
                    btn += '<div class="sayui-popup-button sayui-popup-close" onmousedown="event.stopPropagation()" onclick="saypopup.close()">Close</div>';
                }
                if (options.showMax) {
                    btn += '<div class="sayui-popup-button sayui-popup-max" onmousedown="event.stopPropagation()" onclick="saypopup.toggle()">Max</div>';
                }
                // first insert just body
                var msg = '<div id="sayui-popup" class="sayui-popup" style="opacity: 0; left: '+ left +'px; top: '+ top +'px;'+
                          '     width: ' + parseInt(options.width) + 'px; height: ' + parseInt(options.height) + 'px; '+
                              sayutils.cssPrefix('transform', 'scale(0.8)', true) + '"></div>';
                $('body').append(msg);
                // parse rel=*
                var parts = $('#sayui-popup');
                ...
                // then content
                var msg = '<div class="sayui-popup-title" style="'+ (!options.title ? 'display: none' : '') +'">' + btn + '</div>'+
                          '<div class="sayui-box" style="'+ (!options.title ? 'top: 0px !important;' : '') +
                                    (!options.buttons ? 'bottom: 0px !important;' : '') + '">'+
                          '    <div class="sayui-popup-body' + (!options.title !== '' ? ' sayui-popup-no-title' : '') +
                                    (!options.buttons ? ' sayui-popup-no-buttons' : '') + '" style="' + options.style + '">' +
                          '    </div>'+
                          '</div>'+
                          '<div class="sayui-popup-buttons" style="'+ (!options.buttons ? 'display: none' : '') +'"></div>'+
                          '<input class="sayui-popup-hidden" style="position: absolute; top: -100px"/>'; // this is needed to keep focus in popup
                $('#sayui-popup').html(msg);

                if (options.title) $('#sayui-popup .sayui-popup-title').append(options.title);
                if (options.buttons) $('#sayui-popup .sayui-popup-buttons').append(options.buttons);
                if (options.body) $('#sayui-popup .sayui-popup-body').append(options.body);

                // allow element to render
                setTimeout(function () {
                    $('#sayui-popup')
                        .css('opacity', '1')
                        .css(sayutils.cssPrefix({
                ...
                // clean transform
                setTimeout(function () {
                    $('#sayui-popup').css(sayutils.cssPrefix('transform', ''));
                    // event after
                    saypopup.status = 'open';
                    setTimeout(function () {
                        obj.trigger($.extend(edata, { phase: 'after' }));
                    }, 100);
                }, options.speed * 1000);
                ...
                if (saypopup._prev == null && saypopup._template != null) obj.restoreTemplate();
                ...
                if (old_options != null) {
                    if (!old_options.maximized && (old_options['width'] != options['width'] || old_options['height'] != options['height'])) {
                        saypopup.resize(options.width, options.height);
                    }
                    options.prevSize  = options.width + 'px:' + options.height + 'px';
                    options.maximized = old_options.maximized;
                }
                // show new items
                var cloned = $('#sayui-popup .sayui-box').clone();
                cloned.removeClass('sayui-box').addClass('sayui-box-temp').find('.sayui-popup-body').empty().append(options.body);
                // parse rel=*
                ...
                $('#sayui-popup .sayui-box').after(cloned);

                if (options.buttons) {
                    $('#sayui-popup .sayui-popup-buttons').show().html('').append(options.buttons);
                    $('#sayui-popup .sayui-popup-body').removeClass('sayui-popup-no-buttons');
                    $('#sayui-popup .sayui-box, #sayui-popup .sayui-box-temp').css('bottom', '');
                } else {
                    $('#sayui-popup .sayui-popup-buttons').hide().html('');
                    $('#sayui-popup .sayui-popup-body').addClass('sayui-popup-no-buttons');
                    $('#sayui-popup .sayui-box, #sayui-popup .sayui-box-temp').css('bottom', '0px');
                }
                if (options.title) {
                    $('#sayui-popup .sayui-popup-title')
                        .show()
                        .html((options.showClose ? '<div class="sayui-popup-button sayui-popup-close" onmousedown="event.stopPropagation()" onclick="saypopup.close()">Close</div>' : '') +
                              (options.showMax ? '<div class="sayui-popup-button sayui-popup-max" onmousedown="event.stopPropagation()" onclick="saypopup.toggle()">Max</div>' : ''))
                        .append(options.title);
                    $('#sayui-popup .sayui-popup-body').removeClass('sayui-popup-no-title');
                    $('#sayui-popup .sayui-box, #sayui-popup .sayui-box-temp').css('top', '');
                } else {
                    $('#sayui-popup .sayui-popup-title').hide().html('');
                    $('#sayui-popup .sayui-popup-body').addClass('sayui-popup-no-title');
                    $('#sayui-popup .sayui-box, #sayui-popup .sayui-box-temp').css('top', '0px');
                }
                // transition
                var div_old = $('#sayui-popup .sayui-box')[0];
                var div_new = $('#sayui-popup .sayui-box-temp')[0];
                sayutils.transition(div_old, div_new, options.transition, function () {
                    // clean up
                    obj.restoreTemplate();
                    $(div_old).remove();
                    $(div_new).removeClass('sayui-box-temp').addClass('sayui-box');
                    var $body = $(div_new).find('.sayui-popup-body');
                    if ($body.length == 1) $body[0].style.cssText = options.style;
                    // remove max state
                    $('#sayui-popup').data('prev-size', null);
            ...
            $('#sayui-popup .sayui-popup-title').on('mousedown', function (event) {
                if (!saypopup.get().maximized) mvStart(event);
            });

            // handlers
            function mvStart(evnt) {
                if (!evnt) evnt = window.event;
                saypopup.status = 'moving';
                tmp.resizing = true;
                tmp.isLocked = $('#sayui-popup > .sayui-lock').length == 1 ? true : false;
                tmp.x = evnt.screenX;
                tmp.y = evnt.screenY;
                tmp.pos_x = $('#sayui-popup').position().left;
                tmp.pos_y = $('#sayui-popup').position().top;
                if (!tmp.isLocked) saypopup.lock({ opacity: 0 });
            ...
            function mvMove(evnt) {
                ...
                $('#sayui-popup').css(sayutils.cssPrefix({
                    'transition': 'none',
            ...
            function mvStop(evnt) {
                ...
                if (!tmp.isLocked) saypopup.unlock();
            }
            return this;
        },

        keydown: function (event) {
            var options = $('#sayui-popup').data('options');
            if (options && !options.keyboard) return;
            // trigger event
            var edata = saypopup.trigger({ phase: 'before', type: 'keydown', target: 'popup', options: options, originalEvent: event });
            if (edata.isCancelled === true) return;
            // default behavior
            switch (event.keyCode) {
                case 27:
                    event.preventDefault();
                    if ($('#sayui-popup .sayui-message').length > 0) saypopup.message(); else saypopup.close();
                    break;
            }
            // event after
            saypopup.trigger($.extend(edata, { phase: 'after'}));
        },

        close: function (options) {
            var obj = this;
            var options = $.extend({}, $('#sayui-popup').data('options'), options);
            if ($('#sayui-popup').length === 0 || this.status == 'closed') return;
            if (this.status == 'opening') {
                setTimeout(function () { saypopup.close(); }, 100);
                return;
            }
            // trigger event
            ...
            saypopup.unlockScreen(options);
            setTimeout(function () {
                // return template
                obj.restoreTemplate();
                $('#sayui-popup').remove();
                saypopup.status = 'closed';
                // restore active
        ...
        toggle: function () {
            ...
            if (edata.isCancelled === true) return;
            // defatul action
            if (options.maximized === true) saypopup.min(); else saypopup.max();
            // event after
            setTimeout(function () {
                obj.trigger($.extend(edata, { phase: 'after'}));
        ...
        max: function () {
            ...
            if (edata.isCancelled === true) return;
            // default behavior
            saypopup.status   = 'resizing';
            options.prevSize = $('#sayui-popup').css('width') + ':' + $('#sayui-popup').css('height');
            // do resize
            saypopup.resize(10000, 10000, function () {
                saypopup.status    = 'open';
                options.maximized = true;
                obj.trigger($.extend(edata, { phase: 'after'}));
                // resize gird, form, layout inside popup
                $('#sayui-popup .sayui-grid, #sayui-popup .sayui-form, #sayui-popup .sayui-layout').each(function () {
                    var name = $(this).attr('name');
                    if (sayui[name] && sayui[name].resize) sayui[name].resize();
                })
            });
        },
        ...
        min: function () {
            ...
            saypopup.status = 'resizing';
            // do resize
            saypopup.resize(parseInt(size[0]), parseInt(size[1]), function () {
                saypopup.status = 'open';
        ...
        get: function () {
            return $('#sayui-popup').data('options');
        },

        set: function (options) {
            saypopup.open(options);
        },

        clear: function() {
            $('#sayui-popup .sayui-popup-title').html('');
            $('#sayui-popup .sayui-popup-body').html('');
            $('#sayui-popup .sayui-popup-buttons').html('');
        },

        reset: function () {
            saypopup.open(saypopup.defaults);
        },

        load: function (options) {
            saypopup.status = 'loading';
            if (options.url == null) {
            ...
            // load url
            var html = $('#sayui-popup').data(url);
            if (html != null) {
                popup(html, selector);
            } else {
                $.get(url, function (data, status, obj) { // should always be $.get as it is template
                    popup(obj.responseText, selector);
                    $('#sayui-popup').data(url, obj.responseText); // remember for possible future purposes
                });
            }
            function popup(html, selector) {
                delete options.url;
                $('body').append('<div id="sayui-tmp" style="display: none">' + html + '</div>');
                if (selector != null && $('#sayui-tmp #'+selector).length > 0) {
                    $('#sayui-tmp #' + selector).saypopup(options);
                } else {
                    $('#sayui-tmp > div').saypopup(options);
                }
                // link styles
                if ($('#sayui-tmp > style').length > 0) {
                    var style = $('<div>').append($('#sayui-tmp > style').clone()).html();
                    if ($('#sayui-popup #div-style').length === 0) {
                        $('#sayui-popup').append('<div id="div-style" style="position: absolute; left: -100; width: 1px"></div>');
                    }
                    $('#sayui-popup #div-style').html(style);
                }
                $('#sayui-tmp').remove();
            }
        },
        ...
        message: function (options) {
            ...
            $().saytag(); // hide all tags
            if (!options) options = { width: 200, height: 100 };
            var pWidth   = parseInt($('#sayui-popup').width());
            var pHeight  = parseInt($('#sayui-popup').height());
            options.originalWidth  = options.width;
            options.originalHeight = options.height;
            if (parseInt(options.width) < 10)  options.width  = 10;
            if (parseInt(options.height) < 10) options.height = 10;
            if (options.hideOnClick == null) options.hideOnClick = false;
            var poptions    = $('#sayui-popup').data('options') || {};
            var titleHeight = parseInt($('#sayui-popup > .sayui-popup-title').css('height'));
            if (options.width == null || options.width > poptions.width - 10) {
                options.width = poptions.width - 10;
            }
            if (options.height == null || options.height > poptions.height - titleHeight - 5) {
            ...
            var head     = $('#sayui-popup .sayui-popup-title');
            var msgCount = $('#sayui-popup .sayui-message').length;
            // remove message
            if ($.trim(options.html) === '' && $.trim(options.body) === '' && $.trim(options.buttons) === '') {
                var $msg = $('#sayui-popup #sayui-message'+ (msgCount-1));
                var options = $msg.data('options') || {};
                $msg.css(sayutils.cssPrefix({
                    'transition': '0.15s',
                    'transform': 'translateY(-' + options.height + 'px)'
                ...
                if (msgCount == 1) {
                    ...
                    $('#sayui-popup #sayui-message'+ (msgCount-2)).css('z-index', 1500);
                }
                setTimeout(function () {
                    var $focus = $msg.data('prev_focus');
                    $msg.remove();
                ...
                // hide previous messages
                $('#sayui-popup .sayui-message').css('z-index', 1390);
                head.css('z-index', 1501);
                // add message
                $('#sayui-popup .sayui-box')
                    .before('<div id="sayui-message' + msgCount + '" class="sayui-message" style="display: none; z-index: 1500; ' +
                                (head.length === 0 ? 'top: 0px;' : 'top: ' + sayutils.getSize(head, 'height') + 'px;') +
                                (options.width  != null ? 'width: ' + options.width + 'px; left: ' + ((pWidth - options.width) / 2) + 'px;' : 'left: 10px; right: 10px;') +
                                (options.height != null ? 'height: ' + options.height + 'px;' : 'bottom: 6px;') +
                                sayutils.cssPrefix('transition', '.3s', true) + '"' +
                                (options.hideOnClick === true ? 'onclick="saypopup.message();"' : '') + '>' +
                            '</div>');
                $('#sayui-popup #sayui-message'+ msgCount).data('options', options).data('prev_focus', $(':focus'));
                var display = $('#sayui-popup #sayui-message'+ msgCount).css('display');
                $('#sayui-popup #sayui-message'+ msgCount).css(sayutils.cssPrefix({
                    'transform': (display == 'none' ? 'translateY(-' + options.height + 'px)' : 'translateY(0px)')
                }));
                if (display == 'none') {
                    $('#sayui-popup #sayui-message'+ msgCount).show().html(options.html);
                    // timer needs to animation
                    setTimeout(function () {
                        $('#sayui-popup #sayui-message'+ msgCount).css(sayutils.cssPrefix({
                            'transform': (display == 'none' ? 'translateY(0px)' : 'translateY(-' + options.height + 'px)')
                    ...
                    // timer for lock
                    if (msgCount === 0) saypopup.lock();
                    setTimeout(function() {
                        obj.focus();
                        // has to be on top of lock
                        $('#sayui-popup #sayui-message'+ msgCount).css(sayutils.cssPrefix({ 'transition': '0s' }));
        ...
        focus: function () {
            ...
            // in message or popup
            var cnt = $('#sayui-popup .sayui-message').length - 1;
            var msg = $('#sayui-popup #sayui-message' + cnt);
            if (msg.length > 0) {
                var btn =$(msg[msg.length - 1]).find('button');
                if (btn.length > 0) btn[0].focus();
                tmp = msg;
            } else if (pop.length > 0) {
                var btn = pop.find('.sayui-popup-buttons button');
                if (btn.length > 0) btn[0].focus();
                tmp = pop;
            }
            ...
            $(tmp).find(sel)
                .on('blur.keep-focus', function (event) {
                    setTimeout(function () {
                        var focus = $(':focus');
                        if ((focus.length > 0 && !$(tmp).find(sel).is(focus)) || focus.hasClass('sayui-popup-hidden')) {
                            var el = $(tmp).find(sel);
        ...
        lock: function (msg, showSpinner) {
            var args = Array.prototype.slice.call(arguments, 0);
            args.unshift($('#sayui-popup'));
            sayutils.lock.apply(window, args);
        },

        unlock: function (speed) {
            sayutils.unlock($('#sayui-popup'), speed);
        },

        // --- INTERNAL FUNCTIONS

        lockScreen: function (options) {
            if ($('#sayui-lock').length > 0) return false;
            if (options == null) options = $('#sayui-popup').data('options');
            if (options == null) options = {};
            options = $.extend({}, saypopup.defaults, options);
            // show element
            $('body').append('<div id="sayui-lock" ' +
                ...
                '    style="position: ' + (sayutils.engine == 'IE5' ? 'absolute' : 'fixed') + '; z-Index: 1199; left: 0px; top: 0px; ' +
                '           padding: 0px; margin: 0px; background-color: ' + options.color + '; width: 100%; height: 100%; opacity: 0;"></div>');
            // lock screen
            setTimeout(function () {
                $('#sayui-lock')
                    .css('opacity', options.opacity)
                    .css(sayutils.cssPrefix('transition', options.speed + 's opacity'));
            }, 1);
            // add events
            if (options.modal == true) {
                $('#sayui-lock').on('mousedown', function () {
                    $('#sayui-lock')
                        .css('opacity', '0.6')
                        .css(sayutils.cssPrefix('transition', '.1s'));
                });
                $('#sayui-lock').on('mouseup', function () {
                    setTimeout(function () {
                        $('#sayui-lock')
                            .css('opacity', options.opacity)
                            .css(sayutils.cssPrefix('transition', '.1s'));
                ...
                $('#sayui-lock').on('mousedown', function () { saypopup.close(); });
            }
            return true;
        },
        ...
        **/
        ...
        restoreTemplate: function () {
            ...
            var template = saypopup._template;
            var title    = options.title;
            var body     = options.body;
            var buttons  = options.buttons;
            if (saypopup._prev) {
                template = saypopup._prev.template;
                title    = saypopup._prev.title;
                body     = saypopup._prev.body;
                buttons  = saypopup._prev.buttons;
                delete saypopup._prev;
            } else {
                delete saypopup._template;
            }
            if (template != null) {
                var $tmp = $(template);
                if ($tmp.length === 0) return;
    ...
    $.extend(saypopup, sayutils.event);
...
var sayalert = function (msg, title, callBack) {
    var $ = jQuery;
    if (title == null) title = sayutils.lang('Notification');
    if ($('#sayui-popup').length > 0 && saypopup.status != 'closing') {
        saypopup.message({
            width   : 400,
            height  : 170,
            body    : '<div class="sayui-centered sayui-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons : '<button onclick="saypopup.message();" class="sayui-popup-btn sayui-btn">' + sayutils.lang('Ok') + '</button>',
            onOpen: function () {
                $('#sayui-popup .sayui-message .sayui-popup-btn').focus();
            },
            onClose: function () {
                if (typeof callBack == 'function') callBack();
            }
        ...
        saypopup.open({
            ...
            title     : title,
            body      : '<div class="sayui-centered sayui-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons   : '<button onclick="saypopup.close();" class="sayui-popup-btn sayui-btn">' + sayutils.lang('Ok') + '</button>',
            onOpen: function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () { $('#sayui-popup .sayui-popup-btn').focus(); }, 1);
            },
            onKeydown: function (event) {
                $('#sayui-popup .sayui-popup-btn').focus().addClass('clicked');
            },
            onClose: function () {
                if (typeof callBack == 'function') callBack();
            }
...
var sayconfirm = function (msg, title, callBack) {
    var $ = jQuery;
    var options  = {};
    var defaults = {
        msg         : '',
        title       : sayutils.lang('Confirmation'),
        width       : ($('#sayui-popup').length > 0 ? 400 : 450),
        height      : ($('#sayui-popup').length > 0 ? 170 : 220),
        yes_text    : 'Yes',
        yes_class   : '',
        yes_style   : '',
        yes_callBack: null,
    ...
    if ($('#sayui-popup').length > 0 && saypopup.status != 'closing' && saypopup.get()) {
        if (options.width > saypopup.get().width) options.width = saypopup.get().width;
        if (options.height > (saypopup.get().height - 50)) options.height = saypopup.get().height - 50;
          saypopup.message({
            width   : options.width,
            height  : options.height,
            body    : '<div class="sayui-centered sayui-confirm-msg" style="font-size: 13px;">' + options.msg + '</div>',
            buttons : '<button id="Yes" class="sayui-popup-btn sayui-btn '+ options.yes_class +'" style="'+ options.yes_style +'">' + sayutils.lang(options.yes_text) + '</button>' +
                      '<button id="No" class="sayui-popup-btn sayui-btn '+ options.no_class +'" style="'+ options.no_style +'">' + sayutils.lang(options.no_text) + '</button>',
            onOpen: function () {
                $('#sayui-popup .sayui-message .sayui-btn').on('click.sayconfirm', function (event) {
                    saypopup._confirm_btn = event.target.id;
                    saypopup.message();
                });
            },
            onClose: function () {
                // needed this because there might be other messages
                $('#sayui-popup .sayui-message .sayui-btn').off('click.sayconfirm');
                // need to wait for message to slide up
                setTimeout(function () {
                    if (typeof options.callBack == 'function') options.callBack(saypopup._confirm_btn);
                    if (saypopup._confirm_btn == 'Yes' && typeof options.yes_callBack == 'function') options.yes_callBack();
                    if (saypopup._confirm_btn == 'No'  && typeof options.no_callBack == 'function') options.no_callBack();
                }, 300);
            }
        ...
        if (!sayutils.isInt(options.height)) options.height = options.height + 50;
        saypopup.open({
            width      : options.width,
            height     : options.height,
            title      : options.title,
            ...
            onOpen: function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () {
                    $('#sayui-popup .sayui-popup-btn').on('click', function (event) {
                        saypopup.close();
                    ...
                    $('#sayui-popup .sayui-popup-btn#Yes').focus();
                }, 1);
            },
            onKeydown: function (event) {
                // if there are no messages
                if ($('#sayui-popup .sayui-message').length === 0) {
                    switch (event.originalEvent.keyCode) {
                        case 13: // enter
                            $('#sayui-popup .sayui-popup-btn#Yes').focus().addClass('clicked'); // no need fo click as enter will do click
                            saypopup.close();
                            break;
                        case 27: // esc
                            $('#sayui-popup .sayui-popup-btn#No').focus().click();
                            saypopup.close();
                            break;
                    }
                }
...
var sayprompt = function (label, title, callBack) {
    var $ = jQuery;
    ...
    var defaults = {
        ...
        value       : '',
        attrs       : '',
        title       : sayutils.lang('Notification'),
        ok_text     : sayutils.lang('Ok'),
        cancel_text : sayutils.lang('Cancel'),
        width       : ($('#sayui-popup').length > 0 ? 400 : 450),
        height      : ($('#sayui-popup').length > 0 ? 170 : 220),
        callBack    : null
    }
    ...
    if ($('#sayui-popup').length > 0 && saypopup.status != 'closing' && saypopup.get()) {
        ...
        if (options.height > (saypopup.get().height - 50)) options.height = saypopup.get().height - 50;
          saypopup.message({
            width   : options.width,
            height  : options.height,
            body    : '<div class="sayui-centered" style="font-size: 13px;">'+
                      '   <label style="margin-right: 10px;">' + options.label + ':</label>'+
                      '   <input id="sayprompt" '+ options.attrs +'>'+
                      '</div>',
            buttons : '<button id="Ok" class="sayui-popup-btn sayui-btn">' + options.ok_text + '</button><button id="Cancel" class="sayui-popup-btn sayui-btn">' + options.cancel_text + '</button>',
            onOpen: function () {
                $('#sayprompt').val(options.value);
                $('#sayui-popup .sayui-message .sayui-btn#Ok').on('click.sayprompt', function (event) {
                    saypopup._prompt_value = $('#sayprompt').val();
                    saypopup.message();
                });
                $('#sayui-popup .sayui-message .sayui-btn#Cancel').on('click.sayprompt', function (event) {
                    saypopup._prompt_value = null;
                    saypopup.message();
                });
                // set focus
                setTimeout(function () { $('#sayprompt').focus(); }, 100);
            },
            onClose: function () {
                // needed this because there might be other messages
                $('#sayui-popup .sayui-message .sayui-btn').off('click.sayprompt');
                // need to wait for message to slide up
                setTimeout(function () {
                    if (typeof options.callBack == 'function' && saypopup._prompt_value != null) {
                        options.callBack(saypopup._prompt_value);
                    }
                }, 300);
            }
            // onKeydown will not work here
        ...
        saypopup.open({
            ...
            modal      : true,
            showClose  : false,
            body       : '<div class="sayui-centered" style="font-size: 13px;"><label style="margin-right: 10px;">' + options.label + ':</label><input id="sayprompt"></div>',
            buttons    : '<button id="Ok" class="sayui-popup-btn sayui-btn">' + options.ok_text + '</button><button id="Cancel" class="sayui-popup-btn sayui-btn">' + options.cancel_text + '</button>',
            onOpen: function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () {
                    $('#sayprompt').val(options.value);
                    $('#sayprompt').sayfield('text');
                    $('#sayui-popup .sayui-popup-btn#Ok').on('click', function (event) {
                        saypopup._prompt_value = $('#sayprompt').val();
                        saypopup.close();
                        if (typeof options.callBack == 'function') options.callBack(saypopup._prompt_value);
                    });
                    $('#sayui-popup .sayui-popup-btn#Cancel').on('click', function (event) {
                        saypopup._prompt_value = null;
                        saypopup.close();
                    });
                    $('#sayui-popup .sayui-popup-btn#Ok');
                    // set focus
                    setTimeout(function () { $('#sayprompt').focus(); }, 100);
                }, 1);
            },
            onKeydown: function (event) {
                ...
                if ($('#sayui-popup .sayui-message').length === 0) {
                    switch (event.originalEvent.keyCode) {
                        case 13: // enter
                            $('#sayui-popup .sayui-popup-btn#Ok').focus().addClass('clicked'); // no need fo click as enter will do click
                            saypopup.close();
                            break;
                        case 27: // esc
                            saypopup.close();
...
(function ($) {
    var saytabs = function (options) {
        ...
        $.extend(true, this, sayobj.tabs, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.saytabs = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!sayutils.checkName(method, 'saytabs')) return;
            // extend tabs
            var tabs   = method.tabs || [];
            var object = new saytabs(method);
            for (var i = 0; i < tabs.length; i++) {
                object.tabs[i] = $.extend({}, saytabs.prototype.tab, tabs[i]);
            }
            if ($(this).length !== 0) {
                object.render($(this)[0]);
            }
    ...
    saytabs.prototype = {
        ...
        insert: function (id, tab) {
            ...
            for (var i = 0; i < tab.length; i++) {
                ...
                if (!sayutils.checkUniqueId(tab[i].id, this.tabs, 'tabs', this.name)) return;
                // add tab
                var newTab = $.extend({}, saytabs.prototype.tab, tab[i]);
                if (id == null) {
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                // remove from screen
                $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(tab.id)).remove();
            }
            this.resize();
            return removed;
        },
        ...
        tooltipShow: function (id, event, forceRefresh) {
            ...
            var $el  = $(this.box).find('#tabs_'+ this.name + '_tab_'+ sayutils.escapeId(id));
            ...
            setTimeout(function () {
                if ($el.prop('_mouse_over') === true && $el.prop('_mouse_tooltip') !== true) {
                    $el.prop('_mouse_tooltip', true);
                    // show tooltip
                    $el.saytag(sayutils.lang(txt), { position: pos });
                }
        ...
        refresh: function (id) {
            var time = (new Date()).getTime();
            if (this.flow == 'up') $(this.box).addClass('sayui-tabs-up'); else $(this.box).removeClass('sayui-tabs-up');
            // event before
            ...
            if (id == null) {
                ...
                var jq_el    = $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(tab.id));
                var closable = '';
                if (tab.closable && !tab.disabled) {
                    closable = '<div class="sayui-tab-close" '+
                               '    onmouseover = "sayui[\''+ this.name +'\'].tooltipShow(\''+ tab.id +'\', event);"'+
                               '    onmouseout  = "sayui[\''+ this.name +'\'].tooltipHide(\''+ tab.id +'\', event);"'+
                               '    onclick="sayui[\''+ this.name +'\'].animateClose(\''+ tab.id +'\', event);">'+
                               '</div>';
                }
                var tabHTML = closable +
                    '    <div class="sayui-tab'+ (this.active === tab.id ? ' active' : '') + (tab.closable ? ' closable' : '')
                                + (tab['class'] ? ' ' + tab['class'] : '') +'" style="'+ tab.style +'" '+
                    '        onmouseover = "' + (!tab.disabled ? "sayui['"+ this.name +"'].tooltipShow('"+ tab.id +"', event);" : "") + '"'+
                    '        onmouseout  = "' + (!tab.disabled ? "sayui['"+ this.name +"'].tooltipHide('"+ tab.id +"', event);" : "") + '"'+
                    '        onclick="sayui[\''+ this.name +'\'].click(\''+ tab.id +'\', event);">' + sayutils.lang(text) + '</div>';
                if (jq_el.length === 0) {
                    ...
                    if (this.get(id, true) !== this.tabs.length-1 && $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(this.tabs[parseInt(this.get(id, true))+1].id)).length > 0) {
                        $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(this.tabs[parseInt(this.get(id, true))+1].id)).before(html);
                    } else {
        ...
        render: function (box) {
            ...
            if (box != null) {
                if ($(this.box).find('> table #tabs_'+ this.name + '_right').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('sayui-reset sayui-tabs')
                        .html('');
                }
                this.box = box;
            }
            if (!this.box) return false;
            // render all buttons
            var html =  '<div class="sayui-scroll-wrapper" onmousedown="var el=sayui[\''+ this.name +'\']; if (el) el.resize();">'+
                '<table cellspacing="0" cellpadding="1" width="100%"><tbody>'+
                '    <tr><td width="100%" id="tabs_'+ this.name +'_right" align="right">'+ this.right +'</td></tr>'+
                '</tbody></table>'+
                '</div>'+
                '<div class="sayui-scroll-left" onclick="var el=sayui[\''+ this.name +'\']; if (el) el.scroll(\'left\');"></div>'+
                '<div class="sayui-scroll-right" onclick="var el=sayui[\''+ this.name +'\']; if (el) el.scroll(\'right\');"></div>';
            $(this.box)
                .attr('name', this.name)
                .addClass('sayui-reset sayui-tabs')
                .html(html);
            if ($(this.box).length > 0) $(this.box)[0].style.cssText += this.style;
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        scroll: function (direction) {
            var box = $(this.box);
            var obj = this;
            var scrollBox  = box.find('.sayui-scroll-wrapper');
        ...
        click: function (id, event) {
            ...
            if (edata.isCancelled === true) return;
            // default action
            $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(this.active) +' .sayui-tab').removeClass('active');
            this.active = tab.id;
            // route processing
            if (typeof tab.route == 'string') {
                var route = tab.route !== '' ? String('/'+ tab.route).replace(/\/{2,}/g, '/') : '';
                var info  = sayutils.parseRoute(route);
                if (info.keys.length > 0) {
                    for (var k = 0; k < info.keys.length; k++) {
        ...
        animateClose: function(id, event) {
            ...
            // default action
            var obj = this;
            $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(tab.id)).css(sayutils.cssPrefix('transition', '.2s')).css('opacity', '0');
            setTimeout(function () {
                var width = $(obj.box).find('#tabs_'+ obj.name +'_tab_'+ sayutils.escapeId(tab.id)).width();
                $(obj.box).find('#tabs_'+ obj.name +'_tab_'+ sayutils.escapeId(tab.id))
                    .html('<div style="width: '+ width +'px; '+ sayutils.cssPrefix('transition', '.2s', true) +'"></div>');
                setTimeout(function () {
                    $(obj.box).find('#tabs_'+ obj.name +'_tab_'+ sayutils.escapeId(tab.id)).find(':first-child').css({ 'width': '0px' });
                }, 50);
            }, 200);
            setTimeout(function () {
                obj.remove(id);
        ...
        animateInsert: function(id, tab) {
            ...
            // check for unique
            if (!sayutils.checkUniqueId(tab.id, this.tabs, 'tabs', this.name)) return;
            // insert simple div
            var jq_el   = $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(tab.id));
            if (jq_el.length !== 0) return; // already exists
            // measure width
            if (tab.text == null && tab.caption != null) tab.text = tab.caption;
            var tmp = '<div id="_tmp_tabs" class="sayui-reset sayui-tabs" style="position: absolute; top: -1000px;">'+
                '<table cellspacing="0" cellpadding="1" width="100%"><tbody><tr>'+
                '<td id="_tmp_simple_tab" style="" valign="middle">'+
                    (tab.closable ? '<div class="sayui-tab-close"></div>' : '') +
                '    <div class="sayui-tab '+ (this.active === tab.id ? 'active' : '') +'">'+ tab.text +'</div>'+
                '</td></tr></tbody></table>'+
                '</div>';
            $('body').append(tmp);
            // create dummy element
            var tabHTML = '<div style="width: 1px; '+ sayutils.cssPrefix('transition', '.2s', true) +'">&#160;</div>';
            var addStyle = '';
            if (tab.hidden) { addStyle += 'display: none;'; }
            if (tab.disabled) { addStyle += 'opacity: 0.2;'; }
            var html = '<td id="tabs_'+ this.name +'_tab_'+ tab.id +'" style="'+ addStyle +'" valign="middle">'+ tabHTML +'</td>';
            if (this.get(id, true) !== this.tabs.length && $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(this.tabs[parseInt(this.get(id, true))].id)).length > 0) {
                $(this.box).find('#tabs_'+ this.name +'_tab_'+ sayutils.escapeId(this.tabs[parseInt(this.get(id, true))].id)).before(html);
            } else {
                $(this.box).find('#tabs_'+ this.name +'_right').before(html);
            }
            // -- move
            ...
            setTimeout(function () {
                ...
                $('#_tmp_tabs').remove();
                $('#tabs_'+ obj.name +'_tab_'+ sayutils.escapeId(tab.id) +' > div').css('width', width+'px');
            }, 1);
            setTimeout(function () {
                // insert for real
                obj.insert(id, tab);
    ...
    $.extend(saytabs.prototype, sayutils.event);
    sayobj.tabs = saytabs;
})(jQuery);
...
(function ($) {
    var saytoolbar = function (options) {
        ...
        $.extend(true, this, sayobj.toolbar, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.saytoolbar = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!sayutils.checkName(method, 'saytoolbar')) return;
            // extend items
            var items = method.items || [];
            var object = new saytoolbar(method);
            $.extend(object, { items: [], handlers: [] });
            for (var i = 0; i < items.length; i++) {
                object.items[i] = $.extend({}, saytoolbar.prototype.item, items[i]);
                // menus
    ...
    saytoolbar.prototype = {
        ...
        item: {
            ...
            text        : null,
            html        : '',
            tooltip     : null,        // saytoolbar.tooltip should be
            count       : null,
            hidden      : false,
            disabled    : false,
        ...
        insert: function (id, items) {
            ...
            for (var o = 0; o < items.length; o++) {
                ...
                if (items[o].type == null) {
                    console.log('ERROR: The parameter "type" is required but not supplied in saytoolbar.add() method.');
                    return;
                }
                if ($.inArray(String(items[o].type), ['button', 'check', 'radio', 'drop', 'menu', 'menu-radio', 'menu-check', 'color', 'text-color', 'break', 'html', 'spacer']) == -1) {
                    console.log('ERROR: The parameter "type" should be one of the following [button, check, radio, drop, menu, break, html, spacer] '+
                            'in saytoolbar.add() method.');
                    return;
                }
                if (items[o].id == null && items[o].type != 'break' && items[o].type != 'spacer') {
                    console.log('ERROR: The parameter "id" is required but not supplied in saytoolbar.add() method.');
                    return;
                }
                if (!sayutils.checkUniqueId(items[o].id, this.items, 'toolbar items', this.name)) return;
                // add item
                var it = $.extend({}, saytoolbar.prototype.item, items[o]);
                if (id == null) {
                    this.items.push(it);
                } else {
                    var middle = this.get(id, true);
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                removed++;
                // remove from screen
                $(this.box).find('#tb_'+ this.name +'_item_'+ sayutils.escapeId(it.id)).remove();
                // remove from array
        ...
        uncheck: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(it.type) != -1 && it.checked) {
                    // hide overlay
                    setTimeout(function () {
                        var el = $('#tb_'+ obj.name +'_item_'+ sayutils.escapeId(it.id));
                        el.sayoverlay({ name: obj.name });
                    }, 1);
                }
                items++;
                it.checked = false;
        ...
        click: function (id, event) {
            ...
            if (it && !it.disabled) {
                ...
                var btn = '#tb_'+ this.name +'_item_'+ sayutils.escapeId(it.id) +' table.sayui-button';
                ...
                if (['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(it.type) != -1) {
                    ...
                    if (it.checked) {
                        ...
                        setTimeout(function () {
                            ...
                            if (left > 19) left = 19;
                            if (it.type == 'drop') {
                                el.sayoverlay(it.html, $.extend({ name: obj.name, left: left, top: 3 }, it.overlay, {
                            ...
                            if (['menu', 'menu-radio', 'menu-check'].indexOf(it.type) != -1) {
                                ...
                                el.saymenu($.extend({ name: obj.name, items: it.items, left: left, top: 3 }, it.overlay, {
                                    type: menuType,
                            ...
                            if (['color', 'text-color'].indexOf(it.type) != -1) {
                                ...
                                $(el).saycolor({ color: it.color, transparent: it.transparent }, function (color, index) {
                                    if (color != null) {
        ...
        render: function (box) {
            ...
            if (box != null) {
                if ($(this.box).find('> table #tb_'+ this.name + '_right').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('sayui-reset sayui-toolbar')
                        .html('');
                }
                this.box = box;
            }
            ...
            $(this.box)
                .attr('name', this.name)
                .addClass('sayui-reset sayui-toolbar')
                .html(html);
            if ($(this.box).length > 0) $(this.box)[0].style.cssText += this.style;
            // refresh all
            this.refresh();
        ...
        refresh: function (id) {
            ...
            var el = $(this.box).find('#tb_'+ this.name +'_item_'+ sayutils.escapeId(it.id));
            ...
            if (el.length === 0) {
                ...
                if (this.get(id, true) == this.items.length-1) {
                    ...
                    $(this.box).find('#tb_'+ this.name +'_item_'+ sayutils.escapeId(this.items[parseInt(this.get(id, true))+1].id)).before(html);
                }
            } else {
                if (['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(it.type) != -1 && it.checked == false) {
                    if ($('#sayui-overlay-'+ this.name).length > 0) $('#sayui-overlay-'+ this.name)[0].hide();
                }
                // refresh
                el.html(html);
        ...
        getItemHTML: function (item) {
            ...
            if (item.img)  img = '<td><div class="sayui-tb-image sayui-icon '+ item.img +'"></div></td>';
            if (item.icon) img = '<td><div class="sayui-tb-image"><span class="'+ item.icon +'"></span></div></td>';
            ...
            if (html === '') switch (item.type) {
                ...
                case 'text-color':
                    ...
                    if (item.type == 'color') {
                        text = '<div style="height: 12px; width: 12px; margin-top: 1px; border: 1px solid #8A8A8A; border-radius: 1px; box-shadow: 0px 0px 1px #fff; '+
                               ...
                               (item.text ? '<div style="margin-left: 17px;">' + sayutils.lang(item.text) + '</div>' : '');
                    }
                    if (item.type == 'text-color') {
                        text = '<div style="color: #'+ (item.color != null ? item.color : '444') +';">'+
                                    (item.text ? sayutils.lang(item.text) : '<b>Aa</b>') +
                               '</div>';
                    }
                case 'menu':
                case 'menu-check':
                ...
                case 'drop':
                    html += '<table cellpadding="0" cellspacing="0" '+
                            '       class="sayui-button '+ (item.checked ? 'checked' : '') +'" '+
                            '       onclick     = "var el=sayui[\''+ this.name + '\']; if (el) el.click(\''+ item.id +'\', event);" '+
                            '       onmouseover = "' + (!item.disabled ? "jQuery(this).addClass('over'); sayui['"+ this.name +"'].tooltipShow('"+ item.id +"', event);" : "") + '"'+
                            '       onmouseout  = "' + (!item.disabled ? "jQuery(this).removeClass('over').removeClass('down'); sayui['"+ this.name +"'].tooltipHide('"+ item.id +"', event);" : "") + '"'+
                            ...
                            '  <tr>' +
                                    img +
                                    (text !== ''
                                        ? '<td class="sayui-tb-caption" nowrap="nowrap" style="'+ (item.style ? item.style : '') +'">'+ sayutils.lang(text) +'</td>'
                                        : ''
                                    ) +
                                    (item.count != null
                                        ? '<td class="sayui-tb-count" nowrap="nowrap"><span>'+ item.count +'</span></td>'
                                        : ''
                                    ) +
                                    (((['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(item.type) != -1) && item.arrow !== false) ?
                                        '<td class="sayui-tb-down" nowrap="nowrap"><div></div></td>' : '') +
                            '  </tr></tbody></table>'+
                            '</td></tr></tbody></table>';
                    break;
                ...
                case 'break':
                    html += '<table cellpadding="0" cellspacing="0"><tbody><tr>'+
                            '    <td><div class="sayui-break">&#160;</div></td>'+
                            '</tr></tbody></table>';
                    break;
        ...
        tooltipShow: function (id, event, forceRefresh) {
            if (this.tooltip == null) return;
            var $el  = $(this.box).find('#tb_'+ this.name + '_item_'+ sayutils.escapeId(id));
            var item = this.get(id);
            var pos  = this.tooltip;
            var txt  = item.tooltip;
            if (typeof txt == 'function') txt = txt.call(this, item);
...
(function ($) {
    var saysidebar = function (options) {
        ...
        $.extend(true, this, sayobj.sidebar, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.saysidebar = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!sayutils.checkName(method, 'saysidebar')) return;
            // extend items
            var nodes  = method.nodes;
            var object = new saysidebar(method);
    ...
    saysidebar.prototype = {
        ...
        insert: function (parent, before, nodes) {
            ...
            for (var o = 0; o < nodes.length; o++) {
                ...
                tmp = $.extend({}, saysidebar.prototype.node, node);
                tmp.sidebar = this;
                tmp.parent  = parent;
                nd = tmp.nodes || [];
        ...
        select: function (id) {
            ...
            $(this.box).find('#node_'+ sayutils.escapeId(id))
                .addClass('sayui-selected')
                .find('.sayui-icon').addClass('sayui-icon-selected');
        ...
        unselect: function (id) {
            ...
            $(this.box).find('#node_'+ sayutils.escapeId(id))
                .removeClass('sayui-selected')
                .find('.sayui-icon').removeClass('sayui-icon-selected');
        ...
        collapse: function (id) {
            ...
            if (edata.isCancelled === true) return;
            // default action
            $(this.box).find('#node_'+ sayutils.escapeId(id) +'_sub').slideUp(200);
            $(this.box).find('#node_'+ sayutils.escapeId(id) +' .sayui-node-dots:first-child').html('<div class="sayui-expand">+</div>');
            nd.expanded = false;
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        expand: function (id) {
            ...
            if (edata.isCancelled === true) return;
            // default action
            $(this.box).find('#node_'+ sayutils.escapeId(id) +'_sub').slideDown(200);
            $(this.box).find('#node_'+ sayutils.escapeId(id) +' .sayui-node-dots:first-child').html('<div class="sayui-expand">-</div>');
            nd.expanded = true;
            // event after
            this.trigger($.extend(edata, { phase: 'after' }));
        ...
        click: function (id, event) {
            ...
            $(obj.box).find('.sayui-node.sayui-selected').each(function (index, el) {
                var oldID     = $(el).attr('id').replace('node_', '');
                var oldNode = obj.get(oldID);
                if (oldNode != null) oldNode.selected = false;
                $(el).removeClass('sayui-selected').find('.sayui-icon').removeClass('sayui-icon-selected');
            });
            // select new one
            var newNode = $(obj.box).find('#node_'+ sayutils.escapeId(id));
            var oldNode = $(obj.box).find('#node_'+ sayutils.escapeId(obj.selected));
            newNode.addClass('sayui-selected').find('.sayui-icon').addClass('sayui-icon-selected');
            ...
            setTimeout(function () {
                ...
                if (edata.isCancelled === true) {
                    // restore selection
                    newNode.removeClass('sayui-selected').find('.sayui-icon').removeClass('sayui-icon-selected');
                    oldNode.addClass('sayui-selected').find('.sayui-icon').addClass('sayui-icon-selected');
                    return;
                }
                // default action
                if (oldNode != null) oldNode.selected = false;
        ...
        focus: function (event) {
            ...
            // default behaviour
            this.hasFocus = true;
            $(this.box).find('.sayui-selected').removeClass('sayui-inactive');
            setTimeout(function () {
        ...
        scrollIntoView: function (id) {
            ...
            var nd = this.get(id);
            if (nd == null) return;
            var body   = $(this.box).find('.sayui-sidebar-div');
            var item   = $(this.box).find('#node_'+ sayutils.escapeId(id));
        ...
        contextMenu: function (id, event) {
            ...
            if (obj.menu.length > 0) {
                $(obj.box).find('#node_'+ sayutils.escapeId(id))
                    .saymenu({
                        items: obj.menu,
                        contextMenu: true,
                        originalEvent: event,
        ...
        render: function (box) {
            ...
            // default action
            if (box != null) {
                if ($(this.box).find('> div > div.sayui-sidebar-div').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('sayui-reset sayui-sidebar')
                        .html('');
                }
                this.box = box;
            }
            ...
            $(this.box)
                .attr('name', this.name)
                .addClass('sayui-reset sayui-sidebar')
                .html('<div>'+
                        '<input id="sidebar_'+ this.name +'_focus" style="position: absolute; top: 0; right: 0; width: 1px; z-index: -1; opacity: 0"/>'+
                        '<div class="sayui-sidebar-top"></div>' +
                        '<div class="sayui-sidebar-div"></div>'+
                        '<div class="sayui-sidebar-bottom"></div>'+
                    '</div>'
                );
            $(this.box).find('> div').css({
            ...
            if (this.flatButton == true) {
                flatHTML = '<div class="sayui-flat-'+ (this.flat ? 'right' : 'left') +'" onclick="sayui[\''+ this.name +'\'].goFlat()"></div>';
            }
            if (this.topHTML !== '' || flatHTML !== '') {
                $(this.box).find('.sayui-sidebar-top').html(this.topHTML + flatHTML);
                $(this.box).find('.sayui-sidebar-div')
                    .css('top', $(this.box).find('.sayui-sidebar-top').height() + 'px');
            }
            if (this.bottomHTML !== '') {
                $(this.box).find('.sayui-sidebar-bottom').html(this.bottomHTML);
                $(this.box).find('.sayui-sidebar-div')
                    .css('bottom', $(this.box).find('.sayui-sidebar-bottom').height() + 'px');
            }
            // focus
            var kbd_timer;
            ...
            $(this.box).off('mousedown').on('mousedown', function (event) {
                ...
                setTimeout(function () {
                    ...
                    if (['INPUT', 'TEXTAREA', 'SELECT'].indexOf(event.target.tagName.toUpperCase()) == -1) {
                        var $input = $(obj.box).find('#sidebar_'+ obj.name + '_focus');
                        if (!$input.is(':focus')) {
                            if ($(event.target).hasClass('sayui-node')) {
                                var top = $(event.target).position().top + $(obj.box).find('.sayui-sidebar-top').height() + event.offsetY;
        ...
        refresh: function (id) {
            ...
            // default action
            $(this.box).find('> div').removeClass('sayui-sidebar-flat').addClass(this.flat ? 'sayui-sidebar-flat' : '').css({
                width : $(this.box).width() + 'px',
                height: $(this.box).height() + 'px'
            });
            var obj = this;
            ...
            if (id == null) {
                node = this;
                nm   = '.sayui-sidebar-div';
            } else {
                node = this.get(id);
                if (node == null) return;
                nm   = '#node_'+ sayutils.escapeId(node.id) + '_sub';
            }
            var nodeHTML;
            if (node !== this) {
                var tmp  = '#node_'+ sayutils.escapeId(node.id);
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    html =
                        '<div class="sayui-node-group sayui-level-'+ level +'" id="node_'+ nd.id +'"'+
                        '   onclick="sayui[\''+ obj.name +'\'].toggle(\''+ nd.id +'\')"'+
                        '   oncontextmenu="sayui[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                        ...
                        ((nd.groupShowHide && nd.collapsible) ? '<span>'+ (!nd.hidden && nd.expanded ? sayutils.lang('Hide') : sayutils.lang('Show')) +'</span>' : '<span></span>') +
                        (typeof nd.text == 'function' ? nd.text.call(obj, nd) : '<span>'+ nd.text +'</span>') +
                        '</div>'+
                        '<div class="sayui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    if (obj.flat) {
                        html = '<div class="sayui-node-group" id="node_'+ nd.id +'"><span>&#160;</span></div>'+
                               '<div id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    }
                } else {
                    if (nd.selected && !nd.disabled) obj.selected = nd.id;
                    tmp = '';
                    if (img) tmp  = '<div class="sayui-node-image sayui-icon '+ img +    (nd.selected && !nd.disabled ? " sayui-icon-selected" : "") +'"></div>';
                    if (icon) tmp = '<div class="sayui-node-image"><span class="'+ icon +'"></span></div>';
                    var text = nd.text;
                    if (typeof nd.text == 'function') text = nd.text.call(obj, nd);
                    html =  '<div class="sayui-node sayui-level-'+ level +' '+ (nd.selected ? 'sayui-selected' : '') +' '+ (nd.disabled ? 'sayui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                            '    ondblclick="sayui[\''+ obj.name +'\'].dblClick(\''+ nd.id +'\', event);"'+
                            '    oncontextmenu="sayui[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                            '    onClick="sayui[\''+ obj.name +'\'].click(\''+ nd.id +'\', event); ">'+
                            '<table cellpadding="0" cellspacing="0" style="margin-left:'+ (level*18) +'px; padding-right:'+ (level*18) +'px"><tbody><tr>'+
                            '<td class="sayui-node-dots" nowrap="nowrap" onclick="sayui[\''+ obj.name +'\'].toggle(\''+ nd.id +'\'); '+
                            '        if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '    <div class="sayui-expand">'    + (nd.nodes.length > 0 ? (nd.expanded ? '-' : '+') : (nd.plus ? '+' : '')) + '</div>' +
                            '</td>'+
                            '<td class="sayui-node-data" nowrap="nowrap">'+
                                    tmp +
                                    (nd.count || nd.count === 0 ? '<div class="sayui-node-count">'+ nd.count +'</div>' : '') +
                                    '<div class="sayui-node-caption">'+ text +'</div>'+
                            '</td>'+
                            '</tr></tbody></table>'+
                            '</div>'+
                            '<div class="sayui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    if (obj.flat) {
                        html =  '<div class="sayui-node sayui-level-'+ level +' '+ (nd.selected ? 'sayui-selected' : '') +' '+ (nd.disabled ? 'sayui-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                                '    onmouseover="jQuery(this).find(\'.sayui-node-data\').saytag(sayutils.base64decode(\''+
                                                sayutils.base64encode(text + (nd.count || nd.count === 0 ? ' - <span class="sayui-node-count">'+ nd.count +'</span>' : '')) + '\'), '+
                                '               { id: \'' + nd.id + '\', left: -5 })"'+
                                '    onmouseout="jQuery(this).find(\'.sayui-node-data\').saytag(null, { id: \'' + nd.id + '\' })"'+
                                '    ondblclick="sayui[\''+ obj.name +'\'].dblClick(\''+ nd.id +'\', event);"'+
                                '    oncontextmenu="sayui[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                                '    onClick="sayui[\''+ obj.name +'\'].click(\''+ nd.id +'\', event); ">'+
                                '<div class="sayui-node-data sayui-node-flat">'+ tmp +'</div>'+
                                '</div>'+
                                '<div class="sayui-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    }
                }
        ...
        unlock: function (speed) {
            sayutils.unlock(this.box, speed);
        }
    };
...
(function ($) {
    ...
    var sayfield = function (options) {
        ...
        $.extend(true, this, sayobj.field);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.sayfield = function (method, options) {
        // call direct
        if (this.length === 0) {
            var pr = sayfield.prototype;
            if (pr[method]) {
            ...
            return this.each(function (index, el) {
                var obj = $(el).data('sayfield');
                // if object is not defined, define it
                if (obj == null) {
                    var obj = new sayfield(method);
                    $.extend(obj, { handlers: [] });
                    if (el) obj.el = $(el)[0];
                    obj.init();
                    $(el).data('sayfield', obj);
                    return obj;
                } else { // fully re-init
                    obj.clear();
    ...
    /*     To add custom types
        $().sayfield('addType', 'myType', function (options) {
            ...
            $(this.el).on('blur', function (event)  { // keyCode & charCode differ in FireFox
                var ch = this.value;
                if (ch != 'a' && ch != 'b' && ch != 'c') {
                    $(this).saytag(sayutils.lang("Not a single character from the set of 'abc'"));
                }
            });
        });
    */
    ...
    sayfield.prototype = {
        ...
        init: function () {
            ...
            if (['INPUT', 'TEXTAREA'].indexOf(this.el.tagName.toUpperCase()) == -1) {
                console.log('ERROR: sayfield could only be applied to INPUT or TEXTAREA.', this.el);
                return;
            }
            ...
            switch (this.type) {
                ...
                case 'hex':
                    defaults = {
                        ...
                        step               : 1,
                        autoFormat         : true,
                        currencyPrefix     : sayutils.settings.currencyPrefix,
                        currencySuffix     : sayutils.settings.currencySuffix,
                        currencyPrecision  : sayutils.settings.currencyPrecision,
                        decimalSymbol      : sayutils.settings.decimalSymbol,
                        groupSymbol        : sayutils.settings.groupSymbol,
                        arrows             : false,
                        keyboard           : true,
                        precision          : null,
                        silent             : true,
                ...
                case 'date':
                    defaults = {
                        format       : sayutils.settings.dateFormat, // date format
                        keyboard     : true,
                        silent       : true,
                ...
                case 'time':
                    defaults = {
                        format    : sayutils.settings.timeFormat,
                        keyboard  : true,
                        silent    : true,
                        start     : '',
                        end       : '',
                ...
                case 'datetime':
                    defaults = {
                        format      : sayutils.settings.dateFormat + ' | ' + sayutils.settings.timeFormat,
                        keyboard    : true,
                        silent      : true,
                        ...
                        colored     : {},       // { '12/17/2014': 'blue:green', '12/18/2014': 'gray:white'  }; // key has to be formatted with sayutils.settings.dateFormat
                ...
                case 'combo':
                    defaults = {
                        ...
                        method          : null,          // default comes from sayutils.settings.dataType
                ...
                case 'file':
                    ...
                    if ($(this.el).attr('placeholder') == null) {
                        $(this.el).attr('placeholder', sayutils.lang('Attach files by dragging and dropping or Click to Select'));
                    }
                    this.addMulti();
                    this.watchSize();
                    break;
            ...
            $(this.el)
                .addClass('sayfield sayui-input')
                .data('sayfield', this)
                .on('change.sayfield',   this.tmp.onChange)
                .on('click.sayfield',    this.tmp.onClick)         // ignore click because it messes overlays
                .on('focus.sayfield',    this.tmp.onFocus)
                .on('blur.sayfield',     this.tmp.onBlur)
                .on('keydown.sayfield',  this.tmp.onKeydown)
                .on('keyup.sayfield',    this.tmp.onKeyup)
                .on('keypress.sayfield', this.tmp.onKeypress)
                .css(sayutils.cssPrefix('box-sizing', 'border-box'));
        ...
        clear: function () {
            ...
            if (this.type == 'list') {
                $(this.el).removeClass('sayui-select');
            }
            this.type = 'clear';
            ...
            $(this.el)
                ...
                .removeClass('sayfield')
                .removeData()       // removes all attached data
                .off('.sayfield');   // remove only events added by sayfield
            // remove helpers
        ...
        refresh: function () {
            ...
            if (['list'].indexOf(this.type) != -1) {
                ...
                setTimeout(function () {
                    ...
                    if (!$.isEmptyObject(selected) && options.icon) {
                        options.prefix = '<span class="sayui-icon '+ options.icon +'"style="cursor: pointer; font-size: 14px;' +
                    ...
                    if ($(focus).val() === '') {
                        ...
                        $(obj.el).val(selected && selected.text != null ? sayutils.lang(selected.text) : '');
                    } else {
            ...
            if (['enum', 'file'].indexOf(this.type) != -1) {
                ...
                if (selected) {
                   for (var s = 0; s < selected.length; s++) {
                       ...
                       if (typeof options.renderItem == 'function') {
                           ren = options.renderItem(it, s, '<div class="sayui-list-remove" title="'+ sayutils.lang('Remove') +'" index="'+ s +'">&#160;&#160;</div>');
                       } else {
                           ren = '<div class="sayui-list-remove" title="'+ sayutils.lang('Remove') +'" index="'+ s +'">&#160;&#160;</div>'+
                                 (obj.type == 'enum' ? it.text : it.name + '<span class="file-size"> - '+ sayutils.formatSize(it.size) +'</span>');
                       }
                ...
                if (html !== '') {
                    ...
                    div.prepend('<div class="sayui-enum-placeholder" style="'+ style +'">'+ $(obj.el).attr('placeholder') +'</div>');
                }
                // ITEMS events
                div.off('scroll.sayfield').on('scroll.sayfield', function (event) {
                    ...
                    .on('click', function (event) {
                        ...
                        event.stopPropagation();
                        // default behavior
                        if ($(event.target).hasClass('sayui-list-remove')) {
                            ...
                            if (obj.type == 'file') {
                                ...
                                preview += '<div style="padding: 8px;">'+
                                    ...
                                    '    <tr><td '+ td1 +'>'+ sayutils.lang('Name') +':</td><td '+ td2 +'>'+ item.name +'</td></tr>'+
                                    '    <tr><td '+ td1 +'>'+ sayutils.lang('Size') +':</td><td '+ td2 +'>'+ sayutils.formatSize(item.size) +'</td></tr>'+
                                    '    <tr><td '+ td1 +'>'+ sayutils.lang('Type') +':</td><td '+ td2 +'>' +
                                    ...
                                    '    </td></tr>'+
                                    '    <tr><td '+ td1 +'>'+ sayutils.lang('Modified') +':</td><td '+ td2 +'>'+ sayutils.date(item.modified) +'</td></tr>'+
                                    '    </tbody></table>'+
                                    '</div>';
                                $('#sayui-overlay').remove();
                                $(target).sayoverlay(preview);
                            }                            // event after
                            obj.trigger($.extend(edata, { phase: 'after' }));
                        }
                    })
                ...
                // adjust height
                $(this.el).height('auto');
                var cntHeight = $(div).find('> div.sayui-multi-items').height() + sayutils.getSize(div, '+height') * 2;
                if (cntHeight < 26) cntHeight = 26;
                if (cntHeight > options.maxHeight) cntHeight = options.maxHeight;
                if (div.length > 0) div[0].scrollTop = 1000;
                var inpHeight = sayutils.getSize($(this.el), 'height') - 2;
        ...
        resize: function () {
            ...
            if (multi) {
                var width = (sayutils.getSize(obj.el, 'width')
        ...
        clean: function (val) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(this.type) != -1) {
                if (typeof val == 'string') {
                    ...
                    val = val.replace(/\s+/g, '').replace(sayutils.settings.groupSymbol, '').replace(sayutils.settings.decimalSymbol, '.');
                }
                ...
                if (val !== '' && sayutils.isFloat(val)) val = Number(val); else val = '';
            }
            return val;
        },
        ...
        format: function (val) {
            ...
            if (options.autoFormat && val !== '') {
                switch (this.type) {
                    case 'money':
                    case 'currency':
                        val = sayutils.formatNumber(val, options.currencyPrecision, options.groupSymbol);
                        if (val !== '') val = options.currencyPrefix + val + options.currencySuffix;
                        break;
                    case 'percent':
                        val = sayutils.formatNumber(val, options.precision, options.groupSymbol);
                        if (val !== '') val += '%';
                        break;
                    case 'float':
                        val = sayutils.formatNumber(val, options.precision, options.groupSymbol);
                        break;
                    case 'int':
                        val = sayutils.formatNumber(val, 0, options.groupSymbol);
                        break;
                }
            }
            return val;
        ...
        change: function (event) {
            ...
            if (['date', 'time', 'datetime'].indexOf(this.type) != -1) {
                ...
                if (sayutils.isInt(obj.el.value) && tmp > 3000) {
                    if (this.type == 'time') $(obj.el).val(sayutils.formatTime(new Date(tmp), options.format)).change();
                    if (this.type == 'date') $(obj.el).val(sayutils.formatDate(new Date(tmp), options.format)).change();
                    if (this.type == 'datetime') $(obj.el).val(sayutils.formatDateTime(new Date(tmp), options.format)).change();
                }
            }
        },
        ...
        focus: function (event) {
            ...
            if (['color', 'date', 'time', 'datetime'].indexOf(obj.type) !== -1) {
                if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                if ($("#sayui-overlay").length > 0) $('#sayui-overlay')[0].hide();
        ...
        blur: function (event) {
            ...
            if (['int', 'float', 'money', 'currency', 'percent'].indexOf(obj.type) != -1) {
                if (val !== '' && !obj.checkType(val)) {
                    ...
                    if (options.silent === false) {
                        $(obj.el).saytag('Not a valid number');
                        setTimeout(function () { $(obj.el).saytag(''); }, 3000);
                    }
                }
            }
            // date or time
            if (['date', 'time', 'datetime'].indexOf(obj.type) != -1) {
                ...
                if (val !== '' && !obj.inRange(obj.el.value)) {
                    ...
                    if (options.silent === false) {
                        $(obj.el).saytag('Not in range');
                        setTimeout(function () { $(obj.el).saytag(''); }, 3000);
                    }
                } else {
                    if (obj.type == 'date' && val !== '' && !sayutils.isDate(obj.el.value, options.format)) {
                        $(obj.el).val('').removeData('selected').change();
                        if (options.silent === false) {
                            $(obj.el).saytag('Not a valid date');
                            setTimeout(function () { $(obj.el).saytag(''); }, 3000);
                        }
                    }
                    else if (obj.type == 'time' && val !== '' && !sayutils.isTime(obj.el.value)) {
                        $(obj.el).val('').removeData('selected').change();
                        if (options.silent === false) {
                            $(obj.el).saytag('Not a valid time');
                            setTimeout(function () { $(obj.el).saytag(''); }, 3000);
                        }
                    }
                    else if (obj.type == 'datetime' && val !== '' && !sayutils.isDateTime(obj.el.value, options.format)) {
                        $(obj.el).val('').removeData('selected').change();
                        if (options.silent === false) {
                            $(obj.el).saytag('Not a valid date');
                            setTimeout(function () { $(obj.el).saytag(''); }, 3000);
        ...
        keyDown: function (event, extra) {
            ...
            if (obj.type == 'date') {
                ...
                var inc     = 1;
                if (event.ctrlKey || event.metaKey) inc = 10;
                var dt = sayutils.isDate($(obj.el).val(), options.format, true);
                if (!dt) { dt = new Date(); daymil = 0; }
                switch (key) {
                    case 38: // up
                        if (event.shiftKey) break; // no action if shift key is pressed
                        var newDT = sayutils.formatDate(dt.getTime() + daymil, options.format);
                        if (inc == 10) newDT = sayutils.formatDate(new Date(dt.getFullYear(), dt.getMonth()+1, dt.getDate()), options.format);
            ...
            if (obj.type == 'datetime') {
                ...
                if (event.ctrlKey || event.metaKey) inc = 10;
                var str = $(obj.el).val();
                var dt  = sayutils.isDateTime(str, this.options.format, true);
                if (!dt) { dt = new Date(); daymil = 0; }
                switch (key) {
                    case 38: // up
                        if (event.shiftKey) break; // no action if shift key is pressed
                        var newDT = sayutils.formatDateTime(dt.getTime() + daymil, options.format);
                        if (inc == 10) newDT = sayutils.formatDateTime(new Date(dt.getFullYear(), dt.getMonth()+1, dt.getDate()), options.format);
                        $(obj.el).val(newDT).change();
                        cancel = true;
                        break;
                    case 40: // down
            ...
            if (['list', 'combo', 'enum'].indexOf(obj.type) != -1) {
                ...
                switch (key) {
                    ...
                    case 39: // right
                        ...
                        break;
                    case 13: // enter
                        if ($('#sayui-overlay').length === 0) break; // no action if overlay not open
                    ...
                    case 38: // up
                        ...
                        break;
                    case 40: // down
                        options.index = sayutils.isInt(options.index) ? parseInt(options.index) : -1;
                        options.index++;
                        while (options.index < options.items.length-1 && options.items[options.index].hidden) options.index++;
                        if (options.index == options.items.length-1 && options.items[options.index].hidden) {
                            while (options.items[options.index] && options.items[options.index].hidden) options.index--;
                        ...
                        if (focus.val() === '' && $('#sayui-overlay').length === 0) {
        ...
        keyUp: function (event) {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ([16, 17, 18, 20, 37, 39, 91].indexOf(event.keyCode) == -1) { // no refreah on crtl, shift, left/right arrows, etc
                    ...
                    if ($('#sayui-overlay').length === 0 || [38, 40].indexOf(event.keyCode) == -1) { // no search on arrows
                        this.search();
                    }
                    // event after
                    this.trigger($.extend(edata, { phase: 'after' }));
        ...
        search: function () {
            ...
            if (obj.tmp.xhr_loading !== true) {
                ...
                obj.updateOverlay();
                setTimeout(function () {
                    var html = $('#sayui-overlay').html() || '';
                    if (options.markSearch && html.indexOf('$.fn.saymenuHandler') != -1) { // do not highlight when no items
                        $('#sayui-overlay').saymarker(search);
                    }
                }, 1);
            } else {
        ...
        updateOverlay: function (indexOnly) {
            ...
            if (this.type == 'color') {
                if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                $(this.el).saycolor({ color: $(this.el).val(), transparent: options.transparent }, function (color) {
            ...
            if (this.type == 'date') {
                if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                if ($('#sayui-overlay').length === 0) {
                    $(obj.el).sayoverlay('<div class="sayui-reset sayui-calendar" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#f5f5f5" }
                    });
                }
                var month, year;
                var dt = sayutils.isDate($(obj.el).val(), obj.options.format, true);
                if (dt) { month = dt.getMonth() + 1; year = dt.getFullYear(); }
                (function refreshCalendar(month, year) {
                    $('#sayui-overlay > div > div').html(obj.getMonthHTML(month, year, $(obj.el).val()));
                    $('#sayui-overlay .sayui-calendar-title')
                        .on('mousedown', function () {
                            if ($(this).next().hasClass('sayui-calendar-jump')) {
                                $(this).next().remove();
                            } else {
                                var selYear, selMonth;
                                $(this).after('<div class="sayui-calendar-jump" style=""></div>');
                                $(this).next().hide().html(obj.getYearHTML()).fadeIn(200);
                                setTimeout(function () {
                                    $('#sayui-overlay .sayui-calendar-jump')
                                        .find('.sayui-jump-month, .sayui-jump-year')
                                        .on('click', function () {
                                            if ($(this).hasClass('sayui-jump-month')) {
                                                $(this).parent().find('.sayui-jump-month').removeClass('selected');
                                                $(this).addClass('selected');
                                                selMonth = $(this).attr('name');
                                            }
                                            if ($(this).hasClass('sayui-jump-year')) {
                                                $(this).parent().find('.sayui-jump-year').removeClass('selected');
                                                $(this).addClass('selected');
                                                selYear = $(this).attr('name');
                                            }
                                            if (selYear != null && selMonth != null) {
                                                $('#sayui-overlay .sayui-calendar-jump').fadeOut(100);
                                                setTimeout(function () { refreshCalendar(parseInt(selMonth)+1, selYear); }, 100);
                                            }
                                        });
                                    $('#sayui-overlay .sayui-calendar-jump >:last-child').prop('scrollTop', 2000);
                                }, 1);
                            }
                        });
                    $('#sayui-overlay .sayui-date')
                        .on('mousedown', function () {
                    ...
                    $('#sayui-overlay .previous').on('mousedown', function () {
                        var tmp = obj.options.current.split('/');
                        tmp[0]  = parseInt(tmp[0]) - 1;
                        refreshCalendar(tmp[0], tmp[1]);
                    });
                    $('#sayui-overlay .next').on('mousedown', function () {
                        var tmp = obj.options.current.split('/');
                        tmp[0]  = parseInt(tmp[0]) + 1;
                        refreshCalendar(tmp[0], tmp[1]);
                    });
            ...
            if (this.type == 'time') {
                if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                if ($('#sayui-overlay').length === 0) {
                    $(obj.el).sayoverlay('<div class="sayui-reset sayui-calendar-time" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#fff" }
                    });
                }
                var h24 = (this.options.format == 'h24');
                $('#sayui-overlay > div').html(obj.getHourHTML());
                $('#sayui-overlay .sayui-time')
                    .on('mousedown', function (event) {
                        $(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                    ...
                    if (this.options.noMinutes == null || this.options.noMinutes === false) {
                        $('#sayui-overlay .sayui-time')
                            .on('mouseup', function () {
                                var hour = $(this).attr('hour');
                                if ($("#sayui-overlay").length > 0) $('#sayui-overlay')[0].hide();
                                $(obj.el).sayoverlay('<div class="sayui-reset sayui-calendar-time"></div>', { css: { "background-color": "#fff" } });
                                $('#sayui-overlay > div').html(obj.getMinHTML(hour));
                                $('#sayui-overlay .sayui-time')
                                    .on('mousedown', function () {
                                        $(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                                    ...
                                    .on('mouseup', function () {
                                        setTimeout(function () { if ($("#sayui-overlay").length > 0) $('#sayui-overlay').removeData('keepOpen')[0].hide(); }, 10);
                                    });
                            });
                    } else {
                        $('#sayui-overlay .sayui-time')
            ...
            if (this.type == 'datetime') {
                ...
                if ($("#sayui-overlay .sayui-time").length > 0) $('#sayui-overlay')[0].hide();
                if ($('#sayui-overlay').length === 0) {
                    $(obj.el).sayoverlay('<div class="sayui-reset sayui-calendar" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#f5f5f5" }
                    });
                ...
                var month, year;
                var dt = sayutils.isDateTime($(obj.el).val(), obj.options.format, true);
                if (dt) { month = dt.getMonth() + 1; year = dt.getFullYear(); }
                var selDate = null;
                (function refreshCalendar(month, year) {
                    $('#sayui-overlay > div > div').html(
                        obj.getMonthHTML(month, year, $(obj.el).val())
                        + (options.btn_now ? '<div class="sayui-calendar-now now">'+ sayutils.lang('Current Date & Time') + '</div>' : '')
                    );
                    $('#sayui-overlay .sayui-calendar-title')
                        .on('mousedown', function () {
                            if ($(this).next().hasClass('sayui-calendar-jump')) {
                        ...
                        .on('mouseup', function () {
                            ...
                            $('#sayui-overlay .sayui-time')
                                .on('mousedown', function (event) {
                                    ...
                                    var txt = sayutils.formatDateTime(selDate, obj.options.format);
                    ...
                    $('#sayui-overlay .now')
                        .on('mousedown', function () {
                            ...
                            var tmp = sayutils.formatDateTime(new Date(), obj.options.format);
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ($(input).is(':focus')) {
                    ...
                    var msgNoItems = sayutils.lang('No matches');
                    if (options.url != null && $(input).val().length < options.minLength && obj.tmp.emptySet !== true) msgNoItems = options.minLength + ' ' + sayutils.lang('letters or more...');
                    if (options.url != null && $(input).val() === '' && obj.tmp.emptySet !== true) msgNoItems = sayutils.lang('Type to search...');
                    if (options.url == null && options.items.length === 0) msgNoItems = sayutils.lang('Empty list');
                    if (options.msgNoItems != null) msgNoItems = options.msgNoItems;
                    if (msgNoItems == 'function') msgNoItems = msgNoItems(options);
                    $(el).saymenu((!indexOnly ? 'refresh' : 'refresh-index'), $.extend(true, {}, options, {
                        search     : false,
        ...
        inRange: function (str, onlyDate) {
            var inRange = false;
            if (this.type == 'date') {
                var dt = sayutils.isDate(str, this.options.format, true);
                if (dt) {
                    // enable range
                    if (this.options.start || this.options.end) {
                        ...
                        var start   = sayutils.isDate(st, this.options.format, true);
                        var end     = sayutils.isDate(en, this.options.format, true);
                ...
                if (dt) {
                    ...
                    if (this.options.start || this.options.end) {
                        ...
                        if (typeof this.options.start == 'object' && this.options.start instanceof Date) {
                            ...
                            var st = (typeof this.options.start == 'string' ? this.options.start : $(this.options.start).val());
                            if (st.trim() !== '') {
                                start = sayutils.isDateTime(st, this.options.format, true);
                            } else {
                                start = '';
                            }
                        }
                        if (typeof this.options.end == 'object' && this.options.end instanceof Date) {
                            ...
                            var en = (typeof this.options.end == 'string' ? this.options.end : $(this.options.end).val());
                            if (en.trim() !== '') {
                                end = sayutils.isDateTime(en, this.options.format, true);
                            } else {
                                end = '';
                            }
                        }
                    ...
                    if (inRange && this.options.blocked) {
                        for (var i=0; i<this.options.blocked.length; i++) {
                            ...
                            if(typeof blocked == 'string') {
                                ...
                                blocked = sayutils.isDateTime(blocked, this.options.format, true);
                            }
        ...
        */
        ...
        checkType: function (ch, loose) {
            ...
            switch (obj.type) {
                case 'int':
                    ...
                    return sayutils.isInt(ch.replace(obj.options.numberRE, ''));
                case 'percent':
                    ch = ch.replace(/%/g, '');
                case 'float':
                    if (loose && ['-', sayutils.settings.decimalSymbol, obj.options.groupSymbol].indexOf(ch) != -1) return true;
                    return sayutils.isFloat(ch.replace(obj.options.numberRE, ''));
                case 'money':
                case 'currency':
                    ...
                    return sayutils.isFloat(ch.replace(obj.options.moneyRE, ''));
                case 'bin':
                    return sayutils.isBin(ch);
                case 'hex':
                case 'color':
                    return sayutils.isHex(ch);
                case 'alphanumeric':
                    return sayutils.isAlphaNumeric(ch);
            }
            return true;
        },
                    ...
                    helper
                        ...
                        .on('click', function (event) {
                            if (obj.options.icon && typeof obj.onIconClick == 'function') {
                                // event before
                                var edata = obj.trigger({ phase: 'before', type: 'iconClick', target: obj.el, el: $(this).find('span.sayui-icon')[0] });
                                if (edata.isCancelled === true) return;
        ...
        addSuffix: function () {
            ...
            setTimeout(function () {
                ...
                if (obj.options.arrows) {
                    ...
                    // add fresh
                    $(obj.el).after(
                        '<div class="sayui-field-helper" style="border: 1px solid transparent">&#160;'+
                        '    <div class="sayui-field-up" type="up">'+
                        '        <div class="arrow-up" type="up"></div>'+
                        '    </div>'+
                        '    <div class="sayui-field-down" type="down">'+
                        '        <div class="arrow-down" type="down"></div>'+
                        '    </div>'+
                        '</div>');
                    var height = sayutils.getSize(obj.el, 'height');
                    helper = $(obj.el).next();
                    helper.css({
                            'color'         : $(obj.el).css('color'),
                            'font-family'   : $(obj.el).css('font-family'),
                ...
                if (obj.options.suffix !== '') {
                    ...
                    helper.css('margin-left', '-'+ (sayutils.getSize(helper, 'width') + parseInt($(obj.el).css('margin-right'), 10) + 2) + 'px');
                    pr += helper.width() + 3;
                    $(obj.el).css('padding-right', pr + 'px');
                    // remember helper
        ...
        addFocus: function () {
            ...
            var html =
                '<div class="sayui-field-helper">'+
                '    <div class="sayui-icon icon-search" style="opacity: 0; display: none"></div>'+
            ...
            helper.find('input')
                .on('click', function (event) {
                    if ($('#sayui-overlay').length === 0) obj.focus(event);
                    event.stopPropagation();
                })
                .on('focus', function (event) {
        ...
        addMulti: function () {
            ...
            var margin =
                ...
                'margin-left    : ' + $(obj.el).css('margin-left') + '; ' +
                'margin-right   : ' + $(obj.el).css('margin-right') + '; '+
                'width          : ' + (sayutils.getSize(obj.el, 'width')
                                    - parseInt($(obj.el).css('margin-left'), 10)
                                    - parseInt($(obj.el).css('margin-right'), 10))
                                    + 'px;';
            if (obj.type == 'enum') {
                ...
                html =  '<div class="sayui-field-helper sayui-list" style="'+ margin + '; box-sizing: border-box">'+
                        '    <div style="padding: 0px; margin: 0px; display: inline-block" class="sayui-multi-items">'+
                        '    <ul>'+
            ...
            if (obj.type == 'file') {
                html =  '<div class="sayui-field-helper sayui-list" style="'+ margin + '; box-sizing: border-box">'+
                        ...
                        '   </div>'+
                        '    <div style="position: absolute; padding: 0px; margin: 0px; display: inline-block" class="sayui-multi-items">'+
            ...
            if (obj.type == 'file') {
                ...
                div.on('click', function (event) {
                    ...
                    .on('dragenter', function (event) {
                        if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                        $(div).addClass('sayui-file-dragover');
                    })
                    .on('dragleave', function (event) {
                        if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                        var tmp = $(event.target).parents('.sayui-field-helper');
                        if (tmp.length === 0) $(div).removeClass('sayui-file-dragover');
                    })
                    .on('drop', function (event) {
                        if ($(obj.el).prop('readonly') || $(obj.el).prop('disabled')) return;
                        $(div).removeClass('sayui-file-dragover');
        ...
        addFile: function (file) {
            ...
            if (options.maxFileSize !== 0 && newItem.size > options.maxFileSize) {
                err = 'Maximum file size is '+ sayutils.formatSize(options.maxFileSize);
                if (options.silent === false) $(obj.el).saytag(err);
                console.log('ERROR: '+ err);
                return;
            }
            if (options.maxSize !== 0 && size + newItem.size > options.maxSize) {
                err = 'Maximum total size is '+ sayutils.formatSize(options.maxSize);
                if (options.silent === false) $(obj.el).saytag(err);
                console.log('ERROR: '+ err);
                return;
            }
        ...
        getMonthHTML: function (month, year, selected) {
            var td          = new Date();
            var months      = sayutils.settings.fullmonths;
            var daysCount   = ['31', '28', '31', '30', '31', '30', '31', '31', '30', '31', '30', '31'];
            var today       = td.getFullYear() + '/' + (Number(td.getMonth()) + 1) + '/' + td.getDate();
            var days        = sayutils.settings.fulldays.slice();    // creates copy of the array
            var sdays       = sayutils.settings.shortdays.slice();   // creates copy of the array
            if (sayutils.settings.weekStarts != 'M') {
            ...
            if (options == null) options = {};
            // normalize date
            year  = sayutils.isInt(year)  ? parseInt(year)  : td.getFullYear();
            month = sayutils.isInt(month) ? parseInt(month) : td.getMonth() + 1;
            ...
            var html  =
                '<div class="sayui-calendar-title title">'+
                '    <div class="sayui-calendar-previous previous"> <div></div> </div>'+
                '    <div class="sayui-calendar-next next"> <div></div> </div> '+
                        months[month-1] +', '+ year +
                '</div>'+
                '<table class="sayui-calendar-days" cellspacing="0"><tbody>'+
                '    <tr class="sayui-day-title">' + dayTitle + '</tr>'+
                '    <tr>';

            var day = 1;
            if (sayutils.settings.weekStarts != 'M') weekDay++;
            if(this.type === 'datetime') {
                var dt_sel = sayutils.isDateTime(selected, options.format, true);
                selected = sayutils.formatDate(dt_sel, sayutils.settings.dateFormat);
            }
            for (var ci = 1; ci < 43; ci++) {
                if (weekDay === 0 && ci == 1) {
                    for (var ti = 0; ti < 6; ti++) html += '<td class="sayui-day-empty">&#160;</td>';
                    ci += 6;
                } else {
                    if (ci < weekDay || day > daysCount[month-1]) {
                        html += '<td class="sayui-day-empty">&#160;</td>';
                ...
                var DT  = new Date(dt);
                var className = '';
                if (DT.getDay() === 6) className  = ' sayui-saturday';
                if (DT.getDay() === 0) className  = ' sayui-sunday';
                if (dt == today) className += ' sayui-today';
                ...
                if(this.type === 'datetime') {
                    ...
                    // tmp_dt      = sayutils.formatDate(dt, fm);
                    tmp_dt      = sayutils.formatDateTime(dt, options.format);
                    tmp_dt_fmt  = sayutils.formatDate(dt, sayutils.settings.dateFormat);
                } else {
                    tmp_dt      = sayutils.formatDate(dt, options.format);
                    tmp_dt_fmt  = tmp_dt;
                }
                ...
                html += '<td class="'+ (this.inRange(tmp_dt, true) ? 'sayui-date ' + (tmp_dt_fmt == selected ? 'sayui-date-selected' : '') : 'sayui-blocked') + className + '" '+
        ...
        getYearHTML: function () {
            var months = sayutils.settings.shortmonths;
            var start_year = sayutils.settings.dateStartYear;
            var end_year = sayutils.settings.dateEndYear;
            var mhtml  = '';
            var yhtml  = '';
            for (var m = 0; m < months.length; m++) {
                mhtml += '<div class="sayui-jump-month" name="'+ m +'">'+ months[m] + '</div>';
            }
            for (var y = start_year; y <= end_year; y++) {
                yhtml += '<div class="sayui-jump-year" name="'+ y +'">'+ y + '</div>';
            }
        ...
        getHourHTML: function () {
            var tmp = [];
            var options = this.options;
            if (options == null) options = { format: sayutils.settings.timeFormat };
            ...
            for (var a = 0; a < 24; a++) {
                ...
                if (this.type === 'datetime') {
                    var dt = sayutils.isDateTime(this.el.value, options.format, true);
                    var fm = options.format.split('|')[0].trim();
                    tm1 = sayutils.formatDate(dt, fm) + ' ' + tm1;
                    tm2 = sayutils.formatDate(dt, fm) + ' ' + tm2;
                }
                tmp[Math.floor(a/8)] += '<div class="'+ (this.inRange(tm1) || this.inRange(tm2) ? 'sayui-time ' : 'sayui-blocked') + '" hour="'+ a +'">'+ time +'</div>';
            }
            var html =
                '<div class="sayui-calendar">'+
                '   <div class="sayui-calendar-title">'+ sayutils.lang('Select Hour') +'</div>'+
                '   <div class="sayui-calendar-time"><table><tbody><tr>'+
                '       <td>'+ tmp[0] +'</td>' +
                '       <td>'+ tmp[1] +'</td>' +
                '       <td>'+ tmp[2] +'</td>' +
                '   </tr></tbody></table></div>'+
        ...
        getMinHTML: function (hour) {
            ...
            for (var a = 0; a < 60; a += 5) {
                ...
                if (this.type === 'datetime') {
                    var dt = sayutils.isDateTime(this.el.value, options.format, true);
                    var fm = options.format.split('|')[0].trim();
                    tm = sayutils.formatDate(dt, fm) + ' ' + tm;
                }
                tmp[ind] += '<div class="'+ (this.inRange(tm) ? 'sayui-time ' : 'sayui-blocked') + '" min="'+ a +'">'+ time +'</div>';
            }
            var html =
                '<div class="sayui-calendar">'+
                '   <div class="sayui-calendar-title">'+ sayutils.lang('Select Minute') +'</div>'+
                '   <div class="sayui-calendar-time"><table><tbody><tr>'+
                '       <td>'+ tmp[0] +'</td>' +
                '       <td>'+ tmp[1] +'</td>' +
                '       <td>'+ tmp[2] +'</td>' +
    ...
    $.extend(sayfield.prototype, sayutils.event);
...
(function ($) {
    var sayform = function(options) {
        ...
        this.postData    = {};
        this.httpHeaders = {};
        this.method      = null;     // only used when not null, otherwise set based on sayutils.settings.dataType
        ...
        $.extend(true, this, sayobj.form, options);
    };

    // ====================================================
    // -- Registers as a jQuery plugin

    $.fn.sayform = function(method) {
        if ($.isPlainObject(method)) {
            var obj = this;
            // check name parameter
            if (!sayutils.checkName(method, 'sayform')) return;
            // remember items
            ...
            if (object.formURL === '') {
                if (String(object.formHTML).indexOf('sayui-page') == -1) {
                    object.formHTML = '<div class="sayui-page page-0">'+ object.formHTML +'</div>';
                }
    ...
    sayform.prototype = {
        ...
        message: function(options) {
            if (typeof options == 'string') {
                options = {
                    ...
                    onOpen  : function (event) {
                        setTimeout(function () {
                            $(event.box).find('.sayui-btn').focus();
                        }, 25);
                    }
                };
            }
            sayutils.message.call(this, {
                box   : this.box,
                path  : 'sayui.' + this.name,
                title : '.sayui-form-header:visible',
                body  : '.sayui-form-box'
            }, options);
        },
        ...
        validate: function (showErrors) {
            ...
            $().saytag(); // hide all tags before validating
            ...
            for (var f = 0; f < this.fields.length; f++) {
                ...
                switch (field.type) {
                    case 'int':
                        if (this.record[field.name] && !sayutils.isInt(this.record[field.name])) {
                            errors.push({ field: field, error: sayutils.lang('Not an integer') });
                        }
                        break;
                    case 'float':
                        if (this.record[field.name] && !sayutils.isFloat(this.record[field.name])) {
                            errors.push({ field: field, error: sayutils.lang('Not a float') });
                        }
                        break;
                    case 'money':
                        if (this.record[field.name] && !sayutils.isMoney(this.record[field.name])) {
                            errors.push({ field: field, error: sayutils.lang('Not in money format') });
                        }
                        break;
                    case 'color':
                    case 'hex':
                        if (this.record[field.name] && !sayutils.isHex(this.record[field.name])) {
                            errors.push({ field: field, error: sayutils.lang('Not a hex number') });
                        }
                        break;
                    case 'email':
                        if (this.record[field.name] && !sayutils.isEmail(this.record[field.name])) {
                            errors.push({ field: field, error: sayutils.lang('Not a valid email') });
                        }
                        break;
                    case 'checkbox':
                        // convert true/false
                    ...
                    case 'date':
                        ...
                        if (!field.options.format) field.options.format = sayutils.settings.dateFormat;
                        if (this.record[field.name] && !sayutils.isDate(this.record[field.name], field.options.format)) {
                            errors.push({ field: field, error: sayutils.lang('Not a valid date') + ': ' + field.options.format });
                        } else {
                        }
                        break;
                    case 'list':
                ...
                if (field.required && (val === '' || ($.isArray(val) && val.length === 0) || ($.isPlainObject(val) && $.isEmptyObject(val)))) {
                    errors.push({ field: field, error: sayutils.lang('Required field') });
                }
                if (field.equalto && this.record[field.name] != this.record[field.equalto]) {
                    errors.push({ field: field, error: sayutils.lang('Field should be equal to ') + field.equalto });
                }
            }
            // event before
            ...
            if (showErrors) {
                for (var e = 0; e < edata.errors.length; e++) {
                    ...
                    if (err.field.type == 'radio') { // for radio and checkboxes
                        $($(err.field.el).parents('div')[0]).saytag(err.error, { "class": 'sayui-error' });
                    } else if (['enum', 'file'].indexOf(err.field.type) != -1) {
                        (function (err) {
                            setTimeout(function () {
                                var fld = $(err.field.el).data('sayfield').helpers.multi;
                                $(err.field.el).saytag(err.error);
                                $(fld).addClass('sayui-error');
                            }, 1);
                        })(err);
                    } else {
                        $(err.field.el).saytag(err.error, { "class": 'sayui-error' });
                    }
        ...
        request: function (postData, callBack) { // if (1) param then it is call back if (2) then postData and callBack
            ...
            this.original = {};
            // call server to get data
            this.lock(sayutils.lang(this.msgRefresh));
            var url = edata.url;
            ...
            switch (sayutils.settings.dataType) {
                case 'HTTP':
                    ...
                    if (status != 'error') {
                        ...
                        if (responseText != null && responseText !== '') {
                            ...
                            if (data == null) {
                                data = {
                                    status       : 'error',
                                    message      : sayutils.lang(obj.msgNotJSON),
                                    responseText : responseText
                                };
                            }
                            if (data['status'] == 'error') {
                                obj.error(sayutils.lang(data['message']));
                            } else {
                        ...
                        data = {
                            status       : 'error',
                            message      : sayutils.lang(obj.msgAJAXerror),
                            responseText : responseText
                        };
                    }
                    // event after
        ...
        save: function (postData, callBack) {
            ...
            obj.lock(sayutils.lang(obj.msgSaving) + ' <span id="'+ obj.name +'_progress"></span>');
        ...
        lock: function (msg, showSpinner) {
            var args = Array.prototype.slice.call(arguments, 0);
            args.unshift(this.box);
            setTimeout(function () { sayutils.lock.apply(window, args); }, 10);
        },

        unlock: function (speed) {
            var box = this.box;
            setTimeout(function () { sayutils.unlock(box, speed); }, 25); // needed timer so if server fast, it will not flash
        },
        ...
        generateHTML: function () {
            ...
            for (var f = 0; f < this.fields.length; f++) {
                ...
                // input control
                var input = '<input name="'+ field.name +'" class="sayui-input" type="text" '+ field.html.attr +' tabindex="'+ (f+1) +'"/>';
                switch (field.type) {
                    case 'pass':
                    case 'password':
                        input = '<input name="' + field.name + '" class="sayui-input" type = "password" ' + field.html.attr + ' tabindex="'+ (f+1) +'"/>';
                        break;
                    case 'checkbox':
                        input = '<input name="'+ field.name +'" class="sayui-input" type="checkbox" '+ field.html.attr +' tabindex="'+ (f+1) +'"/>';
                        break;
                    case 'radio':
                        input = '';
                        // normalized options
                        ...
                        if (items.length > 0) {
                            items = sayobj.field.prototype.normMenu(items);
                        }
                        // generate
                        for (var i = 0; i < items.length; i++) {
                            input += '<label><input name="' + field.name + '" class="sayui-input" type = "radio" ' + field.html.attr + ' value="'+ items[i].id + '"/>' +
                                '&#160;' + items[i].text + '</label><br/>';
                        }
                        break;
                    case 'select':
                        input = '<select name="' + field.name + '" class="sayui-input" ' + field.html.attr + ' tabindex="'+ (f+1) +'">';
                        // normalized options
                        var items =  field.options.items ? field.options.items : field.html.items;
                        if (!$.isArray(items)) items = [];
                        if (items.length > 0) {
                        ...
                        break;
                    case 'textarea':
                        input = '<textarea name="'+ field.name +'" class="sayui-input" '+ field.html.attr +' tabindex="'+ (f+1) +'"></textarea>';
                        break;
                    case 'toggle':
                        input = '<input name="'+ field.name +'" type="checkbox" '+ field.html.attr +' class="sayui-input sayui-toggle" tabindex="'+ (f+1) +'"/><div><div></div></div>';
                        break;
                    case 'html':
                    case 'custom':
                    case 'empty':
                ...
                if (field.html.group && (group != field.html.group)) {
                    html += '\n   <div class="sayui-group-title">'+ field.html.group + '</div>\n   <div class="sayui-group">';
                    group = field.html.group;
                }
                html += '\n      <div class="sayui-field '+ (field.html.span != null ? 'sayui-span'+ field.html.span : '') +'" style="'+ field.html.style +'">'+
                        '\n         <label>' + sayutils.lang(field.html.caption) +'</label>'+
                        '\n         <div>'+ input + sayutils.lang(field.html.text) + '</div>'+
                        '\n      </div>';
            ...
            if (!$.isEmptyObject(this.actions)) {
                ...
                for (var a in this.actions) { // it is an object
                    ...
                    if ($.isPlainObject(act)) {
                        ...
                        info.caption = a;
                        if (['save', 'update', 'create'].indexOf(a.toLowerCase()) != -1) info['class'] = 'sayui-btn-blue'; else info['class'] = '';
                    }
                    buttons += '\n    <button name="'+ a +'" class="sayui-btn '+ info['class'] +'" style="'+ info.style +'">'+
                                            sayutils.lang(info.caption) +'</button>';
                }
                buttons += '\n</div>';
            }
            html = '';
            for (var p = 0; p < pages.length; p++){
                html += '<div class="sayui-page page-'+ p +'" ' + ((p===0)?'':'style="display: none;"') + '><div class="sayui-column-container" style="display: flex;">';
                for (var c = 0; c < pages[p].length; c++){
                    html += '<div class="sayui-column col-'+ c +'">' + (pages[p][c] || '') + '\n</div>';
                }
                html += '\n</div></div>';
            }
            html += buttons;
        ...
        resize: function () {
            ...
            if (edata.isCancelled === true) return;
            // default behaviour
            var main    = $(this.box).find('> div.sayui-form-box');
            var header  = $(this.box).find('> div .sayui-form-header');
            var toolbar = $(this.box).find('> div .sayui-form-toolbar');
            var tabs    = $(this.box).find('> div .sayui-form-tabs');
            var page    = $(this.box).find('> div .sayui-page');
            var cpage   = $(this.box).find('> div .sayui-page.page-'+ this.page);
            var dpage   = $(this.box).find('> div .sayui-page.page-'+ this.page + ' > div');
            var buttons = $(this.box).find('> div .sayui-buttons');
            // if no height, calculate it
            resizeElements();
            if (parseInt($(this.box).height()) === 0 || $(this.box).data('auto-size') === true) {
                $(this.box).height(
                    (header.length > 0 ? sayutils.getSize(header, 'height') : 0) +
                    ((typeof this.tabs === 'object' && $.isArray(this.tabs.tabs) && this.tabs.tabs.length > 0) ? sayutils.getSize(tabs, 'height') : 0) +
                    ((typeof this.toolbar == 'object' && $.isArray(this.toolbar.items) && this.toolbar.items.length > 0) ? sayutils.getSize(toolbar, 'height') : 0) +
                    (page.length > 0 ? sayutils.getSize(dpage, 'height') + sayutils.getSize(cpage, '+height') + 12 : 0) +  // why 12 ???
                    (buttons.length > 0 ? sayutils.getSize(buttons, 'height') : 0)
                );
            ...
            function resizeElements() {
                ...
                toolbar.css('top', (obj.header !== '' ? sayutils.getSize(header, 'height') : 0));
                tabs.css('top', (obj.header !== '' ? sayutils.getSize(header, 'height') : 0)
                              + ((typeof obj.toolbar == 'object' && $.isArray(obj.toolbar.items) && obj.toolbar.items.length > 0) ? sayutils.getSize(toolbar, 'height') : 0));
                page.css('top', (obj.header !== '' ? sayutils.getSize(header, 'height') : 0)
                              + ((typeof obj.toolbar == 'object' && $.isArray(obj.toolbar.items) && obj.toolbar.items.length > 0) ? sayutils.getSize(toolbar, 'height') + 5 : 0)
                              + ((typeof obj.tabs === 'object' && $.isArray(obj.tabs.tabs) && obj.tabs.tabs.length > 0) ? sayutils.getSize(tabs, 'height') + 5 : 0));
                page.css('bottom', (buttons.length > 0 ? sayutils.getSize(buttons, 'height') : 0));
            }
        },
        ...
        refresh: function (field) {
            ...
            if (field != null) {
                ...
                // default action
                $(this.box).find('.sayui-page').hide();
                $(this.box).find('.sayui-page.page-' + this.page).show();
                $(this.box).find('.sayui-form-header').html(this.header);
            ...
            for (var f = from_field; f < to_field; f++) {
                ...
                $(field.$el).off('change').on('change', function () {
                    ...
                    if (['int', 'float', 'percent', 'money', 'currency'].indexOf(field.type) != -1) {
                        value_new = $(this).data('sayfield').clean(value_new);
                    }
                    ...
                    if (['enum', 'file'].indexOf(field.type) != -1) {
                        if (val.length > 0) {
                            var fld = $(field.el).data('sayfield').helpers.multi;
                            $(fld).removeClass('sayui-error');
                        }
                    }
                ...
                // required
                if (field.required) {
                    $(field.el).parent().parent().addClass('sayui-required');
                } else {
                    $(field.el).parent().parent().removeClass('sayui-required');
                }
                // disabled
            ...
            for (var f = from_field; f < to_field; f++) {
                ...
                if (!$(field.el).hasClass('sayui-input')) $(field.el).addClass('sayui-input');
                ...
                switch (field.type) {
                    ...
                    case 'percent':
                        // issue #761
                        field.el.value = value;
                        $(field.el).sayfield($.extend({}, field.options, { type: field.type }));
                        break;
                    case 'hex':
                    case 'alphanumeric':
                    case 'color':
                    ...
                    case 'time':
                        ...
                        break;
                    case 'toggle':
                        if (sayutils.isFloat(value)) value = parseFloat(value);
                    ...
                    case 'combo':
                        if (field.type == 'list') {
                            ...
                            if ($.isArray(items) && items.length > 0 && !$.isPlainObject(items[0])) {
                                field.options.items = sayobj.field.prototype.normMenu(items);
                            }
                        ...
                        $(field.el).sayfield($.extend({}, field.options, { type: field.type, selected: value }));
                        break;
                    case 'enum':
                    case 'file':
            ...
            // wrap pages in div
            var tmp = $(this.box).find('.sayui-page');
            for (var i = 0; i < tmp.length; i++) {
        ...
        render: function (box) {
            ...
            if (typeof box == 'object') {
                ...
                if ($(this.box).find('#form_'+ this.name +'_tabs').length > 0) {
                    $(this.box).removeAttr('name')
                        .removeClass('sayui-reset sayui-form')
                        .html('');
                }
                this.box = box;
            }
            ...
            var html =  '<div class="sayui-form-box">' +
                        (this.header !== '' ? '<div class="sayui-form-header">' + this.header + '</div>' : '') +
                        '    <div id="form_'+ this.name +'_toolbar" class="sayui-form-toolbar" style="display: none"></div>' +
                        '    <div id="form_'+ this.name +'_tabs" class="sayui-form-tabs" style="display: none"></div>' +
                            this.formHTML +
                        '</div>';
            $(this.box).attr('name', this.name)
                .addClass('sayui-reset sayui-form')
                .html(html);
            if ($(this.box).length > 0) $(this.box)[0].style.cssText += this.style;

            // init toolbar regardless it is defined or not
            if (typeof this.toolbar.render !== 'function') {
                this.toolbar = $().saytoolbar($.extend({}, this.toolbar, { name: this.name +'_toolbar', owner: this }));
                this.toolbar.on('click', function (event) {
                    var edata = obj.trigger({ phase: 'before', type: 'toolbar', target: event.target, originalEvent: event });
                    if (edata.isCancelled === true) return;
                    // no default action
            ...
            if (typeof this.tabs.render !== 'function') {
                this.tabs = $().saytabs($.extend({}, this.tabs, { name: this.name +'_tabs', owner: this, active: this.tabs.active }));
            ...
            // attach to resize event
            if ($('.sayui-layout').length === 0) { // if there is layout, it will send a resize event
                this.tmp_resize = function (event) { sayui[obj.name].resize(); };
                $(window).off('resize', 'body').on('resize', 'body', this.tmp_resize);
            }
            // focus on load
            function focusEl() {
                var inputs = $(obj.box).find('div:not(.sayui-field-helper) > input, select, textarea, div > label:nth-child(1) > :radio').not('.file-input');
    ...
    $.extend(sayform.prototype, sayutils.event);
    sayobj.form = sayform;
})(jQuery);
